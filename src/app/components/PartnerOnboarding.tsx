      ...
      {steps.map((s, i) => (
        <React.Fragment key={s.id}>
          <div className="flex items-center gap-1.5">
            ...
            <span style={{ fontSize:11, color:i===idx?"#E8EDF5":"#4A5A7A", fontWeight:i===idx?600:400 }}>{s.label}</span>
          </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background:"#070A12" }}>
        <div className="w-full max-w-lg text-center">
          ...
          <h2 style={{ ...DISPLAY, fontSize:32, color:"#E8EDF5", marginBottom:8 }}>Welcome to the Partner Program!</h2>
          <p style={{ color:"#8A9AB8", fontSize:15, lineHeight:1.9, marginBottom:8 }}>
    ...
    <div className="min-h-screen" style={{ background:"#070A12" }}>
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div style={{ color:"#4A5A7A", fontSize:9, ...MONO }}>PARTNER PORTAL — ONBOARDING</div>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        {step === "overview" && (
          <div className="space-y-6">
            <div className="text-center mb-2">
              <h1 style={{ ...DISPLAY, fontSize:30, color:"#E8EDF5", marginBottom:8 }}>Final Pass Down Partner Program</h1>
              <p style={{ color:"#8A9AB8", fontSize:15, maxWidth:520, margin:"0 auto" }}>
            ...
            {/* Volume tiers */}
            <div>
              <div style={{ color:"#8A9AB8", fontSize:11, ...MONO, marginBottom:12, textAlign:"center" }}>COMMISSION TIERS — ADVANCE AUTOMATICALLY BY VOLUME</div>
              <div className="grid grid-cols-3 gap-4">
                {VOLUME_TIERS.map(t => (
                  <div key={t.tier} className="rounded-2xl p-6 text-center" style={{ ...GLASS, border:`2px solid ${t.color}30`, boxShadow:`0 0 24px ${t.glow}` }}>
                    ...
                    <div style={{ color:"#8A9AB8", fontSize:12, lineHeight:1.5 }}>{t.range}</div>
                ...
                {[
                  ...
                  return (
                    <div key={row.accounts}>
                      <div style={{ color:"#4A5A7A", fontSize:10, ...MONO }}>{row.accounts} ACCOUNTS · TIER {row.tier}</div>
                      <div style={{ ...DISPLAY, fontSize:24, color:"#6E90C9", marginTop:4 }}>${est.toLocaleString()}<span style={{ fontSize:11, color:"#4A5A7A" }}>/mo</span></div>
                    </div>
                  );
                })}
              </div>
              <p style={{ color:"#4A5A7A", fontSize:10, textAlign:"center", marginTop:8 }}>Based on avg Premium plan ($24.99/mo). Actual earnings vary by plan mix.</p>
            </div>
        ...
        {step === "payment" && (
          <div>
            <div className="mb-8">
              <h1 style={{ ...DISPLAY, fontSize:28, color:"#E8EDF5", marginBottom:6 }}>One-Time Setup Fee</h1>
              <p style={{ color:"#8A9AB8", fontSize:14 }}>Pay once. No recurring charges. Your commission account is activated immediately after payment.</p>
            </div>
              ...
              <div className="rounded-2xl p-6" style={{ ...GLASS, border:"2px solid rgba(91,110,225,0.2)" }}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <div style={{ color:"#8A9AB8", fontSize:13 }}>{org.name || "Your Organization"} · {ORG_TYPES.find(t=>t.id===org.type)?.label}</div>
                  </div>
                ...
                <div className="space-y-2 pt-4 border-t" style={{ borderColor:"rgba(91,110,225,0.1)" }}>
                  {[
                    ...
                    <div key={f} className="flex items-center gap-2">
                      ...
                      <span style={{ color:"#8A9AB8", fontSize:12 }}>{f}</span>
                    </div>
                  ))}
                </div>
              </div>
    ...
    <div className="flex overflow-hidden" style={{ height:"calc(100vh - 60px)", fontFamily:"var(--font-body)" }}>
      ...
      <aside className="w-52 flex flex-col flex-shrink-0" style={{ background:"#070A12", borderRight:"1px solid rgba(91,110,225,0.12)" }}>
        ...
        <div className="mx-2 my-2 px-3 py-2.5 rounded-2xl" style={{ background:`${currentTier.color}18`, border:`1px solid ${currentTier.color}40` }}>
          ...
          <div style={{ color:"#8A9AB8", fontSize:8, marginTop:1 }}>{stats.accounts} active accounts</div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between px-5 py-2.5 border-b flex-shrink-0"
          ...
          <div>
            <h1 style={{ ...DISPLAY, fontSize:16, color:"#E8EDF5" }}>Partner Dashboard</h1>
            <div style={{ color:"#4A5A7A", fontSize:9, ...MONO }}>
        ...
        <main className="flex-1 overflow-y-auto p-5" style={{ background:"#070A12" }}>
                ...
                <div className="p-5 rounded-2xl" style={{ ...GLASS, border:`1px solid ${nextTier.color}30`, background:`${nextTier.color}04` }}>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <div style={{ color:"#E8EDF5", fontSize:13, marginTop:2 }}>
                  ...
                  <div style={{ color:"#4A5A7A", fontSize:10, marginTop:4 }}>
                      ...
                      {months.map(d => {
                        ...
                        return (
                          <div key={d.m} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
                            <span style={{ color:"#8A9AB8", fontSize:8, ...MONO }}>${(d.v/1000).toFixed(1)}k</span>
                            ...
                            <span style={{ color:"#4A5A7A", fontSize:8, ...MONO }}>{d.m}</span>
                          </div>
                        );
                      })}
                    </div>
          ...
          {tab === "referrals" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h2 style={{ ...DISPLAY, fontSize:20, color:"#E8EDF5" }}>Your Referrals</h2>
                  ...
                  <thead>
                    <tr style={{ borderBottom:"1px solid rgba(91,110,225,0.08)" }}>
                      {["Ref #","Client","Plan","Joined",`Commission (${currentTier.rate}%)`,"Status"].map(h => (
                        <th key={h} className="px-4 py-3 text-left" style={{ color:"#4A5A7A", fontSize:9, ...MONO }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {DEMO_REFERRALS.map((r, i) => (
                      <tr key={r.id} style={{ borderBottom:i<DEMO_REFERRALS.length-1?"1px solid rgba(91,110,225,0.06)":"none" }}>
                        ...
                        <td className="px-4 py-3" style={{ color:"#E8EDF5", fontSize:12, fontWeight:500 }}>{r.name}</td>
                        <td className="px-4 py-3" style={{ color:"#8A9AB8", fontSize:11 }}>{r.plan}</td>
                        <td className="px-4 py-3" style={{ color:"#8A9AB8", fontSize:10 }}>{r.date}</td>
          ...
          {tab === "payouts" && (
            <div className="space-y-4">
              <h2 style={{ ...DISPLAY, fontSize:20, color:"#E8EDF5" }}>Payout History</h2>
                  ...
                  <tbody>
                    {DEMO_PAYOUTS.map((p, i) => (
                      <tr key={p.ref} style={{ borderBottom:i<DEMO_PAYOUTS.length-1?"1px solid rgba(91,110,225,0.06)":"none" }}>
                        ...
                        <td className="px-4 py-3" style={{ color:"#8A9AB8", fontSize:12 }}>{p.date}</td>
                        <td className="px-4 py-3" style={{ color:"#E8EDF5", fontSize:13, fontWeight:600 }}>${p.amount.toLocaleString()}</td>
                        <td className="px-4 py-3">
          ...
          {tab === "resources" && (
            <div className="space-y-4">
              <h2 style={{ ...DISPLAY, fontSize:20, color:"#E8EDF5" }}>Partner Resources</h2>
              <div className="grid md:grid-cols-2 gap-4">
                {[
          ...
          {tab === "settings" && (
            <div className="space-y-4 max-w-md">
              <h2 style={{ ...DISPLAY, fontSize:20, color:"#E8EDF5" }}>Account Settings</h2>
              {[
