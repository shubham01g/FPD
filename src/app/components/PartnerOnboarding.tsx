...
const VOLUME_TIERS = [
  ...
  { tier:2, label:"Tier 2", range:"51 – 100 active accounts", rate:25, color:"#6C5CE7", glow:"rgba(108,92,231,0.2)" },
        ...
        <React.Fragment key={s.id}>
          <div className="flex items-center gap-1.5">
            <div className="flex items-center justify-center rounded-full text-xs font-bold"
              style={{ width:22, height:22, background:i<=idx?"#6C5CE7":"rgba(108,92,231,0.1)", color:i<=idx?"#fff":"#8A9AB8" }}>
      ...
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background:"#F0F4FA" }}>
        <div className="w-full max-w-lg text-center">
          ...
          <button onClick={onComplete} className="w-full py-4 rounded-2xl font-bold text-base"
            style={{ background:"linear-gradient(135deg,#6C5CE7,#8B7CF6)", color:"#F0F4FA", boxShadow:"0 0 32px rgba(108,92,231,0.35)" }}>
  ...
  return (
    ...
    <div className="min-h-screen" style={{ background:"#F0F4FA" }}>
      ...
      <div className="sticky top-0 z-20 px-6 py-4 border-b"
        style={{ background:"rgba(255,255,255,0.98)", borderColor:"rgba(108,92,231,0.1)", backdropFilter:"blur(16px)" }}>
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src={fpdSquareLogo} alt="FPD" style={{ width:32, height:32, borderRadius:8, objectFit:"cover" }}/>
            <div>
              <div style={{ ...DISPLAY, color:"#6C5CE7", fontSize:11, fontWeight:700, letterSpacing:"0.06em" }}>FINAL PASS DOWN</div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl p-7 text-center glow-surface" style={{ ...GLASS, border:"2px solid rgba(108,92,231,0.2)", background:"rgba(108,92,231,0.02)" }}>
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={f.label} className="flex flex-col items-center gap-2 py-3 rounded-xl" style={{ background:"rgba(108,92,231,0.06)" }}>
                    <span style={{ color:"#6C5CE7" }}>{f.icon}</span>
                ...
                {[
                  ...
                  return (
                    <div key={row.accounts}>
                      ...
                      <div style={{ ...DISPLAY, fontSize:24, color:"#6C5CE7", marginTop:4 }}>${est.toLocaleString()}<span style={{ fontSize:11, color:"#8A9AB8" }}>/mo</span></div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="flex justify-end">
              <button onClick={() => setStep("details")} className="flex items-center gap-2 px-8 py-4 rounded-2xl font-bold"
                style={{ background:"linear-gradient(135deg,#6C5CE7,#8B7CF6)", color:"#F0F4FA", boxShadow:"0 0 24px rgba(108,92,231,0.35)" }}>
                ...
                <div className="flex flex-wrap gap-2">
                  {ORG_TYPES.map(t => (
                    <button key={t.id} onClick={() => setOrg(o=>({...o,type:t.id}))}
                      ...
                      style={{ background:org.type===t.id?"rgba(108,92,231,0.1)":"rgba(108,92,231,0.04)", border:`1px solid ${org.type===t.id?"#6C5CE7":"rgba(108,92,231,0.12)"}`, color:org.type===t.id?"#6C5CE7":"#5A6A88" }}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type||"text"} value={(org as any)[f.key]} onChange={e=>setOrg(o=>({...o,[f.key]:e.target.value}))} placeholder={f.ph}
                    ...
                    style={{ background:"rgba(108,92,231,0.05)", border:"1px solid rgba(108,92,231,0.2)", color:"#0D1428", fontSize:14, outline:"none" }}/>
                </div>
              ))}
              <div>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl p-6 glow-surface" style={{ ...GLASS, border:"2px solid rgba(108,92,231,0.2)" }}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div style={{ ...DISPLAY, fontSize:32, color:"#6C5CE7" }}>${SETUP_FEE}</div>
                </div>
                <div className="space-y-2 pt-4 border-t" style={{ borderColor:"rgba(108,92,231,0.1)" }}>
                  {[
              ...
              <div className="rounded-2xl p-6 space-y-4 glow-surface" style={GLASS}>
                ...
                <div>
                  ...
                  <input placeholder="4242 4242 4242 4242" maxLength={19}
                    className="w-full px-4 py-3 rounded-xl"
                    style={{ background:"rgba(108,92,231,0.05)", border:"1px solid rgba(108,92,231,0.2)", color:"#0D1428", fontSize:14, outline:"none", ...MONO }}/>
                </div>
            ...
            <div className="flex justify-between mt-6">
              ...
              <button onClick={submitPayment} disabled={paying} className="flex items-center gap-2 px-6 py-4 rounded-2xl font-bold text-base"
                style={{ background:"linear-gradient(135deg,#6C5CE7,#8B7CF6)", color:"#F0F4FA", boxShadow:"0 0 28px rgba(108,92,231,0.4)", opacity:paying?0.7:1 }}>
...
function PartnerDashboard({ onSignOut }: { onSignOut: () => void }) {
  ...
  return (
    <div className="flex overflow-hidden" style={{ height:"calc(100vh - 60px)", fontFamily:"var(--font-body)" }}>
      {/* Sidebar */}
      <aside className="w-52 flex flex-col flex-shrink-0" style={{ background:"#0A0A0F", borderRight:"1px solid rgba(108,92,231,0.12)" }}>
        <div className="px-4 py-4 border-b" style={{ borderColor:"rgba(108,92,231,0.12)" }}>
          <div className="flex items-center gap-2.5">
            <img src={fpdSquareLogo} alt="FPD" style={{ width:28, height:28, borderRadius:7, objectFit:"cover" }}/>
            <div>
              <div style={{ ...DISPLAY, color:"#A29BFE", fontSize:9, fontWeight:700, letterSpacing:"0.06em" }}>FINAL PASS DOWN</div>
        ...
        <nav className="flex-1 px-2 py-2 space-y-0.5">
          {navItems.map(item => (
            <button key={item.id} onClick={() => setTab(item.id)}
              ...
              style={{ background:tab===item.id?"rgba(108,92,231,0.12)":"transparent", color:tab===item.id?"#FFFFFF":"#B0C0DC", borderLeft:`2px solid ${tab===item.id?"#6C5CE7":"transparent"}` }}>
              <span style={{ color:tab===item.id?"#A29BFE":"inherit" }}>{item.icon}</span>
              <span style={{ fontSize:12, fontWeight:tab===item.id?600:400 }}>{item.label}</span>
            </button>
          ))}
        </nav>

        <div className="px-2 py-3 border-t" style={{ borderColor:"rgba(108,92,231,0.1)" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between px-5 py-2.5 border-b flex-shrink-0"
          style={{ background:"rgba(255,255,255,0.98)", borderColor:"rgba(108,92,231,0.1)" }}>
          <div>
                ...
                <div className="p-5 rounded-2xl space-y-2 glow-surface" style={GLASS}>
                  ...
                  {[
                    ...
                    <button key={a.label} onClick={a.fn} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-left"
                      style={{ background:"rgba(108,92,231,0.05)", border:"1px solid rgba(108,92,231,0.1)", color:"#6C5CE7" }}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {DEMO_REFERRALS.map((r, i) => (
                      <tr key={r.id} style={{ borderBottom:i<DEMO_REFERRALS.length-1?"1px solid rgba(108,92,231,0.06)":"none" }}>
                        <td className="px-4 py-3" style={{ color:"#6C5CE7", fontSize:10, ...MONO }}>{r.id}</td>
          ...
          {tab === "payouts" && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label:"Total Earned",  value:`$${stats.totalEarned.toLocaleString()}`,  color:"#6C5CE7" },
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {DEMO_PAYOUTS.map((p, i) => (
                      <tr key={p.ref} style={{ borderBottom:i<DEMO_PAYOUTS.length-1?"1px solid rgba(108,92,231,0.06)":"none" }}>
                        <td className="px-4 py-3" style={{ color:"#6C5CE7", fontSize:10, ...MONO }}>{p.ref}</td>
              ...
              {[
                ...
                <div key={f.label} className="flex items-center justify-between p-4 rounded-xl glow-surface" style={GLASS}>
                  ...
                  {f.label !== "Setup Fee Paid" && f.label !== "Current Tier" && (
                    <button onClick={() => toast.success(`Edit ${f.label} — coming soon`)} style={{ color:"#6C5CE7", fontSize:11 }}>Edit</button>
                  )}
                </div>
              ))}
            </div>
