...
const CARD: React.CSSProperties = { background:"#FFFFFF", border:"1px solid rgba(159,122,234,0.12)", boxShadow:"0 2px 12px rgba(159,122,234,0.06)", borderRadius:16 };
            ...
            <div className="grid grid-cols-3 gap-2">
              {(Object.entries(ROLE_LABELS) as [StaffRole, string][]).map(([id, label]) => (
                <button key={id} onClick={() => setForm(p => ({ ...p, role:id }))}
                  ...
                  style={{ background:form.role===id?`${ROLE_COLORS[id]}15`:"rgba(159,122,234,0.04)",
                    border:`1px solid ${form.role===id?ROLE_COLORS[id]:"rgba(159,122,234,0.12)"}`,
              ...
              {AVAILABLE_CLIENTS.map(c => {
                ...
                return (
                  <button key={c.id} onClick={() => setForm(p => ({
                    ...
                    style={{ background:isAssigned?"rgba(159,122,234,0.08)":"rgba(159,122,234,0.03)",
                      border:`1px solid ${isAssigned?"rgba(159,122,234,0.35)":"rgba(159,122,234,0.1)"}` }}>
                    <div className="flex items-center justify-center rounded-full"
                      style={{ width:20, height:20, background:isAssigned?"#9F7AEA":"#fff",
                        border:`2px solid ${isAssigned?"#9F7AEA":"rgba(159,122,234,0.3)"}`, flexShrink:0 }}>
              ...
              {AVAILABLE_CLIENTS.map(c => {
                ...
                return (
                  <button key={c.id} onClick={() => {
                    ...
                    style={{ background:isAssigned?"rgba(159,122,234,0.08)":"rgba(159,122,234,0.03)",
                      border:`1px solid ${isAssigned?"rgba(159,122,234,0.3)":"rgba(159,122,234,0.1)"}` }}>
                    <div className="flex items-center justify-center rounded-full"
                      style={{ width:18, height:18, background:isAssigned?"#9F7AEA":"#fff",
                        border:`2px solid ${isAssigned?"#9F7AEA":"rgba(159,122,234,0.3)"}`, flexShrink:0 }}>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl"
              ...
              <button onClick={() => { copyToClipboard(portalLink); toast.success("Portal link copied"); }}
                style={{ color:"#9F7AEA", flexShrink:0 }}><Copy size={12}/></button>
            </div>
...
export function ConciergeStaffAdmin() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div style={{ color:"#9F7AEA", fontSize:11, ...MONO, letterSpacing:"0.1em", marginBottom:4 }}>WHITE GLOVE · CONCIERGE STAFF</div>
        ...
        <div className="grid md:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.step} className="flex items-start gap-2.5">
              <div className="flex items-center justify-center rounded-full font-bold flex-shrink-0"
                style={{ width:22, height:22, background:"#9F7AEA", color:"#fff", fontSize:11 }}>{s.step}</div>
