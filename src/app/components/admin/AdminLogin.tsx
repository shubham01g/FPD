  ...
  return (
    <div className="min-h-screen flex" style={{ background: "#030710", fontFamily: "var(--font-body)" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 relative overflow-hidden"
        style={{ background: "linear-gradient(145deg,#0A0F2E 0%,#030710 100%)", borderRight: "1px solid rgba(110,139,255,0.15)" }}>
        {/* Grid */}
        <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(110,139,255,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(110,139,255,0.04) 1px,transparent 1px)", backgroundSize: "50px 50px" }} />
        {/* Orbs */}
        <div style={{ position:"absolute", top:"15%", left:"10%", width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle,rgba(110,139,255,0.1) 0%,transparent 70%)", pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:"10%", right:"5%", width:300, height:300, borderRadius:"50%", background:"radial-gradient(circle,rgba(58,91,217,0.07) 0%,transparent 70%)", pointerEvents:"none" }} />

        <div className="relative">
          <div className="flex items-center gap-3 mb-16">
            <img src={fpdSquareLogo} alt="FPD" style={{ width:40, height:40, borderRadius:10, objectFit:"contain", boxShadow:"0 0 20px rgba(110,139,255,0.3)" }}/>
            <div>
              <div style={{ fontFamily:"var(--font-display)", color:"#6E8BFF", fontSize:13, fontWeight:700, letterSpacing:"0.06em" }}>FINAL PASS DOWN</div>
              <div style={{ color:"#34456A", fontSize:9, letterSpacing:"0.15em", ...MONO }}>PLATFORM ADMINISTRATION</div>
            </div>
          </div>
          <Crown size={56} color="#6E8BFF" style={{ marginBottom:24, opacity:0.8 }}/>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:"clamp(2rem,4vw,3rem)", color:"#E8EDF5", lineHeight:1.15, marginBottom:20 }}>
            Admin<br /><span style={{ color:"#6E8BFF" }}>Command Center</span>
          </h1>
        ...
        <div className="relative space-y-4">
          {[
            ...
            <div key={item.label} className="flex items-center gap-3 px-4 py-3 rounded-xl"
              style={{ background:"rgba(110,139,255,0.06)", border:"1px solid rgba(110,139,255,0.15)" }}>
              <span style={{ fontSize:18 }}>{item.icon}</span>
              <span style={{ color:"#B8C8E0", fontSize:13 }}>{item.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-3 mb-10 lg:hidden">
            ...
            <div style={{ fontFamily:"var(--font-display)", color:"#6E8BFF", fontSize:12, fontWeight:700, letterSpacing:"0.06em" }}>ADMIN PORTAL</div>
          </div>

          {!mfa ? (
            <>
              <div className="mb-8">
                <h2 style={{ fontFamily:"var(--font-display)", fontSize:28, color:"#E8EDF5", marginBottom:8 }}>Administrator Sign In</h2>
                <p style={{ color:"#6B7FA8", fontSize:14 }}>This portal is restricted to authorised Final Pass Down administrators.</p>
              </div>
              ...
              <div className="mb-3 px-4 py-3 rounded-xl" style={{ background:"rgba(110,139,255,0.08)", border:"1px solid rgba(110,139,255,0.2)" }}>
                <p style={{ color:"#6E8BFF", fontSize:11, ...MONO }}>DEMO CREDENTIALS</p>
                <p style={{ color:"#B8C8E0", fontSize:12, marginTop:2 }}>admin@finalpassdown.com / Admin2026!</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label style={{ color:"#6B7FA8", fontSize:11, ...MONO, display:"block", marginBottom:6 }}>ADMIN EMAIL</label>
                ...
                <button type="submit" disabled={loading} className="w-full py-4 rounded-xl font-bold text-sm mt-2"
                  style={{ background: loading ? "rgba(110,139,255,0.2)" : "linear-gradient(135deg,#6E8BFF,#B8C6F5)", color: loading ? "#6E8BFF" : "#04080F", fontSize:15, boxShadow: loading ? "none" : "0 0 30px rgba(110,139,255,0.35)", cursor: loading ? "not-allowed" : "pointer" }}>
            ...
            <>
              <div className="mb-8 text-center">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5"
                  style={{ background:"rgba(110,139,255,0.12)", border:"1px solid rgba(110,139,255,0.3)", boxShadow:"0 0 30px rgba(110,139,255,0.15)" }}>
                  <Lock size={28} color="#6E8BFF"/>
                </div>
                ...
                <p style={{ color:"#6E8BFF", fontSize:12, marginTop:6, ...MONO }}>Demo: use any 6 digits</p>
              </div>
              {error && (
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-5" style={{ background:"rgba(252,129,129,0.1)", border:"1px solid rgba(252,129,129,0.25)" }}>
                  <AlertCircle size={15} color="#FC8181"/>
              ...
              <form onSubmit={handleMfa}>
                <input type="text" value={mfaCode} onChange={e=>setMfaCode(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="000000" maxLength={6}
                  ...
                  style={{ background:"rgba(110,139,255,0.06)", border:"2px solid rgba(110,139,255,0.3)", color:"#6E8BFF", fontSize:32, outline:"none", letterSpacing:"0.5em", ...MONO }}/>
                <button type="submit" disabled={loading || mfaCode.length < 6} className="w-full py-4 rounded-xl font-bold text-sm"
                  style={{ background: (loading || mfaCode.length < 6) ? "rgba(110,139,255,0.2)" : "linear-gradient(135deg,#6E8BFF,#B8C6F5)", color:(loading || mfaCode.length < 6)?"#6E8BFF":"#04080F", fontSize:15, boxShadow:(loading || mfaCode.length < 6)?"none":"0 0 30px rgba(110,139,255,0.35)" }}>
