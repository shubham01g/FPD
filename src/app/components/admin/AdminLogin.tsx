  ...
  return (
    <div className="min-h-screen flex" style={{ background: "#030710", fontFamily: "var(--font-body)" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 relative overflow-hidden"
        style={{ background: "linear-gradient(145deg,#0A0F2E 0%,#030710 100%)", borderRight: "1px solid rgba(159,122,234,0.15)" }}>
        {/* Grid */}
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-16">
            <img src={fpdSquareLogo} alt="FPD" style={{ width:40, height:40, borderRadius:10, objectFit:"cover", boxShadow:"0 0 20px rgba(159,122,234,0.3)" }}/>
            <div>
              <div style={{ fontFamily:"var(--font-display)", color:"#9F7AEA", fontSize:13, fontWeight:700, letterSpacing:"0.06em" }}>FINAL PASS DOWN</div>
              <div style={{ color:"#4A3A6A", fontSize:9, letterSpacing:"0.15em", ...MONO }}>PLATFORM ADMINISTRATION</div>
            </div>
          </div>
          <Crown size={56} color="#9F7AEA" style={{ marginBottom:24, opacity:0.8 }}/>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:"clamp(2rem,4vw,3rem)", color:"#E8EDF5", lineHeight:1.15, marginBottom:20 }}>
            Admin<br /><span style={{ color:"#9F7AEA" }}>Command Center</span>
          </h1>
        ...
        <div className="relative space-y-4">
          {[
            ...
            <div key={item.label} className="flex items-center gap-3 px-4 py-3 rounded-xl"
              style={{ background:"rgba(159,122,234,0.06)", border:"1px solid rgba(159,122,234,0.15)" }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-3 mb-10 lg:hidden">
            ...
            <div style={{ fontFamily:"var(--font-display)", color:"#9F7AEA", fontSize:12, fontWeight:700, letterSpacing:"0.06em" }}>ADMIN PORTAL</div>
          </div>
          ...
          {!mfa ? (
            <>
              ...
              <div className="mb-3 px-4 py-3 rounded-xl" style={{ background:"rgba(159,122,234,0.08)", border:"1px solid rgba(159,122,234,0.2)" }}>
                <p style={{ color:"#9F7AEA", fontSize:11, ...MONO }}>DEMO CREDENTIALS</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@finalpassdown.com"
                    ...
                    style={{ background:"rgba(159,122,234,0.06)", border:"1px solid rgba(159,122,234,0.25)", color:"#E8EDF5", fontSize:14, outline:"none" }}/>
                </div>
                <div>
                ...
                <button type="submit" disabled={loading} className="w-full py-4 rounded-xl font-bold text-sm mt-2"
                  style={{ background: loading ? "rgba(159,122,234,0.2)" : "linear-gradient(135deg,#9F7AEA,#C4B5FD)", color: loading ? "#9F7AEA" : "#04080F", fontSize:15, boxShadow: loading ? "none" : "0 0 30px rgba(159,122,234,0.35)", cursor: loading ? "not-allowed" : "pointer" }}>
            ...
            <>
              <div className="mb-8 text-center">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5"
                  style={{ background:"rgba(159,122,234,0.12)", border:"1px solid rgba(159,122,234,0.3)", boxShadow:"0 0 30px rgba(159,122,234,0.15)" }}>
                  <Lock size={28} color="#9F7AEA"/>
                </div>
                ...
                <p style={{ color:"#9F7AEA", fontSize:12, marginTop:6, ...MONO }}>Demo: use any 6 digits</p>
              </div>
              {error && (
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-5" style={{ background:"rgba(252,129,129,0.1)", border:"1px solid rgba(252,129,129,0.25)" }}>
                  <AlertCircle size={15} color="#FC8181"/>
              ...
              <form onSubmit={handleMfa}>
                <input type="text" value={mfaCode} onChange={e=>setMfaCode(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="000000" maxLength={6}
                  ...
                  style={{ background:"rgba(159,122,234,0.06)", border:"2px solid rgba(159,122,234,0.3)", color:"#9F7AEA", fontSize:32, outline:"none", letterSpacing:"0.5em", ...MONO }}/>
                <button type="submit" disabled={loading || mfaCode.length < 6} className="w-full py-4 rounded-xl font-bold text-sm"
                  style={{ background: (loading || mfaCode.length < 6) ? "rgba(159,122,234,0.2)" : "linear-gradient(135deg,#9F7AEA,#C4B5FD)", color:(loading || mfaCode.length < 6)?"#9F7AEA":"#04080F", fontSize:15, boxShadow:(loading || mfaCode.length < 6)?"none":"0 0 30px rgba(159,122,234,0.35)" }}>
