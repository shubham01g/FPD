  ...
  const NavBtn = ({ item }: { item: AdminNavItem }) => (
    <button onClick={() => onNavigate(item.id)}
      ...
      style={{
        ...
        background: isActive(item.id) ? "rgba(159,122,234,0.15)" : "transparent",
        color: isActive(item.id) ? "#C4B5FD" : "rgba(255,255,255,0.65)",
        borderLeft: isActive(item.id) ? "2px solid #9F7AEA" : "2px solid transparent",
        boxShadow: isActive(item.id) ? "inset 0 0 20px rgba(159,122,234,0.05)" : "none",
      }}>
      <span style={{ flexShrink:0, color: isActive(item.id) ? "#9F7AEA" : "inherit" }}>{item.icon}</span>
      {open && (
        <>
    ...
    <div className="flex overflow-hidden" style={{ background:"#EAF0FC", fontFamily:"var(--font-body)", height:"100vh" }}>
      ...
      <aside className="flex flex-col transition-all duration-300 flex-shrink-0"
        ...
        <div className="flex items-center gap-2.5 px-3 py-3 border-b" style={{ borderColor:"rgba(159,122,234,0.12)" }}>
          <img src={fpdSquareLogo} alt="FPD" style={{ width:32, height:32, borderRadius:7, objectFit:"cover", flexShrink:0, boxShadow:"0 0 16px rgba(159,122,234,0.3)" }}/>
          {open && (
            <div className="flex-1 min-w-0">
              <div style={{ fontFamily:"var(--font-display)", color:"#9F7AEA", fontSize:10, fontWeight:700, letterSpacing:"0.06em", whiteSpace:"nowrap" }}>ADMIN PORTAL</div>
        ...
        {open && (
          <div className="mx-3 my-2 px-3 py-2 rounded-xl flex items-center gap-2"
            ...
            <Crown size={13} color="#9F7AEA"/>
            <div>
              <div style={{ color:"#E8EDF5", fontSize:11, fontWeight:600 }}>Admin Session</div>
              <div style={{ color:"#9F7AEA", fontSize:9, fontFamily:"var(--font-mono)" }}>admin@finalpassdown.com</div>
            </div>
          </div>
        )}
        ...
        <header className="flex items-center justify-between px-5 py-2.5 border-b flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full"
              ...
              <Activity size={10} color="#9F7AEA"/>
              <span style={{ color:"#9F7AEA", fontSize:9, fontFamily:"var(--font-mono)", fontWeight:700 }}>ADMIN SESSION ACTIVE</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center justify-center rounded-full"
              style={{ width:28, height:28, background:"rgba(159,122,234,0.15)", color:"#9F7AEA", fontSize:11, fontWeight:700 }}>A</div>
          </div>
        </header>
