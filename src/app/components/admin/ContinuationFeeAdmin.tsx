      ...
      <div style={{ maxWidth:1100, margin:"0 auto" }} className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span style={{ color:"#6E90C9", fontSize:12.5, ...MONO, letterSpacing:"0.1em" }}>ADMIN · LEGACY CONTINUATION FEE</span>
          </div>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:29, color:"#E8EDF5" }}>Legacy Continuation Fee Control</h1>
          <p style={{ color:"#8A9AB8", fontSize:14.5, marginTop:4 }}>Set the fee amount and activation period. Manually activate accounts when a user passes away.</p>
        </div>
        ...
        <div className="p-6 rounded-2xl" style={CARD}>
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#E8EDF5" }}>Fee Configuration</h3>
            <span className="ml-2 px-2 py-0.5 rounded text-xs" style={{ background:"rgba(246,173,85,0.1)", color:"#F6AD55", border:"1px solid rgba(246,173,85,0.25)", ...MONO, fontSize:10 }}>ADMIN ONLY — Changes affect all future payments</span>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            <div>
              <label style={{ color:"#8A9AB8", fontSize:12.5, ...MONO, display:"block", marginBottom:6 }}>ONE-TIME FEE AMOUNT ($)</label>
              <div className="flex items-center gap-2 px-4 py-3 rounded-2xl" style={{ background:"#141B2E", border:"1px solid rgba(91,110,225,0.3)" }}>
                ...
                <input type="number" step="0.01" value={feeAmount} onChange={e => setFeeAmount(e.target.value)}
                  style={{ background:"transparent", border:"none", outline:"none", color:"#E8EDF5", fontSize:20, fontWeight:700, ...MONO, width:"100%" }}/>
              </div>
              <div style={{ color:"#8A9AB8", fontSize:12.5, marginTop:4 }}>Currently displayed as ${feeAmount} to users</div>
            </div>
            <div>
              <label style={{ color:"#8A9AB8", fontSize:12.5, ...MONO, display:"block", marginBottom:6 }}>ACTIVATION WINDOW (MONTHS)</label>
              <div className="flex items-center gap-2 px-4 py-3 rounded-2xl" style={{ background:"#141B2E", border:"1px solid rgba(91,110,225,0.3)" }}>
              ...
              <div style={{ color:"#8A9AB8", fontSize:12.5, marginTop:4 }}>How long vault stays active after death activation</div>
            </div>
            <div className="flex flex-col justify-end">
              ...
              <div style={{ color:"#8A9AB8", fontSize:12.5, marginTop:6 }}>Changes update in real-time via Supabase</div>
            </div>
          </div>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border:"1px solid rgba(91,110,225,0.1)" }}>
          <div className="flex items-center justify-between px-5 py-3 border-b" style={{ background:"rgba(255,255,255,0.08)", borderColor:"rgba(91,110,225,0.08)" }}>
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:17, color:"#E8EDF5" }}>Paid Legacy Continuation Fees</h3>
            <div className="flex items-center gap-2 px-3 py-2 rounded-2xl" style={{ background:"#141B2E", border:"1px solid rgba(91,110,225,0.3)" }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ background:"transparent", border:"none", outline:"none", color:"#E8EDF5", fontSize:13.5, width:160 }}/>
            </div>
          </div>
          ...
          {filtered.map((fee, i) => (
            <div key={fee.id} className="grid px-5 py-4 items-center border-b" style={{ gridTemplateColumns:"auto 1fr auto auto auto auto auto", background:i%2===0?"transparent":"rgba(255,255,255,0.025)", borderColor:"rgba(91,110,225,0.06)", gap:16 }}>
              ...
              <span style={{ color:"#8A9AB8", fontSize:13.5 }}>{fee.paidDate}</span>
              <span style={{ color:"#6E90C9", fontSize:14.5, fontWeight:700, ...MONO }}>${fee.amount}</span>
              <div>
                {fee.activated ? (
                  <div>
                    <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background:"rgba(91,167,214,0.12)", color:"#6FAE8B", ...MONO, fontSize:10 }}>ACTIVATED</span>
                    <div style={{ color:"#8A9AB8", fontSize:11, marginTop:2 }}>Expires: {fee.expiresDate}</div>
                  </div>
                ) : (
                  <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background:"rgba(72,187,120,0.12)", color:"#D99A6B", ...MONO, fontSize:10 }}>PAID · PENDING ACTIVATION</span>
                )}
              </div>
              <div>
                {!fee.activated ? (
                  ...
                  <div style={{ color:"#8A9AB8", fontSize:12.5 }}>Active since {fee.activatedDate}</div>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="px-5 py-4 rounded-2xl border" style={{ background:"rgba(91,110,225,0.03)", borderColor:"rgba(91,110,225,0.12)" }}>
          <div style={{ color:"#6E90C9", fontSize:12.5, ...MONO, fontWeight:700, marginBottom:6 }}>STRIPE WEBHOOK EVENTS TO HANDLE</div>
          <div className="grid md:grid-cols-2 gap-2">
            {["payment_intent.succeeded → mark fee as paid + send confirmation email","customer.subscription.deleted → check for continuation fee before suspending","invoice.payment_failed → send warning before suspension"].map(e => (
              <div key={e} style={{ color:"#8A9AB8", fontSize:13.5 }}>• {e}</div>
            ))}
          </div>
        </div>
      </div>
