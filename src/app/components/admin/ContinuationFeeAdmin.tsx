      ...
      <div style={{ maxWidth:1100, margin:"0 auto" }} className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Shield size={14} color="#5B6EE1"/>
            <span style={{ color:"#5B6EE1", fontSize:11, ...MONO, letterSpacing:"0.1em" }}>ADMIN · LEGACY CONTINUATION FEE</span>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label:"Total Revenue", value:`$${totalRevenue.toLocaleString()}`, color:"#5B6EE1" },
            { label:"Fees Collected", value:fees.length, color:"#48BB78" },
            { label:"Currently Activated", value:activated, color:"#5BA7D6" },
        ...
        <div className="p-6 rounded-2xl glow-surface" style={CARD}>
          ...
          <div className="grid md:grid-cols-3 gap-5">
            <div>
              ...
              <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background:"#F5F8FE", border:"1px solid rgba(91,110,225,0.15)" }}>
                <DollarSign size={15} color="#5B6EE1"/>
            ...
            <div className="flex flex-col justify-end">
              <button onClick={saveConfig} className="flex items-center gap-2 px-5 py-3.5 rounded-xl font-semibold text-sm"
                style={{ background: savedConfig ? "rgba(72,187,120,0.12)" : "linear-gradient(135deg,#5B6EE1,#5B6EE1)", color: savedConfig ? "#48BB78" : "#fff", border: savedConfig ? "1px solid rgba(72,187,120,0.3)" : "none", boxShadow: savedConfig ? "none" : "0 4px 12px rgba(91,110,225,0.3)" }}>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border:"1px solid rgba(91,110,225,0.1)" }}>
          <div className="flex items-center justify-between px-5 py-3 border-b" style={{ background:"#EAF0FC", borderColor:"rgba(91,110,225,0.08)" }}>
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:15, color:"#0D1428" }}>Paid Legacy Continuation Fees</h3>
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background:"#FFFFFF", border:"1px solid rgba(91,110,225,0.1)" }}>
          ...
          {filtered.map((fee, i) => (
            <div key={fee.id} className="grid px-5 py-4 items-center border-b" style={{ gridTemplateColumns:"auto 1fr auto auto auto auto auto", background:i%2===0?"#FFFFFF":"#F8FAFF", borderColor:"rgba(91,110,225,0.06)", gap:16 }}>
              ...
              <div>
                {fee.activated ? (
                  <div>
                    <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background:"rgba(91,167,214,0.12)", color:"#5BA7D6", ...MONO, fontSize:9 }}>ACTIVATED</span>
        ...
        <div className="px-5 py-4 rounded-2xl border" style={{ background:"rgba(91,110,225,0.03)", borderColor:"rgba(91,110,225,0.12)" }}>
          <div style={{ color:"#5B6EE1", fontSize:11, ...MONO, fontWeight:700, marginBottom:6 }}>STRIPE WEBHOOK EVENTS TO HANDLE</div>
