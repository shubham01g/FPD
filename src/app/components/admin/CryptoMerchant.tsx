...
function ProcessorCard({ proc, onUpdate }: { proc: Processor; onUpdate: (id: string, changes: Partial<Processor>) => void }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden glow-surface" style={CARD}>
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t space-y-4" style={{ borderColor:"rgba(58,91,217,0.08)" }}>
          <div className="pt-4">
    ...
    <div className="p-6 space-y-6" style={{ maxWidth:1100 }}>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label:"Total Crypto TXNs",     value:MOCK_TXS.filter(t=>t.status==="confirmed").length, color:"#3A5BD9" },
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-1 p-1 rounded-xl w-fit" style={{ background:"rgba(255,255,255,0.9)", border:"1px solid rgba(58,91,217,0.1)" }}>
        {TABS.map(t => (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl overflow-auto" style={{ border:"1px solid rgba(58,91,217,0.1)" }}>
            <table className="w-full text-sm">
              ...
              <tbody>
                {MOCK_TXS.map((tx, i) => (
                  <tr key={tx.id} style={{ background:i%2===0?"#fff":"#F8FAFF", borderBottom:"1px solid rgba(58,91,217,0.06)" }}>
                    <td className="px-4 py-3 whitespace-nowrap" style={{ color:"#3A5BD9", fontSize:10, ...MONO }}>{tx.id}</td>
          ...
          {[
            ...
            <div key={w.coin} className="p-5 rounded-2xl glow-surface" style={CARD}>
              ...
              <div className="flex items-center gap-2 mt-4 px-3 py-2.5 rounded-xl"
                style={{ background:"rgba(58,91,217,0.05)", border:"1px solid rgba(58,91,217,0.1)" }}>
                ...
                <button onClick={() => { copyToClipboard(w.address); toast.success(`${w.coin} address copied`); }}
                  style={{ color:"#3A5BD9", flexShrink:0 }}><Copy size={13}/></button>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-5 max-w-lg">
          <div className="p-5 rounded-2xl space-y-4 glow-surface" style={CARD}>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <ToggleRight size={28} color="#3A5BD9"/>
            </div>
          ...
          <div className="p-5 rounded-2xl glow-surface" style={{ ...CARD, border:"1px solid rgba(247,147,26,0.3)", background:"rgba(247,147,26,0.03)" }}>
            ...
            {[
              ...
              <div key={v.name} className="flex items-center justify-between py-2 border-b" style={{ borderColor:"rgba(58,91,217,0.06)" }}>
