...
const frequencyColor: Record<Frequency,string> = { Weekly:"#48BB78", Biweekly:"#38B2AC", Monthly:"#2040C0", Quarterly:"#9F7AEA", Yearly:"#F6AD55" };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.45)", backdropFilter:"blur(8px)" }}>
      <div className="w-full max-w-2xl rounded-2xl overflow-hidden" style={CARD}>
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor:"rgba(32,64,192,0.08)" }}>
          <div className="flex items-center gap-2"><CreditCard size={18} color="#2040C0"/><h3 style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#0D1428" }}>Add Subscription / Auto Pay</h3></div>
        ...
        <div className="p-6 overflow-y-auto space-y-4" style={{ maxHeight:"80vh" }}>
          ...
          {/* Payment */}
          <div className="p-4 rounded-xl" style={{ background:"#F5F8FE" }}>
            <div style={{ color:"#8A9AB8", fontSize:10, ...MONO, marginBottom:10 }}>PAYMENT METHOD</div>
            <div className="grid grid-cols-2 gap-4">
          ...
          {/* Login details */}
          <div className="p-4 rounded-xl space-y-3" style={{ background:"rgba(32,64,192,0.03)", border:"1px solid rgba(32,64,192,0.1)" }}>
            <div style={{ color:"#2040C0", fontSize:10, ...MONO, fontWeight:700 }}>ACCOUNT LOGIN DETAILS (for legacy contacts)</div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPw?"text":"password"} value={form.password??""} onChange={e => setForm(p=>({...p,password:e.target.value}))} placeholder="Account password"
                    className="w-full px-4 py-2.5 rounded-xl pr-9" style={{ background:"#fff", border:"1px solid rgba(32,64,192,0.12)", color:"#0D1428", fontSize:13, outline:"none", ...MONO }}/>
          ...
          <div>
            <label style={{ color:"#8A9AB8", fontSize:10, ...MONO, display:"block", marginBottom:4 }}>CANCELLATION INSTRUCTIONS</label>
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.autoPay??true} onChange={e => setForm(p=>({...p,autoPay:e.target.checked}))} style={{ accentColor:"#2040C0", width:15, height:15 }}/>
              <span style={{ color:"#374669", fontSize:13 }}>Auto-Pay Enabled</span>
            </label>
          </div>
          {/* Upload */}
          <div>
            <label style={{ color:"#8A9AB8", fontSize:10, ...MONO, display:"block", marginBottom:4 }}>ATTACH DOCUMENTS</label>
            ...
            <div className="flex items-center gap-2">
              <div onClick={() => fileRef.current?.click()} className="flex-1 flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed cursor-pointer" style={{ borderColor:"rgba(32,64,192,0.2)", background:"rgba(32,64,192,0.02)" }}>
                <Upload size={14} color="#2040C0" style={{ opacity:0.6 }}/>
                <span style={{ color:"#8A9AB8", fontSize:13 }}>Attach bill, statement, or agreement</span>
              </div>
    ...
    <div style={{ background:"#F0F4FA", minHeight:"100%", padding:24 }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }} className="space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 style={{ fontFamily:"var(--font-display)", fontSize:24, color:"#0D1428" }}>Auto Pay & Subscriptions</h1>
            <p style={{ color:"#5A6A88", fontSize:13, marginTop:4 }}>{subs.filter(s=>s.status==="active").length} active subscriptions · ~${totalMonthly.toFixed(2)}/month total</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label:"Monthly Cost", value:`$${totalMonthly.toFixed(2)}`, color:"#2040C0" },
        ...
        <div className="flex items-center gap-3 px-5 py-3 rounded-2xl border" style={{ background:"rgba(246,173,85,0.05)", borderColor:"rgba(246,173,85,0.25)" }}>
          ...
          <span style={{ color:"#374669", fontSize:13 }}>These subscriptions and accounts need to be cancelled or transferred when a user passes. Ensure legacy contacts have access to this page.</span>
        </div>
          ...
          <div className="lg:col-span-2 space-y-2">
            {filtered.map(sub => (
              <div key={sub.id} className="flex items-center gap-4 px-4 py-4 rounded-2xl cursor-pointer transition-all"
                style={{ ...CARD, borderColor:selected?.id===sub.id?"#2040C0":"rgba(32,64,192,0.1)", borderWidth:selected?.id===sub.id?2:1 }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span style={{ color:"#0D1428", fontSize:14, fontWeight:600 }}>{sub.title}</span>
                ...
                <div className="text-right flex-shrink-0">
                  <div style={{ fontFamily:"var(--font-display)", fontSize:16, color:"#0D1428" }}>${sub.amount.toFixed(2)}</div>
            ...
            {selected ? (
              <div className="rounded-2xl p-5 sticky top-4 space-y-4" style={CARD}>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setSelected(null)} style={{ color:"#8A9AB8" }}><X size={13}/></button>
                </div>
                <div className="flex items-center gap-2">
                  <span style={{ fontFamily:"var(--font-display)", fontSize:24, color:"#0D1428" }}>${selected.amount.toFixed(2)}</span>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={row.label} className="flex flex-col px-3 py-2 rounded-xl" style={{ background:"#F5F8FE" }}>
                      <span style={{ color:"#8A9AB8", fontSize:9, ...MONO }}>{row.label.toUpperCase()}</span>
                      <span style={{ color:"#0D1428", fontSize:12 }}>{row.value}</span>
                    </div>
                  ))}
                  {selected.password && (
                    <div className="flex items-center justify-between px-3 py-2 rounded-xl" style={{ background:"#F5F8FE" }}>
                      <div>
                        <div style={{ color:"#8A9AB8", fontSize:9, ...MONO }}>PASSWORD</div>
                        <div style={{ color:"#0D1428", fontSize:12, ...MONO }}>{showPwFor===selected.id ? selected.password : "••••••••"}</div>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => setShowPwFor(showPwFor===selected.id ? null : selected.id)} style={{ color:"#8A9AB8" }}>{showPwFor===selected.id ? <EyeOff size={12}/> : <Eye size={12}/>}</button>
                      </div>
                    </div>
                  )}
                </div>
                {selected.cancelInstructions && (
                  <div className="px-3 py-3 rounded-xl" style={{ background:"rgba(252,129,129,0.06)", border:"1px solid rgba(252,129,129,0.2)" }}>
                    ...
                    <div style={{ color:"#374669", fontSize:12, lineHeight:1.7 }}>{selected.cancelInstructions}</div>
                  </div>
                )}
                {selected.notes && (
                  <div className="px-3 py-3 rounded-xl" style={{ background:"#F5F8FE" }}>
                    <div style={{ color:"#8A9AB8", fontSize:9, ...MONO, marginBottom:4 }}>NOTES</div>
                    <div style={{ color:"#374669", fontSize:12 }}>{selected.notes}</div>
                  </div>
                )}
              </div>
            ) : (
              <div className="rounded-2xl p-8 text-center" style={CARD}>
                <CreditCard size={32} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/>
                <div style={{ color:"#8A9AB8", fontSize:13 }}>Select a subscription to view details</div>
              </div>
            )}
          </div>
        </div>
