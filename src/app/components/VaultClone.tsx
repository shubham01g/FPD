...
const GLASS: React.CSSProperties = {
  ...
  border: "1px solid rgba(32,64,192,0.2)",
...
const ALL_CONTENTS = [
  { icon:<FileText size={13}/>,    color:"#2040C0", label:"All Documents",         count:"14 files", desc:"All 18 folder categories" },
  ...
  { icon:<Camera size={13}/>,      color:"#3355E0", label:"Memories & Media",      count:"24 items",   desc:"Photos, videos, written memories" },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-xl rounded-2xl relative overflow-hidden" style={GLASS}>
        <button onClick={onClose} className="absolute top-5 right-5 z-10" style={{ color:"#5A6A88" }}><X size={16}/></button>

        {/* Header */}
        <div className="px-7 pt-7 pb-5 border-b" style={{ borderColor:"rgba(32,64,192,0.15)" }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background:"rgba(32,64,192,0.15)", color:"#6090FF" }}>
              <Copy size={18}/>
            </div>
            <div>
              <h2 style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#E8EDF5" }}>Legacy Vault Clone</h2>
              <p style={{ color:"#4A5A7A", fontSize:12, marginTop:2 }}>
        ...
        <div className="px-7 py-6 overflow-y-auto" style={{ maxHeight:"75vh" }}>
          ...
          {!isUnlocked && step === "overview" && (
            <div className="space-y-5">
                ...
                <div className="flex flex-col gap-2 mt-4 text-left">
                  {[
                    ...
                    <div key={c.label} className="flex items-center gap-2 px-3 py-2 rounded-lg"
                      ...
                      <span style={{ color:"#8A9AB8", fontSize:12 }}>{c.label}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="p-5 rounded-2xl space-y-3" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.15)" }}>
                <div style={{ color:"#6090FF", fontSize:12, fontWeight:700, ...MONO }}>WHAT GETS UNLOCKED</div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {ALL_CONTENTS.slice(0,8).map(item => (
                    <div key={item.label} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg"
                      ...
                      <span style={{ color:"#6B7FA8", fontSize:11 }}>{item.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <button onClick={onClose} className="w-full py-3 rounded-xl text-sm font-semibold"
                style={{ background:"rgba(32,64,192,0.1)", color:"#6090FF", border:"1px solid rgba(32,64,192,0.2)" }}>
                Close
              </button>
            </div>
          )}
            ...
            <div className="space-y-5">
              <div className="p-4 rounded-2xl flex items-start gap-3"
                ...
                <div>
                  ...
                  <div style={{ color:"#6B7FA8", fontSize:12, lineHeight:1.6 }}>
              ...
              <div>
                <div style={{ color:"#4A5A7A", fontSize:11, ...MONO, marginBottom:10 }}>COMPLETE ACCOUNT CONTENTS</div>
                  ...
                  {ALL_CONTENTS.filter(c => c.label !== "Secret Vault" || includeSecretVault).map(item => (
                    <div key={item.label} className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
                      ...
                      <div className="flex-1 min-w-0">
                        <span style={{ color:"#B8C8E0", fontSize:12, fontWeight:500 }}>{item.label}</span>
                        <span style={{ color:"#4A5A7A", fontSize:11, marginLeft:6 }}>{item.desc}</span>
                      </div>
              ...
              <button onClick={() => setStep("confirm")}
                ...
                style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", boxShadow:"0 0 28px rgba(32,64,192,0.45)" }}>
          ...
          {isUnlocked && step === "confirm" && (
            <div className="space-y-5">
              ...
              <div className="p-4 rounded-2xl" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.15)" }}>
                <div style={{ color:"#4A5A7A", fontSize:11, ...MONO, marginBottom:8 }}>DOWNLOAD SUMMARY</div>
                {[
                  ...
                  <div key={k} className="flex justify-between py-1.5 border-b"
                    ...
                    <span style={{ color:"#4A5A7A", fontSize:12 }}>{k}</span>
                    <span style={{ color:"#B8C8E0", fontSize:12 }}>{v}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep("overview")}
                  ...
                  style={{ background:"rgba(32,64,192,0.06)", color:"#6090FF", border:"1px solid rgba(32,64,192,0.2)" }}>
                  ← Back
                </button>
                <button onClick={startDownload}
                  className="flex-1 py-3 rounded-xl font-bold text-sm"
                  style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", boxShadow:"0 0 20px rgba(32,64,192,0.4)" }}>
                  Confirm & Download
                </button>
              </div>
            </div>
          ...
          {step === "downloading" && (
            <div className="text-center py-8 space-y-5">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto"
                style={{ background:"rgba(32,64,192,0.15)", border:"1px solid rgba(32,64,192,0.3)" }}>
                <Download size={28} color="#2040C0" style={{ animation:"bounce 1s infinite" }}/>
              </div>
              <div>
                <div style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#E8EDF5", marginBottom:6 }}>
                  Preparing Your Legacy Package
                </div>
                <div style={{ color:"#4A5A7A", fontSize:13 }}>
                  Encrypting and packaging all account data...
                </div>
              </div>
              <div className="h-2 rounded-full mx-auto" style={{ background:"rgba(32,64,192,0.15)", maxWidth:320 }}>
                <div className="h-2 rounded-full transition-all duration-300"
                  style={{ width:`${progress}%`, background:"linear-gradient(90deg,#2040C0,#3355E0)", boxShadow:"0 0 12px rgba(32,64,192,0.5)" }}/>
              </div>
              <div style={{ color:"#2040C0", fontSize:14, ...MONO }}>{Math.round(progress)}%</div>
          ...
          {step === "done" && (
            <div className="text-center py-4 space-y-5">
              ...
              <div>
                ...
                <p style={{ color:"#6B7FA8", fontSize:13, lineHeight:1.7 }}>
              ...
              <div className="p-4 rounded-2xl text-left space-y-2"
                ...
                <p style={{ color:"#8AA3C8", fontSize:12, lineHeight:1.7 }}>
              ...
              <div className="flex gap-3">
                <button onClick={() => toast.success("Legacy package download started (demo)")}
                  ...
                  style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", boxShadow:"0 0 20px rgba(32,64,192,0.35)" }}>
                  <Download size={14}/> Save to Device
                </button>
                <button onClick={onClose} className="flex-1 py-3.5 rounded-xl text-sm"
                  style={{ background:"rgba(32,64,192,0.08)", color:"#6090FF", border:"1px solid rgba(32,64,192,0.2)" }}>
                  Close
                </button>
              </div>
            </div>
