...
const PLATFORM_SECTIONS = [
  {
    section:"file_cabinet", label:"Digital File Cabinet", emoji:"📁", color:"#2040C0",
  ...
  {
    section:"memories", label:"Memories & Media", emoji:"📷", color:"#3355E0",
      ...
      <div className="w-full max-w-2xl rounded-2xl overflow-hidden" style={{ ...GLASS, maxHeight:"90vh" }}>
        ...
        <div className="p-6 space-y-5 overflow-y-auto" style={{ maxHeight:"calc(90vh - 70px)" }}>
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
            ...
            {doc.fileType === "image" ? <Image size={18} color="#9F7AEA"/> : <FileText size={18} color="#2040C0"/>}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {PLATFORM_SECTIONS.map(s => (
                <button key={s.section} onClick={() => { setSelectedSection(s.section); setSelectedSub(s.subOptions[0]?.id ?? ""); }}
                  ...
                  style={{ background:selectedSection===s.section?`${s.color}12`:"rgba(32,64,192,0.03)",
                    border:`1.5px solid ${selectedSection===s.section?s.color:"rgba(32,64,192,0.1)"}` }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {activeSection.subOptions.map(sub => (
                  <button key={sub.id} onClick={() => setSelectedSub(sub.id)}
                    ...
                    style={{ background:selectedSub===sub.id?`${activeSection.color}12`:"rgba(32,64,192,0.03)",
                      border:`1.5px solid ${selectedSub===sub.id?activeSection.color:"rgba(32,64,192,0.1)"}` }}>
          ...
          <div className="flex gap-3">
            <button onClick={doUpload} disabled={uploading || !destination}
              ...
              style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA",
                boxShadow:"0 0 20px rgba(32,64,192,0.3)", opacity:uploading||!destination?0.5:1 }}>
              {uploading
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={GLASS}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 px-3 py-3 rounded-xl"
            style={{ background:"rgba(32,64,192,0.05)", border:"1px solid rgba(32,64,192,0.15)" }}>
            <ExternalLink size={12} color="#8A9AB8" style={{ flexShrink:0 }}/>
            <span style={{ color:"#2040C0", fontSize:11, ...MONO, flex:1 }} className="truncate">{link}</span>
            <button onClick={() => { copyToClipboard(link); toast.success("Link copied!"); }}
              style={{ color:"#2040C0", flexShrink:0 }}>
              <Copy size={14}/>
            </button>
          </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 p-1 rounded-xl w-fit" style={{ background:"rgba(255,255,255,0.9)", border:"1px solid rgba(32,64,192,0.1)" }}>
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id as any)}
            ...
            style={{ background:filter===f.id?"#2040C0":"transparent", color:filter===f.id?"#fff":"#5A6A88" }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-12 text-center rounded-2xl" style={GLASS}>
          <Inbox size={36} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/>
        ...
        <div className="space-y-3">
          {filtered.map(doc => {
            ...
            const statusColors = {
              ...
              received:           { color:"#2040C0", bg:"rgba(32,64,192,0.1)",    label:"Received" },
              ...
              <div key={doc.id} className="p-5 rounded-2xl" style={GLASS}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex items-center justify-center rounded-xl flex-shrink-0"
                    style={{ width:56, height:56, background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.1)", overflow:"hidden" }}>
                    {doc.previewUrl && doc.fileType === "image"
                      ? <img src={doc.previewUrl} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                      : <FileText size={24} color="#2040C0"/>
                    }
                  </div>
