      ...
      <div className="w-full max-w-2xl rounded-2xl overflow-hidden" style={{ ...GLASS, maxHeight:"90vh" }}>
        <div className="flex items-center justify-between px-6 py-4 border-b sticky top-0 z-10"
          ...
          <div className="flex items-center gap-2">
            ...
            <span style={{ fontFamily:"var(--font-display)", fontSize:20, color:"#E8EDF5" }}>
        ...
        <div className="p-6 space-y-5 overflow-y-auto" style={{ maxHeight:"calc(90vh - 70px)" }}>
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-2xl"
            ...
            <div className="flex-1">
              <div style={{ color:"#E8EDF5", fontSize:16, fontWeight:500 }}>{doc.category}</div>
              <div style={{ color:"#8A9AB8", fontSize:14 }}>{doc.fileName} · {doc.fileSizeMB} MB</div>
            </div>
          </div>

          {doc.notes && (
            <div className="px-3 py-2 rounded-2xl" style={{ background:"rgba(246,173,85,0.06)", border:"1px solid rgba(246,173,85,0.2)" }}>
              <div style={{ color:"#F6AD55", fontSize:12.5, ...MONO, marginBottom:2 }}>CLIENT NOTE</div>
              <div style={{ color:"#8A9AB8", fontSize:15 }}>"{doc.notes}"</div>
            </div>
          )}

          {/* Step 1: Choose platform section */}
          <div>
            <div style={{ color:"#8A9AB8", fontSize:14, ...MONO, marginBottom:10 }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {PLATFORM_SECTIONS.map(s => (
                <button key={s.section} onClick={() => { setSelectedSection(s.section); setSelectedSub(s.subOptions[0]?.id ?? ""); }}
                  ...
                  <div style={{ fontSize:25 }}>{s.emoji}</div>
                  <div style={{ color:selectedSection===s.section?s.color:"#E8EDF5", fontSize:14, fontWeight:600, lineHeight:1.3 }}>{s.label}</div>
                  <div style={{ color:"#8A9AB8", fontSize:11, lineHeight:1.3 }}>{s.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {activeSection && activeSection.subOptions.length > 1 && (
            <div>
              <div style={{ color:"#8A9AB8", fontSize:14, ...MONO, marginBottom:8 }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {activeSection.subOptions.map(sub => (
                  <button key={sub.id} onClick={() => setSelectedSub(sub.id)}
                    ...
                    <span style={{ color:selectedSub===sub.id?activeSection.color:"#8A9AB8", fontSize:14, fontWeight:selectedSub===sub.id?700:400 }}>
                      {sub.label}
                    </span>
          ...
          {destination && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-2xl"
              ...
              <div>
                <span style={{ color:"#8A9AB8", fontSize:15 }}>Will be saved to: </span>
                <strong style={{ color:"#E8EDF5", fontSize:15 }}>{destination}</strong>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3">
            <button onClick={doUpload} disabled={uploading || !destination}
              ...
              style={{ background:"linear-gradient(135deg,#5B6EE1,#5B6EE1)", color:"#F0F4FA",
                boxShadow:"0 0 20px rgba(91,110,225,0.3)", opacity:uploading||!destination?0.5:1 }}>
              {uploading
            ...
            <button onClick={onClose} className="px-4 py-3 rounded-2xl text-sm"
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={GLASS}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 px-3 py-3 rounded-2xl"
            ...
            <span style={{ color:"#6E90C9", fontSize:14, ...MONO, flex:1 }} className="truncate">{link}</span>
          ...
          <div className="space-y-2">
            <div style={{ color:"#8A9AB8", fontSize:14, ...MONO, marginBottom:4 }}>SEND TO CLIENT VIA</div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div style={{ color:"#6FAE8B", fontSize:14, ...MONO, letterSpacing:"0.1em", marginBottom:4 }}>
            DOCUMENT INBOX
          </div>
          <h2 style={{ fontFamily:"var(--font-display)", fontSize:25, color:"#E8EDF5" }}>
      ...
      <div className="p-4 rounded-2xl space-y-3" style={{ background:"rgba(91,167,214,0.04)", border:"1px solid rgba(91,167,214,0.15)" }}>
        <div style={{ color:"#6FAE8B", fontSize:14, fontWeight:700, ...MONO }}>SEND UPLOAD LINK TO CLIENT</div>
      ...
      <div className="flex gap-1 p-1 rounded-2xl w-fit" style={{ background:"#0A0F1A", border:"1px solid rgba(91,167,214,0.3)" }}>
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id as any)}
            ...
            style={{ background:filter===f.id?"#5B6EE1":"transparent", color:filter===f.id?"#fff":"#8A9AB8" }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-12 text-center rounded-2xl" style={GLASS}>
          ...
          <div style={{ color:"#8A9AB8", fontSize:17.5 }}>
            {filter === "all" ? "No documents submitted yet." : `No ${filter} documents.`}
          </div>
          <div style={{ color:"#B0C0DC", fontSize:15, marginTop:4 }}>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between flex-wrap gap-2">
                      <div>
                        <div style={{ color:"#E8EDF5", fontSize:17.5, fontWeight:600 }}>{doc.category}</div>
                    ...
                    {doc.notes && (
                      <div className="mt-2 px-3 py-2 rounded-2xl"
                        ...
                        <span style={{ color:"#F6AD55", fontSize:12.5, ...MONO }}>CLIENT NOTE: </span>
                        <span style={{ color:"#8A9AB8", fontSize:15 }}>"{doc.notes}"</span>
                      </div>
                    )}
                    ...
                    {doc.status === "uploaded_to_vault" && (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span style={{ color:"#D99A6B", fontSize:15 }}>
                          Saved to account
                        </span>
                      </div>
                    )}
