...
.fpd-dr .hbanner h1 .accent{background:linear-gradient(90deg,#EAC793,${WARN});-webkit-background-clip:text;background-clip:text;color:transparent;}
...
.fpd-dr .btn-primary{display:inline-flex;align-items:center;gap:8px;padding:13px 22px;border-radius:99px;background:linear-gradient(135deg,${WARN},#B4842E);color:#1C1608;font-size:15px;font-weight:700;box-shadow:0 8px 20px -8px rgba(217,165,94,0.5);transition:filter .18s,transform .18s;border:none;cursor:pointer;font-family:var(--font-body);}
...
.fpd-dr .btn-ghost{display:inline-flex;align-items:center;gap:7px;padding:10px 16px;border-radius:99px;background:rgba(217,165,94,0.1);border:1px solid rgba(217,165,94,0.28);color:${WARN};font-size:14px;font-weight:700;cursor:pointer;font-family:var(--font-body);transition:background .18s;}
.fpd-dr .btn-ghost:hover{background:rgba(217,165,94,0.18);}
...
.fpd-dr .pricecard.hi{background:rgba(217,165,94,0.08);border-color:rgba(217,165,94,0.35);}
...
.fpd-dr .catrow.on{background:rgba(217,165,94,0.08);border-color:rgba(217,165,94,0.3);}
...
.fpd-dr .modal-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:20px 22px;background:linear-gradient(135deg,${WARN},#B4842E);}
.fpd-dr .modal-head-title{color:#1C1608;font-weight:700;font-size:15px;}
.fpd-dr .modal-head-sub{color:rgba(28,22,8,0.75);font-size:12px;}
...
.fpd-dr .field input{width:100%;padding:11px 13px;border-radius:10px;background:#0F1624;border:1.5px solid rgba(217,165,94,0.3);color:${TEXT};font-size:14px;outline:none;font-family:var(--font-body);}
.fpd-dr .field input:focus{border-color:rgba(217,165,94,0.6);}
        ...
        <div className="modal-body">
            ...
            <div className="space-y-3">
              <div className="field">
                ...
                <div style={{ position:"relative" }}>
                  ...
                  <CreditCard size={14} color={WARN} style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)" }}/>
                </div>
              </div>
              <div className="field">
          ...
          {step === "done" && (
            <div className="flex flex-col items-center gap-5 py-2">
              ...
              <div style={{ width:"100%", background:"rgba(217,165,94,0.07)",
                border:"1px dashed rgba(217,165,94,0.4)", borderRadius:12, padding:"16px" }}>
                <div style={{ fontSize:11, ...MONO, color:WARN, fontWeight:700, marginBottom:6 }}>
        ...
        <div className="card pad flex flex-col items-center text-center gap-5">
          ...
          <div className="pricegrid">
            {[
              ...
              <div key={opt.label} className={`pricecard ${opt.highlight ? "hi" : ""}`}>
                ...
                <div style={{ fontSize:26, fontWeight:800, color: opt.highlight ? WARN : TEXT }}>
                  {opt.price}
                </div>
          ...
          {!addonActive ? (
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 20px",
              borderRadius:10, background:"rgba(217,165,94,0.1)", border:"1px solid rgba(217,165,94,0.3)" }}>
              <CheckCircle size={14} color={WARN}/>
              <span style={{ fontSize:13, fontWeight:600, color:WARN }}>
      ...
      <div className="wrap">
        ...
        {notifBanner && (
          <div style={{
            ...
            borderRadius: 14, padding:"14px 20px", color:"#1C1608",
            ...
            <button onClick={() => setNotifBanner(false)} style={{ background:"none", border:"none", cursor:"pointer" }}>
              <X size={16} color="#1C1608" style={{ opacity:0.7 }} />
            </button>
          </div>
        )}

        <div>
          <div className="eyebrow" style={{ color:WARN }}>
            <ShieldAlert size={12} color={WARN} /> Disaster Recovery Protection · Bypass Active
          </div>
        ...
        <div className="countdown" style={{
          background: urgentRed
            ? "linear-gradient(135deg,#D06B6B,#8C3E3E)"
            : "linear-gradient(135deg,#D9A55E,#8C6423)",
        }}>
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div style={{ fontSize:11, opacity:0.85, ...MONO, letterSpacing:"0.1em", marginBottom:4 }}>
            ...
            <div style={{ textAlign:"right" }}>
              <div style={{ fontSize:12, opacity:0.8, ...MONO }}>Activated by</div>
              <div style={{ fontSize:14, fontWeight:700 }}>{session!.activatedBy}</div>
              <div style={{ fontSize:12, opacity:0.8, marginTop:2 }}>Reason: {session!.reason}</div>
            </div>
          </div>
          <div style={{ height:4, borderRadius:2, background:"rgba(255,255,255,0.2)", marginTop:16, overflow:"hidden" }}>
            <div style={{ height:"100%", borderRadius:2, background:"rgba(255,255,255,0.75)",
        ...
        {exportPhase === "idle" && (
          <>
            <div className="card pad">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setCategories(cs => cs.map(c => ({ ...c, selected:true })))} className="btn-sec" style={{ color:WARN, borderColor:"rgba(217,165,94,0.3)" }}>All</button>
                ...
                {categories.map(cat => (
                  <label key={cat.id} className={`catrow ${cat.selected ? "on" : ""}`}>
                    <input type="checkbox" checked={cat.selected} onChange={() => toggleCat(cat.id)}
                      style={{ width:16, height:16, accentColor:WARN }} />
                    ...
                    <div style={{ fontSize:12, color:cat.selected ? WARN : FAINT, fontWeight:600, ...MONO }}>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"12px 16px",
              background:"rgba(217,165,94,0.06)", border:"1px solid rgba(217,165,94,0.25)", borderRadius:10 }}>
              <Info size={14} color={WARN} style={{ marginTop:1, flexShrink:0 }} />
        ...
        {exportPhase === "generating" && (
          <div className="card pad flex flex-col items-center gap-6" style={{ padding:40 }}>
            <div style={{ width:80, height:80, borderRadius:"50%", background:"rgba(217,165,94,0.12)",
              border:"2px solid rgba(217,165,94,0.3)", display:"flex", alignItems:"center", justifyContent:"center" }}>
              <FileArchive size={36} color={WARN} style={{ animation:"fpd-dr-spin 2s linear infinite" }} />
            </div>
            ...
            <div style={{ width:"100%", maxWidth:440 }}>
              ...
              <div style={{ height:10, borderRadius:5, background:"rgba(217,165,94,0.15)", overflow:"hidden" }}>
                <div style={{ height:"100%", borderRadius:5, background:"linear-gradient(90deg,#D9A55E,#F0C27A)",
                  width:`${progress}%`, transition:"width 0.6s ease", boxShadow:"0 0 8px rgba(217,165,94,0.5)" }} />
              </div>
            </div>
          ...
          <div className="card pad flex flex-col items-center gap-6" style={{ padding:32 }}>
            ...
            {linkExpiry && (
              <div style={{
                ...
                background: linkCountdown.remaining < 120_000 ? "rgba(208,107,107,0.12)" : "rgba(217,165,94,0.12)",
                border: `1px solid ${linkCountdown.remaining < 120_000 ? "rgba(208,107,107,0.3)" : "rgba(217,165,94,0.3)"}`,
              }}>
                <Clock size={14} color={linkCountdown.remaining < 120_000 ? NEG : WARN} />
                <span style={{ fontSize:13, fontWeight:700, ...MONO, color: linkCountdown.remaining < 120_000 ? NEG : WARN }}>
            ...
            <div style={{ width:"100%", maxWidth:500 }}>
              <div style={{ display:"flex", alignItems:"center", border:"1.5px solid rgba(217,165,94,0.3)", borderRadius:10, overflow:"hidden" }}>
                ...
                <button onClick={copyLink} style={{ padding:"10px 14px", background:"rgba(217,165,94,0.12)",
                  border:"none", borderLeft:"1px solid rgba(217,165,94,0.3)", cursor:"pointer", color:WARN,
        ...
        <div className="card pad">
          ...
          <div style={{ height:8, borderRadius:4, background:"rgba(217,165,94,0.15)", overflow:"hidden" }}>
            <div style={{ height:"100%", borderRadius:4, background:"linear-gradient(90deg,#D9A55E,#F0C27A)", width:"20.9%" }} />
          </div>
          <div className="flex items-center gap-4 mt-3 flex-wrap">
            {[
              { label:"Media", pct:"73.8%", color:WARN },
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px",
          background:"rgba(217,165,94,0.06)", border:"1px solid rgba(217,165,94,0.2)", borderRadius:8, fontSize:12, color:SOFT }}>
          <Zap size={13} color={WARN} />
          <span>
