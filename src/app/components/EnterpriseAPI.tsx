...
const methodColor = { GET:"#48BB78", POST:"#2040C0", PUT:"#F6AD55", DELETE:"#FC8181", PATCH:"#9F7AEA" };
...
export function EnterpriseAPI() {
  ...
  const mc = (m: string) => (methodColor as any)[m] ?? "#5A6A88";
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Code size={15} color="#2040C0" />
            <span style={{ color: "#2040C0", fontSize: 11, ...MONO, letterSpacing: "0.12em" }}>ENTERPRISE</span>
          </div>
        ...
        <div className="flex items-center gap-2 px-4 py-2 rounded-xl" style={{ ...GLASS }}>
          ...
          <span style={{ color: "#48BB78", fontSize: 12, ...MONO }}>API STATUS: OPERATIONAL</span>
        </div>
      </div>
      ...
      <div className="flex gap-1 p-1 rounded-2xl" style={{ background: "rgba(255,255,255,0.95)", width: "fit-content" }}>
        {([["endpoints","Endpoints",<Book size={13}/>],["sdk","SDK Examples",<Terminal size={13}/>],["webhooks","Webhooks",<Webhook size={13}/>],["keys","API Keys",<Key size={13}/>]] as const).map(([id,label,icon]) => (
          <button key={id} onClick={() => setActiveTab(id)}
            ...
            style={{ background: activeTab===id ? "#2040C0" : "transparent", color: activeTab===id ? "#F0F4FA" : "#5A6A88", fontWeight: activeTab===id ? 700 : 400 }}>
            {icon} {label}
          </button>
        ))}
      </div>
      ...
      {activeTab === "endpoints" && (
        <div className="grid" style={{ gridTemplateColumns: "260px 1fr", gap: 20, minHeight: 600 }}>
          ...
          <div className="rounded-2xl overflow-hidden" style={GLASS}>
            <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(32,64,192,0.1)" }}>
              <div style={{ color: "#5A6A88", fontSize: 10, ...MONO, letterSpacing: "0.1em" }}>BASE URL</div>
              <div style={{ color: "#2040C0", fontSize: 11, ...MONO, marginTop: 2 }}>https://api.finalpassdown.com</div>
            </div>
            <div className="overflow-y-auto" style={{ maxHeight: 700 }}>
              {apiGroups.map((group, gi) => (
                <div key={gi}>
                  <button className="w-full flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "rgba(32,64,192,0.06)", background: expandedGroup===gi ? "rgba(32,64,192,0.06)" : "transparent" }}
                    ...
                    <div className="flex items-center gap-2">
                      <span style={{ color: "#8A9AB8", fontSize: 10, ...MONO }}>{group.endpoints.length}</span>
                      {expandedGroup===gi ? <ChevronDown size={12} color="#5A6A88" /> : <ChevronRight size={12} color="#5A6A88" />}
                    </div>
                  </button>
                  {expandedGroup===gi && group.endpoints.map((ep, ei) => (
                    <button key={ei} className="w-full flex items-start gap-2 px-4 py-2.5 border-b text-left"
                      style={{ borderColor: "rgba(32,64,192,0.04)", background: activeEndpoint===ep ? "rgba(32,64,192,0.1)" : "transparent" }}
          ...
          <div className="rounded-2xl p-6" style={GLASS}>
            {!activeEndpoint ? (
              <div className="h-full flex flex-col items-center justify-center text-center">
                <Code size={40} color="rgba(32,64,192,0.2)" style={{ marginBottom: 16 }} />
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-3">
                  ...
                  <code style={{ color: "#0D1428", fontSize: 15, ...MONO }}>{activeEndpoint.path}</code>
                </div>
                ...
                {activeEndpoint.body && (
                  <div>
                    ...
                    <div className="p-4 rounded-xl overflow-x-auto" style={{ background: "#EAF0FC", border: "1px solid rgba(32,64,192,0.15)" }}>
                      <pre style={{ color: "#3355E0", fontSize: 12, ...MONO, margin: 0, whiteSpace: "pre-wrap" }}>{activeEndpoint.body}</pre>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => runTest(activeEndpoint)} disabled={testing}
                    ...
                    style={{ background: testing ? "rgba(32,64,192,0.1)" : "linear-gradient(135deg,#2040C0,#3355E0)", color: testing ? "#2040C0" : "#F0F4FA", boxShadow: testing ? "none" : "0 0 20px rgba(32,64,192,0.3)" }}>
                    <Play size={14} fill="currentColor" />
                    {testing ? "Running..." : "Run Live Test"}
                  </button>
                  <span style={{ color: "#8A9AB8", fontSize: 12 }}>Uses sandbox credentials</span>
                </div>
        ...
        <div className="space-y-5">
          <div className="flex gap-2">
            {Object.keys(sdkExamples).map(lang => (
              <button key={lang} onClick={() => setSdkLang(lang)}
                ...
                style={{ background: sdkLang===lang ? "#2040C0" : "rgba(32,64,192,0.06)", color: sdkLang===lang ? "#F0F4FA" : "#5A6A88", fontWeight: sdkLang===lang ? 700 : 400, ...MONO }}>
                {lang}
              </button>
            ))}
          </div>
          <div className="p-6 rounded-2xl" style={{ ...GLASS }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button style={{ color: "#2040C0", fontSize: 12 }}>Copy</button>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { title:"Rate Limiting", desc:"1,000 requests/minute per API key. 10,000/minute for Enterprise plans.", color:"#2040C0" },
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="p-6 rounded-2xl" style={GLASS}>
              ...
              <div className="space-y-3">
                <div className="p-4 rounded-xl" style={{ background: "rgba(32,64,192,0.06)", border: "1px solid rgba(32,64,192,0.2)" }}>
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {["vault.document.uploaded","contact.verified","storage.warning.80"].map(ev => (
                      <span key={ev} className="px-2 py-0.5 rounded text-xs" style={{ background: "rgba(32,64,192,0.1)", color: "#2040C0", ...MONO, fontSize: 9 }}>{ev}</span>
                    ))}
                  </div>
                  <div style={{ color: "#48BB78", fontSize: 11, marginTop: 8 }}>● Active · 0 failures</div>
                </div>
                <button className="w-full py-3 rounded-xl text-sm" style={{ border: "1px dashed rgba(32,64,192,0.3)", color: "#2040C0" }}>+ Add Webhook Endpoint</button>
              </div>
            </div>
            <div className="p-6 rounded-2xl" style={GLASS}>
              <h3 style={{ fontFamily: "var(--font-display)", fontSize: 16, color: "#0D1428", marginBottom: 16 }}>Webhook Signature Verification</h3>
              <pre className="p-4 rounded-xl overflow-x-auto text-xs" style={{ background: "#EAF0FC", border: "1px solid rgba(32,64,192,0.15)", color: "#68D391", ...MONO, whiteSpace: "pre-wrap" }}>
          ...
          <div className="p-6 rounded-2xl" style={GLASS}>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {webhookEvents.map(ev => (
                <div key={ev.event} className="flex items-start gap-3 p-3 rounded-xl" style={{ background: "rgba(32,64,192,0.04)" }}>
                  <Zap size={13} color="#2040C0" style={{ marginTop: 2, flexShrink: 0 }} />
                  <div>
                    <div style={{ color: "#2040C0", fontSize: 12, ...MONO }}>{ev.event}</div>
        ...
        <div className="space-y-5">
              ...
              <div className="space-y-3">
                {[{ label:"Live Secret Key", key:"fpd_live_sk_xxxxxxxxxxxxxxxxxxxxxxxx", type:"live" }, { label:"Live Publishable Key", key:"fpd_live_pk_xxxxxxxxxxxxxxxxxxxxxxxx", type:"live" }].map(k => (
                  <div key={k.label}>
                    ...
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: "#EAF0FC", border: "1px solid rgba(32,64,192,0.2)" }}>
                      <Lock size={12} color="#5A6A88" />
                      <span style={{ color: "#8A9AB8", fontSize: 12, ...MONO, flex: 1 }}>{k.key}</span>
                      <button onClick={copyKey} style={{ color: "#2040C0" }}>{keyCopied ? <CheckCircle size={13} /> : <Copy size={13} />}</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-6 rounded-2xl" style={GLASS}>
              ...
              <div className="space-y-3">
                {[{ label:"Sandbox Secret Key", key:"fpd_test_sk_xxxxxxxxxxxxxxxxxxxxxxxx" }, { label:"Sandbox Publishable Key", key:"fpd_test_pk_xxxxxxxxxxxxxxxxxxxxxxxx" }].map(k => (
                  <div key={k.label}>
                    <div style={{ color: "#5A6A88", fontSize: 11, marginBottom: 6 }}>{k.label}</div>
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: "#EAF0FC", border: "1px solid rgba(246,173,85,0.2)" }}>
          ...
          <div className="p-6 rounded-2xl" style={GLASS}>
            ...
            <div className="grid grid-cols-4 gap-4">
              {[{ label:"API Calls", value:"84,291", limit:"1M included", color:"#2040C0" }, { label:"Webhooks Sent", value:"12,841", limit:"Unlimited", color:"#48BB78" }, { label:"Documents Accessed", value:"2,190", limit:"Unlimited", color:"#9F7AEA" }, { label:"Rate Limit Hits", value:"0", limit:"1,000/min", color:"#F6AD55" }].map(stat => (
                <div key={stat.label} className="p-4 rounded-xl" style={{ background: "rgba(32,64,192,0.04)", border: "1px solid rgba(32,64,192,0.1)" }}>
