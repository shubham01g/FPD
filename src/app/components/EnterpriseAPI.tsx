...
const methodColor = { GET:"#48BB78", POST:"#6C5CE7", PUT:"#F6AD55", DELETE:"#FC8181", PATCH:"#9F7AEA" };
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Code size={15} color="#6C5CE7" />
            <span style={{ color: "#6C5CE7", fontSize: 11, ...MONO, letterSpacing: "0.12em" }}>ENTERPRISE</span>
          </div>
      ...
      <div className="flex gap-1 p-1 rounded-2xl" style={{ background: "rgba(255,255,255,0.95)", width: "fit-content" }}>
        {([["endpoints","Endpoints",<Book size={13}/>],["sdk","SDK Examples",<Terminal size={13}/>],["webhooks","Webhooks",<Webhook size={13}/>],["keys","API Keys",<Key size={13}/>]] as const).map(([id,label,icon]) => (
          <button key={id} onClick={() => setActiveTab(id)}
            ...
            style={{ background: activeTab===id ? "#6C5CE7" : "transparent", color: activeTab===id ? "#F0F4FA" : "#5A6A88", fontWeight: activeTab===id ? 700 : 400 }}>
            {icon} {label}
          </button>
        ))}
      </div>
          ...
          <div className="rounded-2xl overflow-hidden glow-surface" style={GLASS}>
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 700 }}>
              {apiGroups.map((group, gi) => (
                <div key={gi}>
                  <button className="w-full flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "rgba(108,92,231,0.06)", background: expandedGroup===gi ? "rgba(108,92,231,0.06)" : "transparent" }}
                  ...
                  {expandedGroup===gi && group.endpoints.map((ep, ei) => (
                    <button key={ei} className="w-full flex items-start gap-2 px-4 py-2.5 border-b text-left"
                      style={{ borderColor: "rgba(108,92,231,0.04)", background: activeEndpoint===ep ? "rgba(108,92,231,0.1)" : "transparent" }}
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => runTest(activeEndpoint)} disabled={testing}
                    ...
                    style={{ background: testing ? "rgba(108,92,231,0.1)" : "linear-gradient(135deg,#6C5CE7,#8B7CF6)", color: testing ? "#6C5CE7" : "#F0F4FA", boxShadow: testing ? "none" : "0 0 20px rgba(108,92,231,0.3)" }}>
        ...
        <div className="space-y-5">
          <div className="flex gap-2">
            {Object.keys(sdkExamples).map(lang => (
              <button key={lang} onClick={() => setSdkLang(lang)}
                ...
                style={{ background: sdkLang===lang ? "#6C5CE7" : "rgba(108,92,231,0.06)", color: sdkLang===lang ? "#F0F4FA" : "#5A6A88", fontWeight: sdkLang===lang ? 700 : 400, ...MONO }}>
                {lang}
              </button>
            ))}
          </div>
          <div className="p-6 rounded-2xl" style={{ ...GLASS }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button style={{ color: "#6C5CE7", fontSize: 12 }}>Copy</button>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { title:"Rate Limiting", desc:"1,000 requests/minute per API key. 10,000/minute for Enterprise plans.", color:"#6C5CE7" },
                ...
                <div className="p-4 rounded-xl glow-surface" style={{ background: "rgba(108,92,231,0.06)", border: "1px solid rgba(108,92,231,0.2)" }}>
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {["vault.document.uploaded","contact.verified","storage.warning.80"].map(ev => (
                      <span key={ev} className="px-2 py-0.5 rounded text-xs" style={{ background: "rgba(108,92,231,0.1)", color: "#6C5CE7", ...MONO, fontSize: 9 }}>{ev}</span>
                    ))}
                  </div>
              ...
              <div className="space-y-3">
                {[{ label:"Live Secret Key", key:"fpd_live_sk_xxxxxxxxxxxxxxxxxxxxxxxx", type:"live" }, { label:"Live Publishable Key", key:"fpd_live_pk_xxxxxxxxxxxxxxxxxxxxxxxx", type:"live" }].map(k => (
                  <div key={k.label}>
                    ...
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: "#EAF0FC", border: "1px solid rgba(108,92,231,0.2)" }}>
                      <Lock size={12} color="#5A6A88" />
                      <span style={{ color: "#8A9AB8", fontSize: 12, ...MONO, flex: 1 }}>{k.key}</span>
                      <button onClick={copyKey} style={{ color: "#6C5CE7" }}>{keyCopied ? <CheckCircle size={13} /> : <Copy size={13} />}</button>
                    </div>
                  </div>
                ))}
              </div>
