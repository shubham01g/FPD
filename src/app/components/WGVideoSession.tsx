    ...
    return (
      <div className="rounded-2xl p-5 space-y-4"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div style={{ fontFamily:"var(--font-display)", fontSize:17.5, color:"#E8EDF5" }}>
        ...
        {cameraError && (
          <div className="flex items-start gap-2 px-3 py-2.5 rounded-2xl"
            ...
            <span style={{ color:"#FC8181", fontSize:15, lineHeight:1.5 }}>{cameraError}</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={() => startSession("video")}
            ...
            <div className="text-center">
              <div style={{ fontFamily:"var(--font-display)", fontSize:16, color:"#E8EDF5", fontWeight:700 }}>
                Video Call
              </div>
              <div style={{ color:"#8A9AB8", fontSize:12.5, marginTop:3, lineHeight:1.5 }}>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl"
          ...
          <span style={{ color:"#8A9AB8", fontSize:14 }}>
  ...
  if (callState === "connecting") {
    ...
    return (
      <div className="rounded-2xl p-8 flex flex-col items-center gap-4"
        ...
        <div style={{ color:"#E8EDF5", fontFamily:"var(--font-display)", fontSize:20 }}>
          {isVideo ? "Starting video call…" : "Connecting phone call…"}
        </div>
        <div style={{ color:"#8A9AB8", fontSize:15, textAlign:"center" }}>
          {isVideo
      ...
      <div className="rounded-2xl p-5 space-y-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div style={{ color:"#8A9AB8", fontSize:15 }}>Duration: {formatTime(elapsed)} · ${charge} billed to card on file</div>
          </div>
        </div>
        {docsUploaded.length > 0 && (
          <div>
            <div style={{ color:"#8A9AB8", fontSize:12.5, ...MONO, marginBottom:4 }}>UPLOADED THIS SESSION</div>
      ...
      <div className="flex items-center justify-between px-4 py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full"
            ...
            <span style={{ color:"#FC8181", fontSize:12.5, fontWeight:700, ...MONO }}>
              {isVideo ? "VIDEO" : "PHONE"}
            </span>
          </div>
          {/* Timer */}
          <div className="flex items-center gap-1.5" style={{ color:"#E8EDF5" }}>
            ...
            <span style={{ fontSize:17.5, fontWeight:700, ...MONO }}>{formatTime(elapsed)}</span>
          </div>
          {/* Billing */}
            ...
            {isVideo ? (
              ...
              <div className="relative w-full" style={{ height:280, background:"#111827" }}>
                ...
                {clientConnected ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-3"
                    ...
                    <div className="flex items-center justify-center rounded-full font-bold"
                      style={{ width:72, height:72, background:"rgba(91,167,214,0.2)", color:"#D68FA8", fontSize:35.5, fontFamily:"var(--font-display)" }}>
                      {clientName.split(" ").map(w=>w[0]).join("").slice(0,2)}
                    </div>
                    <div style={{ color:"#E8EDF5", fontSize:17.5, fontFamily:"var(--font-display)" }}>{clientName}</div>
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full"
                      ...
                      <span style={{ color:"#D99A6B", fontSize:14 }}>Connected · camera on</span>
                    </div>
                    <div style={{ color:"#5A6A88", fontSize:14 }}>Client video feed · WebRTC in production</div>
                  </div>
                ) : (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-2" style={{ background:"#0f1626" }}>
                    ...
                    <div style={{ color:"#5A6A88", fontSize:16 }}>Waiting for {clientName} to join…</div>
                    <div style={{ color:"#374151", fontSize:14 }}>Client received a video join link</div>
                ...
                <div className="absolute bottom-4 right-4 rounded-2xl overflow-hidden"
                  ...
                  <div className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded"
                    style={{ background:"rgba(0,0,0,0.7)", color:"#E8EDF5", fontSize:11, ...MONO }}>You</div>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-4 py-10"
                ...
                <div className="relative flex items-center justify-center">
                  ...
                  <div className="flex items-center justify-center rounded-full font-bold"
                    style={{ width:64, height:64, background:"rgba(91,167,214,0.2)", color:"#6FAE8B", fontSize:27.5, fontFamily:"var(--font-display)", border:"2px solid rgba(91,167,214,0.4)", position:"relative" }}>
                    {clientName.split(" ").map(w=>w[0]).join("").slice(0,2)}
                  </div>
                </div>
                <div style={{ color:"#E8EDF5", fontFamily:"var(--font-display)", fontSize:20 }}>{clientName}</div>
                {clientConnected ? (
                  <div className="flex items-center gap-2 px-4 py-1.5 rounded-full"
                    style={{ background:"rgba(72,187,120,0.12)", border:"1px solid rgba(72,187,120,0.25)" }}>
                    <div className="w-2 h-2 rounded-full" style={{ background:"#48BB78", animation:"pulse 1s ease-in-out infinite" }}/>
                    <span style={{ color:"#D99A6B", fontSize:15, fontWeight:600 }}>On the line · audio only</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2" style={{ color:"#5A6A88", fontSize:15 }}>
          ...
          {showTools && (
            <div className="flex-shrink-0 border-l overflow-y-auto"
              ...
              <div className="p-3 space-y-3">
                ...
                {/* Quick upload */}
                <div>
                  <div style={{ color:"#8A9AB8", fontSize:11, ...MONO, marginBottom:6 }}>UPLOAD DURING CALL</div>
                ...
                <div>
                  <div style={{ color:"#8A9AB8", fontSize:11, ...MONO, marginBottom:6 }}>SAVE TO VAULT SECTION</div>
                  <div className="space-y-1">
                    {VAULT_SECTIONS.map(sec => (
                      <button key={sec.id}
                        ...
                        <span style={{ fontSize:17.5 }}>{sec.icon}</span>
