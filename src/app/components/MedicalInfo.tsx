...
function Field({ label, value }: { label:string; value:string }) {
  return (
    <div className="flex flex-col gap-1 px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.04)" }}>
      <span style={{ color:"#5A6A88", fontSize:10, ...MONO }}>{label.toUpperCase()}</span>
      <span style={{ color:"#0D1428", fontSize:13 }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.8)", backdropFilter:"blur(8px)" }}>
      <div className="w-full max-w-sm rounded-2xl p-6" style={GLASS}>
        <div className="flex items-center justify-between mb-5"><h3 style={{ fontFamily:"var(--font-display)", fontSize:17, color:"#0D1428" }}>Add Allergy</h3><button onClick={onClose} style={{ color:"#5A6A88" }}><X size={15}/></button></div>
        <div className="space-y-3">
          ...
          <div><label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>SEVERITY</label>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={submit} disabled={loading} className="flex-1 py-2.5 rounded-xl font-semibold text-sm" style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", opacity:loading?0.7:1 }}>{loading?"Saving...":"Save Allergy"}</button>
...
function AddMedModal({ onClose, onAdd }: { onClose:()=>void; onAdd:(m:Omit<Medication,"id">)=>Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.8)", backdropFilter:"blur(8px)" }}>
      <div className="w-full max-w-sm rounded-2xl p-6" style={GLASS}>
        <div className="flex items-center justify-between mb-5"><h3 style={{ fontFamily:"var(--font-display)", fontSize:17, color:"#0D1428" }}>Add Medication</h3><button onClick={onClose} style={{ color:"#5A6A88" }}><X size={15}/></button></div>
        <div className="space-y-3">
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={submit} disabled={loading} className="flex-1 py-2.5 rounded-xl font-semibold text-sm" style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", opacity:loading?0.7:1 }}>{loading?"Saving...":"Save Medication"}</button>
            <button onClick={onClose} className="px-4 py-2.5 rounded-xl text-sm" style={{ background:"rgba(32,64,192,0.06)", color:"#5A6A88" }}>Cancel</button>
          </div>
        </div>
      </div>
...
export function MedicalInfo() {
  ...
  return (
    <div className="p-6 space-y-5 relative" style={{ maxWidth:1100, ...GRID }}>
      <div>
        <h1 style={{ fontFamily:"var(--font-display)", fontSize:26, color:"#0D1428", marginBottom:4 }}>Medical Information</h1>
        <p style={{ color:"#5A6A88", fontSize:13 }}>Critical medical data accessible to verified legacy contacts and first responders.</p>
      </div>
      ...
      {tab==="emergency" && (
        <div className="grid md:grid-cols-2 gap-5">
          <div className="p-6 rounded-2xl space-y-3" style={GLASS}>
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:15, color:"#0D1428", marginBottom:12 }}>Vital Information</h3>
            <div className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.08)", border:"1px solid rgba(32,64,192,0.2)" }}>
              <span style={{ color:"#5A6A88", fontSize:13 }}>Blood Type</span>
              <span style={{ fontFamily:"var(--font-display)", fontSize:24, color:"#2040C0", fontWeight:700 }}>{emergencyInfo.bloodType}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {[{label:"DNR on File",active:emergencyInfo.dnr},{label:"Organ Donor",active:emergencyInfo.organDonor},{label:"Advance Directive",active:emergencyInfo.advanceDirective}].map(b=>(
                <span key={b.label} className="px-3 py-1 rounded-full text-xs font-semibold"
                  style={{ background:b.active?"rgba(72,187,120,0.12)":"rgba(107,114,128,0.12)", color:b.active?"#48BB78":"#5A6A88" }}>
                  {b.active?"✓":"✗"} {b.label}
                </span>
              ))}
            </div>
          ...
          <div className="p-6 rounded-2xl space-y-3" style={GLASS}>
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:15, color:"#0D1428", marginBottom:12 }}>Primary Physician & Hospital</h3>
          ...
          <div className="p-6 rounded-2xl md:col-span-2" style={GLASS}>
            <h3 style={{ fontFamily:"var(--font-display)", fontSize:15, color:"#0D1428", marginBottom:12 }}>Active Medical Conditions</h3>
            <div className="flex flex-wrap gap-2">
              {emergencyInfo.conditions.map(c=>(
                <span key={c} className="px-3 py-2 rounded-xl text-sm" style={{ background:"rgba(32,64,192,0.06)", color:"#374669", border:"1px solid rgba(32,64,192,0.12)" }}>
                  <Heart size={11} color="#2040C0" style={{ display:"inline", marginRight:6 }}/>{c}
                </span>
              ))}
            </div>
          </div>
      ...
      {tab==="allergies" && (
        <div className="space-y-3">
          ...
          {allergies.length===0 && <div className="py-12 text-center rounded-2xl" style={GLASS}><AlertTriangle size={28} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/><div style={{ color:"#8A9AB8" }}>No allergies recorded.</div></div>}
          {allergies.map(a=>{
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span style={{ color:"#0D1428", fontSize:15, fontWeight:500 }}>{a.allergen}</span>
                      <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background:s.bg, color:s.color, ...MONO, fontSize:9 }}>{s.label}</span>
                      <span className="px-2 py-0.5 rounded text-xs capitalize" style={{ background:"rgba(32,64,192,0.06)", color:"#5A6A88" }}>{a.type}</span>
                    </div>
                    <div style={{ color:"#5A6A88", fontSize:13 }}>Reaction: {a.reaction}</div>
                    <div style={{ color:"#8A9AB8", fontSize:11, marginTop:3, ...MONO }}>Diagnosed: {a.diagnosed}</div>
                  </div>
      ...
      {tab==="medications" && (
        <div className="space-y-3">
          ...
          {medications.length===0 && <div className="py-12 text-center rounded-2xl" style={GLASS}><Pill size={28} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/><div style={{ color:"#8A9AB8" }}>No medications recorded.</div></div>}
          {medications.map(m=>(
            <div key={m.id} className="p-5 rounded-2xl" style={GLASS}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <Pill size={15} color="#2040C0"/>
                    <span style={{ fontFamily:"var(--font-display)", fontSize:16, color:"#0D1428" }}>{m.name}</span>
                    <span style={{ color:"#2040C0", ...MONO, fontSize:13, fontWeight:700 }}>{m.dose}</span>
                  </div>
                  <div className="grid md:grid-cols-3 gap-2">
                    {[{l:"Frequency",v:m.frequency},{l:"Condition",v:m.condition},{l:"Prescriber",v:m.prescriber},{l:"Pharmacy",v:m.pharmacy},{l:"Next Refill",v:m.refillDate}].map(f=>(
                      <div key={f.l} className="px-3 py-2 rounded-xl" style={{ background:"rgba(32,64,192,0.04)" }}>
                        <div style={{ color:"#5A6A88", fontSize:9, ...MONO, marginBottom:2 }}>{f.l.toUpperCase()}</div>
                        <div style={{ color:"#0D1428", fontSize:12 }}>{f.v}</div>
                      </div>
                    ))}
                  </div>
                </div>
