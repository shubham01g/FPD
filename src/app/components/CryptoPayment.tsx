...
function Countdown({ seconds, onExpire }: { seconds: number; onExpire: () => void }) {
  ...
  return (
    <div className="text-center">
      <div style={{ color, fontSize:35.5, fontFamily:"var(--font-mono)", fontWeight:700, lineHeight:1 }}>
        {String(m).padStart(2,"0")}:{String(s).padStart(2,"0")}
      </div>
      <div style={{ color:"#8A9AB8", fontSize:12.5, marginTop:2 }}>Payment window remaining</div>
      ...
      <div className="w-full max-w-md rounded-3xl overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div style={{ color:"#E8EDF5", fontSize:17.5, fontWeight:600 }}>Pay with Crypto</div>
              <div style={{ color:"#4A5A7A", fontSize:14, fontFamily:"var(--font-mono)" }}>{label} · ${amountUSD.toFixed(2)} USD</div>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-5">
          ...
          {step === "select" && (
            <div className="space-y-4">
              <div style={{ color:"#6B7FA8", fontSize:15, textAlign:"center" }}>
                  ...
                  return (
                    <button key={coin.id} onClick={() => startPayment(coin)}
                      ...
                      <div className="flex items-center justify-center rounded-full font-bold flex-shrink-0"
                        style={{ width:40, height:40, background:`${coin.color}20`, color:coin.color, fontSize:22.5 }}>
                        {coin.logoChar}
                      </div>
                      <div className="flex-1 text-left">
                        <div style={{ color:"#E8EDF5", fontSize:16, fontWeight:600 }}>{coin.name}</div>
                        <div style={{ color:"#4A5A7A", fontSize:14 }}>{coin.network}</div>
                      </div>
                      <div className="text-right">
                        <div style={{ color:coin.color, fontSize:16, fontWeight:700, fontFamily:"var(--font-mono)" }}>
                          {amt} {coin.symbol}
                        </div>
                        <div style={{ color:"#4A5A7A", fontSize:12.5 }}>${coin.demoRate.toLocaleString()}/{coin.symbol}</div>
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="pt-2 border-t" style={{ borderColor:"rgba(255,255,255,0.06)" }}>
                <div style={{ color:"#4A5A7A", fontSize:12.5, fontFamily:"var(--font-mono)", marginBottom:8, textAlign:"center" }}>POWERED BY</div>
                <div className="flex justify-center gap-4">
                  {PROCESSORS.filter(p=>p.status==="connected").map(p => (
                    <div key={p.id} className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl"
                      style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)" }}>
                      <span style={{ fontSize:17.5 }}>{p.logo}</span>
                      <span style={{ color:"#6B7FA8", fontSize:14 }}>{p.name}</span>
                    </div>
                  ))}
                </div>
                <div style={{ color:"#4A5A7A", fontSize:12.5, textAlign:"center", marginTop:8 }}>
            ...
            <div className="space-y-5 text-center">
              ...
              <div className="flex items-center justify-center gap-3">
                <div className="flex items-center justify-center rounded-full font-bold"
                  style={{ width:48, height:48, background:`${selectedCoin.color}20`, color:selectedCoin.color, fontSize:27.5 }}>
                  {selectedCoin.logoChar}
                </div>
                <div className="text-left">
                  <div style={{ color:"#E8EDF5", fontSize:27.5, fontWeight:700, fontFamily:"var(--font-mono)" }}>
                    {cryptoAmount} {selectedCoin.symbol}
                  </div>
                  <div style={{ color:"#6B7FA8", fontSize:15 }}>${amountUSD.toFixed(2)} USD · {selectedCoin.network}</div>
                </div>
              </div>
              ...
              {/* Wallet address */}
              <div>
                <div style={{ color:"#6B7FA8", fontSize:12.5, fontFamily:"var(--font-mono)", marginBottom:6 }}>
                ...
                <div className="flex items-center gap-2 px-3 py-3 rounded-2xl"
                  ...
                  <code style={{ color:"#E8EDF5", fontSize:14, flex:1, textAlign:"left", wordBreak:"break-all", fontFamily:"var(--font-mono)" }}>
              ...
              <div className="flex items-center justify-between px-3 py-2 rounded-2xl"
                style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)" }}>
                <span style={{ color:"#6B7FA8", fontSize:14, fontFamily:"var(--font-mono)" }}>INVOICE ID</span>
                <span style={{ color:"#E8EDF5", fontSize:14, fontFamily:"var(--font-mono)" }}>{invoiceId}</span>
              </div>
              ...
              <div className="space-y-2 text-left">
                <div className="flex items-start gap-2 px-3 py-2.5 rounded-2xl"
                  ...
                  <p style={{ color:"#F6AD55", fontSize:14, lineHeight:1.6 }}>
                ...
                <div className="flex items-start gap-2 px-3 py-2.5 rounded-2xl"
                  ...
                  <p style={{ color:"#6FAE8B", fontSize:14, lineHeight:1.6 }}>
              ...
              <div className="px-3 py-2 rounded-2xl"
                ...
                <span style={{ color:"#D99A6B", fontSize:12.5, fontFamily:"var(--font-mono)" }}>
          ...
          {step === "confirming" && (
            <div className="text-center py-8 space-y-6">
              ...
              <div>
                <div style={{ color:"#E8EDF5", fontSize:22.5, fontWeight:600, marginBottom:6 }}>Payment Detected</div>
                <div style={{ color:"#6B7FA8", fontSize:16, lineHeight:1.8 }}>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between px-4 py-3 rounded-2xl"
                  ...
                  <span style={{ color:"#6B7FA8", fontSize:15 }}>Amount</span>
                  <span style={{ color:selectedCoin.color, fontSize:16, fontFamily:"var(--font-mono)", fontWeight:700 }}>
                    {cryptoAmount} {selectedCoin.symbol}
                  </span>
                </div>
                <div className="flex items-center justify-between px-4 py-3 rounded-2xl"
                  style={{ background:"rgba(255,255,255,0.04)" }}>
                  <span style={{ color:"#6B7FA8", fontSize:15 }}>Network</span>
                  <span style={{ color:"#E8EDF5", fontSize:15 }}>{selectedCoin.network}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-3 rounded-2xl"
                  style={{ background:"rgba(255,255,255,0.04)" }}>
                  <span style={{ color:"#6B7FA8", fontSize:15 }}>TX Hash</span>
                  <span style={{ color:"#E8EDF5", fontSize:12.5, fontFamily:"var(--font-mono)" }}>
          ...
          {step === "success" && (
            <div className="text-center py-8 space-y-5">
              ...
              <div>
                <div style={{ color:"#E8EDF5", fontSize:27.5, fontWeight:700, marginBottom:6 }}>Payment Confirmed!</div>
                <div style={{ color:"#6B7FA8", fontSize:16, lineHeight:1.8 }}>
              ...
              <div className="space-y-2 text-left">
                {[
                  ...
                  <div key={k} className="flex justify-between px-4 py-2.5 rounded-2xl"
                    ...
                    <span style={{ color:"#6B7FA8", fontSize:15 }}>{k}</span>
                    <span style={{ color: k==="Status" ? "#D99A6B" : "#E8EDF5", fontSize:15, fontFamily:k==="Invoice"?"var(--font-mono)":"inherit" }}>{v}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {step === "expired" && (
            <div className="text-center py-8 space-y-5">
              ...
              <div>
                <div style={{ color:"#E8EDF5", fontSize:25, fontWeight:600, marginBottom:6 }}>Payment Window Expired</div>
                <div style={{ color:"#6B7FA8", fontSize:16, lineHeight:1.8 }}>
...
export function CryptoPayButton({
  ...
  const styles: Record<string, React.CSSProperties> = {
    sm: { padding:"6px 12px", fontSize:14, borderRadius:10 },
    md: { padding:"10px 18px", fontSize:16, borderRadius:12 },
    lg: { padding:"14px 28px", fontSize:19, borderRadius:14 },
  };
  return (
    <>
      <button onClick={() => setOpen(true)}
        ...
        <span style={{ fontSize: size === "sm" ? 16 : size === "lg" ? 25 : 20 }}>₿</span>
        Pay with Crypto
      </button>
      <CryptoPayment
        open={open}
