...
/* Icon-chip tone → colour, used for the at-a-glance stats. */
...
/* ── At-a-glance stats — single row of 4 ── */
.fpd-dash .kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;}
@media (max-width:900px){.fpd-dash .kpi-grid{grid-template-columns:1fr 1fr;}}
...
/* ── Quick actions — a slim inline strip instead of a full card ── */
.fpd-dash .action-strip{display:flex;gap:10px;flex-wrap:wrap;}
.fpd-dash .astrip-btn{display:inline-flex;align-items:center;gap:9px;padding:11px 18px;border-radius:99px;border:1px solid;cursor:pointer;font-size:14px;font-weight:600;font-family:var(--font-body);transition:transform .15s;}
.fpd-dash .astrip-btn:hover{transform:translateY(-1px);}

/* ── Storage & usage (with trend graph) ── */
...
export function UserDashboard({ onNavigate }: UserDashboardProps) {
  ...
  /* ── The six estate essentials — still drive the header's readiness
     figure and "finish your vault" CTA, just no longer rendered as their
     own checklist card. ── */
  const essentials = [
  ...
  const quickActions = [
    { icon: <Plus size={16} />, label: "Add document", page: "file-cabinet", hue: ACCENT },
    { icon: <Users size={16} />, label: "Add contact", page: "contacts-legacy", hue: POS },
    ...
    <div className="fpd-dash">
      ...
      <div className="wrap">
        ...
        <div className="action-strip">
          {quickActions.map(a => (
            <button
              key={a.label} className="astrip-btn"
              style={{ borderColor: `${a.hue}44`, background: `${a.hue}14`, color: a.hue }}
          ...
          <div className="card pad">
            ...
            {docCategories.length > 0 && (
              <div className="grid-legend">
                ...
                <span className="legend-note">{docsTotal} documents total, {Math.min(4, docsTotal)} shown</span>
              </div>
            )}
          </div>
