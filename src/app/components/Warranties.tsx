      ...
      <div className="flex flex-wrap gap-1.5">
        {["all",...CATEGORIES].map(cat=>(
          <button key={cat} onClick={()=>setFilterCat(cat)}
            ...
            style={{ background:filterCat===cat?"var(--primary)":"rgba(32,64,192,0.05)", color:filterCat===cat?"#070D1A":"var(--muted-foreground)", border:`1px solid ${filterCat===cat?"var(--primary)":"rgba(32,64,192,0.12)"}` }}>
            {cat==="all" ? "All" : cat}
          </button>
        ))}
      </div>
            ...
            <button className="w-full p-5 text-left" onClick={()=>setExpanded(expanded===w.id?null:w.id)}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="flex items-center justify-center rounded-xl flex-shrink-0" style={{ width:44, height:44, background:"rgba(32,64,192,0.08)" }}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded" style={{ background:"rgba(32,64,192,0.07)", color:"var(--primary)" }}>{w.category}</span>
                    </div>
                  </div>
                </div>
