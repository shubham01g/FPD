...
const FULL_COVERAGE = [
  { icon:<FileText size={14}/>,    color:"#2040C0", label:"All Documents (18 folders)", desc:"Every file uploaded across all 18 folder categories" },
  ...
  { icon:<Camera size={14}/>,      color:"#3355E0", label:"Memories & Family Media", desc:"Photos, videos, written memories, diary entries" },
      ...
      <div style={{ maxWidth:960, margin:"0 auto" }} className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Shield size={15} color="#2040C0"/>
            <span style={{ color:"#2040C0", fontSize:11, ...MONO, letterSpacing:"0.1em" }}>LEGACY PROTECTION</span>
          </div>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:26, color:"#0D1428", marginBottom:8 }}>Activate Legacy Access</h1>
          <p style={{ color:"#5A6A88", fontSize:14, lineHeight:1.8 }}>
            A one-time fee of <strong style={{ color:"#0D1428" }}>$199</strong> that preserves your legacy contacts' ability to download your
            <strong style={{ color:"#0D1428" }}> complete Final Pass Down account</strong> — every document, video, memory, record, and file you have ever uploaded — after your passing is verified.
          </p>
        </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {!status.paid && (
              <button onClick={() => { setShowPayment(true); }}
                ...
                style={{ background:"rgba(32,64,192,0.1)", color:"#2040C0" }}>
                1. Pay Fee →
              </button>
            )}
        ...
        <div className="p-5 rounded-2xl" style={{ background:"rgba(32,64,192,0.04)", border:"1px solid rgba(32,64,192,0.15)" }}>
          <div style={{ color:"#2040C0", fontSize:12, fontWeight:700, ...MONO, marginBottom:10 }}>HOW ACCESS WORKS — TWO CONDITIONS MUST BOTH BE MET</div>
          <div className="grid md:grid-cols-2 gap-4">
            <div className="flex items-start gap-3 p-4 rounded-xl"
              style={{ background:status.paid?"rgba(72,187,120,0.08)":"rgba(32,64,192,0.04)", border:`1px solid ${status.paid?"rgba(72,187,120,0.25)":"rgba(32,64,192,0.12)"}` }}>
              <div className="flex items-center justify-center rounded-full flex-shrink-0"
                style={{ width:32, height:32, background:status.paid?"rgba(72,187,120,0.15)":"rgba(32,64,192,0.1)", color:status.paid?"#48BB78":"#2040C0", fontSize:13, fontWeight:700 }}>
        ...
        <div className="p-5 rounded-2xl" style={CARD}>
          <button onClick={() => setShowCoverage(!showCoverage)}
            ...
            <div className="flex items-center gap-2">
              <Download size={16} color="#2040C0"/>
              <span style={{ fontFamily:"var(--font-display)", fontSize:16, color:"#0D1428" }}>What the Legacy Vault Clone Downloads</span>
            </div>
            {showCoverage ? <ChevronUp size={16} color="#8A9AB8"/> : <ChevronDown size={16} color="#8A9AB8"/>}
          </button>
          <p style={{ color:"#5A6A88", fontSize:13, marginTop:6, lineHeight:1.7 }}>
            When conditions are met and a legacy contact clicks <strong style={{ color:"#0D1428" }}>LEGACY VAULT CLONE</strong>, they receive a complete, encrypted download of <strong style={{ color:"#2040C0" }}>everything</strong> the account holder ever saved — across all 30+ life categories.
          </p>
          {showCoverage && (
            <div className="grid md:grid-cols-2 gap-3 mt-4">
              {FULL_COVERAGE.map(item => (
                <div key={item.label} className="flex items-start gap-3 px-4 py-3 rounded-xl"
                  style={{ background:"rgba(32,64,192,0.03)", border:"1px solid rgba(32,64,192,0.08)" }}>
                  <div className="flex items-center justify-center rounded-lg flex-shrink-0"
                    style={{ width:28, height:28, background:`${item.color}14`, color:item.color }}>
                    {item.icon}
                  </div>
                  <div>
                    <div style={{ color:"#0D1428", fontSize:12, fontWeight:600 }}>{item.label}</div>
                    <div style={{ color:"#8A9AB8", fontSize:11, lineHeight:1.5 }}>{item.desc}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {status.paid && (
          <div className="flex items-start gap-4 p-6 rounded-2xl border" style={{ background:"rgba(72,187,120,0.05)", borderColor:"rgba(72,187,120,0.3)" }}>
            ...
            <div className="flex-1">
              <div style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#0D1428", marginBottom:6 }}>Legacy Continuation Fee Paid ✓</div>
              <p style={{ color:"#5A6A88", fontSize:13, lineHeight:1.7, marginBottom:12 }}>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={item.label} className="px-4 py-3 rounded-xl" style={{ background:"rgba(72,187,120,0.06)", border:"1px solid rgba(72,187,120,0.15)" }}>
                    ...
                    <div style={{ color:"#0D1428", fontSize:12, fontWeight:500 }}>{item.value}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
            ...
            {!status.paid ? (
              <div className="p-6 rounded-2xl" style={CARD}>
                <div className="text-center mb-6">
                  <div style={{ fontFamily:"var(--font-display)", fontSize:52, color:"#2040C0", fontWeight:900, lineHeight:1 }}>$199</div>
                  <div style={{ color:"#5A6A88", fontSize:14, marginTop:4 }}>One-time · Never expires · Non-refundable</div>
                ...
                <div className="space-y-3 mb-5">
                  <div style={{ color:"#5A6A88", fontSize:11, ...MONO, marginBottom:6 }}>WHO IS PAYING?</div>
                  {[
                    ...
                    <button key={opt.id} onClick={() => setPayerType(opt.id as any)}
                      ...
                      style={{ background:payerType===opt.id?"rgba(32,64,192,0.08)":"#F5F8FE", border:`2px solid ${payerType===opt.id?"#2040C0":"transparent"}`, color:"#0D1428" }}>
                      <div className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-0.5"
                        style={{ borderColor:payerType===opt.id?"#2040C0":"#C8D4EE" }}>
                        {payerType===opt.id && <div style={{ width:8, height:8, borderRadius:"50%", background:"#2040C0" }}/>}
                      </div>
                      <div>
                        <div style={{ fontSize:13, fontWeight:500 }}>{opt.label}</div>
                        <div style={{ fontSize:11, color:"#8A9AB8", marginTop:2 }}>{opt.sub}</div>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  <button onClick={() => setShowPayment(true)} className="w-full py-4 rounded-2xl font-bold text-base"
                    style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#fff", boxShadow:"0 8px 24px rgba(32,64,192,0.4)" }}>
                  ...
                  <div className="flex items-center justify-center gap-5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span style={{ color:"#8A9AB8", fontSize:10 }}>Stripe · PCI-DSS</span>
                    </div>
                    <span style={{ color:"#C8D4EE" }}>·</span>
                    <div className="flex items-center gap-1.5">
                      <span style={{ color:"#F7931A", fontSize:12 }}>₿</span>
                      <span style={{ color:"#8A9AB8", fontSize:10 }}>Coinbase Commerce · BitPay</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl text-center space-y-4" style={CARD}>
                ...
                <div style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#0D1428" }}>Fee Paid — Awaiting Confirmation of Passing</div>
                <p style={{ color:"#5A6A88", fontSize:13, lineHeight:1.7 }}>
                  Your legacy contacts will be able to download <strong style={{ color:"#2040C0" }}>everything in your account</strong> for <strong style={{ color:"#2040C0" }}>{status.activationPeriod} months</strong> from your date of passing — once a legacy contact submits confirmation of passing and FPD administrators verify it.
                </p>
                <div className="px-4 py-3 rounded-xl text-left" style={{ background:"rgba(32,64,192,0.04)", border:"1px solid rgba(32,64,192,0.1)" }}>
                  <div style={{ color:"#8A9AB8", fontSize:10, fontFamily:"var(--font-mono)", marginBottom:6 }}>ACCEPTED CONFIRMATION DOCUMENTS</div>
                ...
                <div className="px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.05)", border:"1px solid rgba(32,64,192,0.1)" }}>
                  <div style={{ color:"#8A9AB8", fontSize:10, ...MONO, marginBottom:4 }}>TRANSACTION ID</div>
                  <div style={{ color:"#2040C0", fontSize:13, ...MONO }}>{status.transactionId}</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-5 rounded-2xl space-y-4" style={CARD}>
            <div style={{ fontFamily:"var(--font-display)", fontSize:16, color:"#0D1428", marginBottom:4 }}>Common Questions</div>
            {[
              ...
              <div key={item.q} className="pb-3 border-b" style={{ borderColor:"rgba(32,64,192,0.06)" }}>
                <div style={{ color:"#0D1428", fontSize:12, fontWeight:600, marginBottom:3 }}>{item.q}</div>
                <div style={{ color:"#5A6A88", fontSize:12, lineHeight:1.6 }}>{item.a}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.45)", backdropFilter:"blur(8px)" }}>
          <div className="w-full max-w-md rounded-2xl p-7" style={CARD}>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background:"rgba(32,64,192,0.1)" }}>
                <CreditCard size={20} color="#2040C0"/>
              </div>
              <div>
                <div style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#0D1428" }}>Payment Details</div>
                <div style={{ color:"#5A6A88", fontSize:12 }}>Legacy Continuation Fee · $199.00</div>
              </div>
            </div>
            <div className="space-y-4">
              <div>
                <label style={{ color:"#5A6A88", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>CARDHOLDER NAME</label>
                <input value={name} onChange={e=>setName(e.target.value)} placeholder="James Doe"
                  className="w-full px-4 py-3 rounded-xl" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.15)", color:"#0D1428", fontSize:14, outline:"none" }}/>
              </div>
              <div>
                <label style={{ color:"#5A6A88", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>CARD NUMBER</label>
                <input value={cardNum} onChange={e=>setCardNum(formatCard(e.target.value))} placeholder="4242 4242 4242 4242" maxLength={19}
                  className="w-full px-4 py-3 rounded-xl" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.15)", color:"#0D1428", fontSize:14, outline:"none", fontFamily:"var(--font-mono)" }}/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label style={{ color:"#5A6A88", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>EXPIRY</label>
                  <input value={expiry} onChange={e=>setExpiry(formatExpiry(e.target.value))} placeholder="MM/YY" maxLength={5}
                    className="w-full px-4 py-3 rounded-xl" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.15)", color:"#0D1428", fontSize:14, outline:"none", fontFamily:"var(--font-mono)" }}/>
                </div>
                <div>
                  <label style={{ color:"#5A6A88", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>CVC</label>
                  <input value={cvv} onChange={e=>setCvv(e.target.value.replace(/\D/g,"").slice(0,4))} placeholder="•••"
                    className="w-full px-4 py-3 rounded-xl" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.15)", color:"#0D1428", fontSize:14, outline:"none", fontFamily:"var(--font-mono)" }}/>
                </div>
              </div>
              <div className="px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.04)", border:"1px solid rgba(32,64,192,0.12)" }}>
                <div className="flex justify-between mb-1"><span style={{ color:"#5A6A88", fontSize:13 }}>Legacy Continuation Fee</span><span style={{ color:"#0D1428", fontSize:13 }}>$199.00</span></div>
                <div className="flex justify-between mb-1"><span style={{ color:"#5A6A88", fontSize:13 }}>Processing Fee</span><span style={{ color:"#0D1428", fontSize:13 }}>$0.00</span></div>
                <div className="border-t my-2" style={{ borderColor:"rgba(32,64,192,0.1)" }}/>
                <div className="flex justify-between"><span style={{ color:"#0D1428", fontSize:14, fontWeight:700 }}>Total</span><span style={{ color:"#2040C0", fontSize:16, fontWeight:700, fontFamily:"var(--font-display)" }}>$199.00</span></div>
              </div>
              <div className="flex gap-3">
                <button onClick={handlePay} disabled={processing} className="flex-1 py-3.5 rounded-xl font-bold text-sm"
                  style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#fff", boxShadow:"0 4px 16px rgba(32,64,192,0.35)", opacity:processing?0.8:1 }}>
                  {processing ? "Processing Stripe Payment..." : "Pay $199 Now"}
                </button>
                <button onClick={() => setShowPayment(false)} className="px-5 py-3.5 rounded-xl text-sm" style={{ background:"#F0F4FA", color:"#5A6A88" }}>Cancel</button>
              </div>
              <div className="flex items-center justify-center gap-2">
                <Lock size={11} color="#8A9AB8"/>
                <span style={{ color:"#8A9AB8", fontSize:11 }}>256-bit SSL · Stripe PCI-DSS Level 1</span>
              </div>
            </div>
          </div>
        </div>
