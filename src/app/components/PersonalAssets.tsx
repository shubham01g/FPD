...
export function PersonalAssets() {
  ...
  const Field = ({ label, value }: { label: string; value: string }) => (
    <div className="flex flex-col gap-1 px-3 py-2.5 rounded-lg" style={{ background: "#EAF0FC" }}>
            ...
            <div key={v.id} className="rounded-xl border overflow-hidden" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="rounded-xl p-2.5" style={{ background: "rgba(32,64,192,0.1)" }}><Car size={20} color="var(--primary)" /></div>
                  <div>
        ...
        <div className="space-y-4">
          ...
          {realEstateList.map(r => (
            <div key={r.id} className="rounded-xl border overflow-hidden" style={{ background:"var(--card)", borderColor:"var(--border)" }}>
              {/* Property photo */}
              <div className="relative" style={{ height: r.photo ? 220 : 80, background: r.photo ? "transparent" : "rgba(32,64,192,0.04)", borderBottom:"1px solid var(--border)" }}>
                {r.photo ? (
                  <img src={r.photo} alt={r.address} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                ) : (
                  <div className="flex flex-col items-center justify-center h-full gap-2">
                    <Home size={28} color="rgba(32,64,192,0.25)"/>
          ...
          {digitalList.map(d => (
            <div key={d.id} className="p-5 rounded-xl border" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="px-2 py-0.5 rounded text-xs" style={{ background: "rgba(32,64,192,0.1)", color: "var(--primary)", fontFamily: "var(--font-mono)" }}>{d.category.toUpperCase()}</span>
                  </div>
        ...
        <div className="space-y-4">
          ...
          {weaponList.map(w => (
            <div key={w.id} className="rounded-xl border overflow-hidden" style={{ background:"var(--card)", borderColor:"var(--border)" }}>
              {/* Firearm photo */}
              <div className="relative" style={{ height: w.photo ? 200 : 72, background:"rgba(32,64,192,0.03)", borderBottom:"1px solid var(--border)" }}>
                {w.photo ? (
                  <img src={w.photo} alt={`${w.make} ${w.model}`} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                ) : (
                  <div className="flex items-center justify-center h-full gap-2">
                    <Image size={20} color="rgba(32,64,192,0.2)"/>
              ...
              <div className="p-5">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {[["Blade / Edge",w.blade],["Handle / Grip",w.handle],["Storage Location",w.storage],["Transfer Instructions",w.transfer]].map(([label,value])=>(
                    <div key={label} className="flex flex-col gap-1 px-3 py-2.5 rounded-lg" style={{ background:"#EAF0FC" }}>
