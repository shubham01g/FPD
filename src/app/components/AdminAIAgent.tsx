...
function TypingIndicator() {
  return (
    <div className="flex gap-1 items-center px-4 py-3">
      {[0,1,2].map(i => (
        <div key={i} style={{ width:7, height:7, borderRadius:"50%", background:"#9F7AEA", animation:`pulse 1.4s ease-in-out ${i*0.2}s infinite`, opacity:0.6 }}/>
      ))}
    </div>
  );
}
...
export function AdminAIAgent({ inline = false }: { inline?: boolean }) {
  ...
  const wrapStyle: React.CSSProperties = inline
    ? { display:"flex", flexDirection:"column", height:"calc(100vh - 120px)", borderRadius:16, overflow:"hidden", border:"1px solid rgba(159,122,234,0.25)", background:"#FFFFFF", boxShadow:"0 4px 20px rgba(159,122,234,0.1)" }
    : { position:"fixed", bottom:24, left:24, zIndex:50, display:"flex", flexDirection:"column", borderRadius:16, overflow:"hidden", boxShadow:"0 20px 60px rgba(159,122,234,0.3)", width:390, height:minimized?"auto":580, background:"#FFFFFF", border:"1px solid rgba(159,122,234,0.25)" };
  ...
  if (!open && !inline) return (
    <button onClick={() => setOpen(true)}
      ...
      style={{ background:"linear-gradient(135deg,#9F7AEA,#C4B5FD)", color:"#04080F", boxShadow:"0 8px 32px rgba(159,122,234,0.45)" }}>
  ...
  return (
    <div style={wrapStyle}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ background:"linear-gradient(135deg,#9F7AEA,#7C3AED)" }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div style={{ color:"#fff", fontSize:13, fontWeight:700 }}>Admin AI Assistant</div>
            <div className="flex items-center gap-1">
              <div style={{ width:6, height:6, borderRadius:"50%", background:"#48BB78", boxShadow:"0 0 6px #48BB78" }}/>
              <span style={{ color:"rgba(255,255,255,0.7)", fontSize:10, ...MONO }}>Command Center Expert</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1">
      ...
      {!minimized && (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ background:"#FAF5FF" }}>
            {messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.role==="user"?"justify-end":"justify-start"}`}>
                <div style={{ maxWidth:"88%", background:msg.role==="user"?"linear-gradient(135deg,#9F7AEA,#7C3AED)":"#FFFFFF", color:msg.role==="user"?"#fff":undefined, borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", padding:"10px 14px", boxShadow:msg.role==="agent"?"0 2px 8px rgba(159,122,234,0.1)":undefined, border:msg.role==="agent"?"1px solid rgba(159,122,234,0.15)":undefined }}>
            ...
            {typing && <div style={{ background:"#fff", borderRadius:16, border:"1px solid rgba(159,122,234,0.15)", width:"fit-content" }}><TypingIndicator/></div>}
            <div ref={endRef}/>
          </div>

          {/* Suggestions */}
          <div className="px-3 py-2 flex gap-2 overflow-x-auto flex-shrink-0" style={{ borderTop:"1px solid rgba(159,122,234,0.1)" }}>
            {ADMIN_SUGGESTIONS.map(s => (
              <button key={s} onClick={()=>send(s)}
                className="text-xs px-3 py-1.5 rounded-full whitespace-nowrap flex-shrink-0"
                style={{ background:"rgba(159,122,234,0.08)", color:"#9F7AEA", border:"1px solid rgba(159,122,234,0.2)" }}>
                {s}
              </button>
            ))}
          </div>
          ...
          {/* Input */}
          <div className="flex gap-2 p-3 flex-shrink-0" style={{ borderTop:"1px solid rgba(159,122,234,0.1)", background:"#fff" }}>
            <input value={input} onChange={e=>setInput(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&send(input)}
              placeholder="Ask anything about the Command Center…"
              style={{ flex:1, border:"1px solid rgba(159,122,234,0.25)", borderRadius:12, padding:"8px 14px", fontSize:13, outline:"none", background:"rgba(159,122,234,0.03)" }}/>
            <button onClick={()=>send(input)} disabled={!input.trim()}
              className="flex items-center justify-center rounded-xl flex-shrink-0"
              style={{ width:38, height:38, background:input.trim()?"linear-gradient(135deg,#9F7AEA,#7C3AED)":"rgba(159,122,234,0.15)", color:input.trim()?"#fff":"#C4B5FD" }}>
              <Send size={16}/>
            </button>
          </div>
        </>
