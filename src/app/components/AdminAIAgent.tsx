...
const ADMIN_KB: Record<string, string> = {
  ...
  // ── Analytics ────────────────────────────────────────────────────
  "analytics": `The **Analytics** tab is laid out for demographic and behavioral insights, but **most of it has no data source yet**:\n\n**Not collected today:** gender, date of birth, relationship status, address/country, device type, acquisition source, sessions/DAU, feature usage, retention cohorts, and NPS. The users table has no columns for these and no events table exists.\n\nEach chart shows a "not collected" notice explaining what is missing and what would populate it.\n\n**What is real:** total users and plan mix, which come from the live database.\n\nTo access: Command Center → **Analytics** tab.`,
  ...
  // ── Reports & Downloads ──────────────────────────────────────────
  "reports": `The **Reports & Downloads Tab** exports live data straight from the database:\n\n**Reports with live data:** Revenue Trend · Payout History · $199 Continuation Fee Log · Full User Roster · New Signups · Storage Usage by User · ID Verification · Affiliate Performance · Partner Activity · Audit Log Export\n\n**Marked "No data source":** Overage Billing, White Glove Billing, Churn, Manually Onboarded, Storage Alerts, WG Client Status, GDPR Export, Push Notification and Email Delivery reports — each card says what is missing.\n\n**Formats:** CSV / JSON\n\n**Date ranges:** Last 7 / 30 / 90 days, Year to date, All time (on reports that have a date column)\n\n**This Session's Exports:** Files you generated since the page loaded, with their real size and row count. Nothing is stored on the server.\n\nTo access: Command Center → **Reports & Downloads** tab.`,
