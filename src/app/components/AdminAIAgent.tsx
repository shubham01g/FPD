...
export function AdminAIAgent({ inline = false }: { inline?: boolean }) {
  ...
  const wrapStyle: React.CSSProperties = inline
    ? { display:"flex", flexDirection:"column", height:"calc(100vh - 120px)", borderRadius:16, overflow:"hidden", border:"1px solid rgba(91,167,214,0.25)", background:"#FFFFFF", boxShadow:"0 4px 20px rgba(91,167,214,0.1)" }
    : { position:"fixed", bottom:24, left:24, zIndex:50, display:"flex", flexDirection:"column", borderRadius:16, overflow:"hidden", boxShadow:"0 20px 60px rgba(91,167,214,0.3)", width:390, height:minimized?"auto":580, background:"#FFFFFF", border:"1px solid rgba(91,167,214,0.25)" };
    ...
    <div style={wrapStyle}>
      ...
      {!minimized && (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ background:"#FAF5FF" }}>
            {messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.role==="user"?"justify-end":"justify-start"}`}>
                <div style={{ maxWidth:"88%", background:msg.role==="user"?"linear-gradient(135deg,#5BA7D6,#5B6EE1)":"#FFFFFF", color:msg.role==="user"?"#fff":undefined, borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", padding:"10px 14px", boxShadow:msg.role==="agent"?"0 2px 8px rgba(91,167,214,0.1)":undefined, border:msg.role==="agent"?"1px solid rgba(91,167,214,0.15)":undefined }}>
            ...
            {typing && <div style={{ background:"#fff", borderRadius:16, border:"1px solid rgba(91,167,214,0.15)", width:"fit-content" }}><TypingIndicator/></div>}
            <div ref={endRef}/>
          </div>
          ...
          {/* Input */}
          <div className="flex gap-2 p-3 flex-shrink-0" style={{ borderTop:"1px solid rgba(91,167,214,0.1)", background:"#fff" }}>
