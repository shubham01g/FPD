  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div style={{ color: MUTED, fontSize: 12.5, fontFamily: "var(--font-mono)" }}>LIVE PLATFORM PREVIEW</div>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${pc}30`, boxShadow: `0 0 40px ${pc}10`, width: previewTab === "mobile" ? 380 : "100%", margin: "0 auto" }}>
        ...
        <div style={{ background: "#EAF0FC", borderBottom: `1px solid ${pc}20`, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 28, height: 28, borderRadius: 7, background: `linear-gradient(135deg,${pc},${ac})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12.5, fontWeight: 700, color: "#F0F4FA" }}>{config.logoText}</div>
            {previewTab === "desktop" && (
              <div>
                <div style={{ color: pc, fontSize: 11, fontWeight: 700, letterSpacing: "0.06em" }}>{config.companyName.toUpperCase()}</div>
                <div style={{ color: "#8A9AB8", fontSize: 9, letterSpacing: "0.1em" }}>{config.tagline}</div>
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {previewTab === "desktop" && <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#48BB78", boxShadow: `0 0 6px #48BB78` }} />}
            <div style={{ width: 24, height: 24, borderRadius: "50%", background: `${pc}20`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: pc, fontWeight: 700 }}>JD</div>
          </div>
        </div>
          ...
          {previewTab === "desktop" && (
            <div style={{ background: "#EAF0FC", borderRadius: 12, padding: 10, border: `1px solid ${pc}15` }}>
              {[
                ...
                <div key={i} style={{ padding: "6px 8px", borderRadius: 8, marginBottom: 2, background: item.active ? `${pc}18` : "transparent", borderLeft: item.active ? `2px solid ${pc}` : "2px solid transparent", color: item.active ? "#0D1428" : "#8A9AB8", fontSize: 12.5 }}>
                  {item.label}
                </div>
              ))}
              <div style={{ marginTop: 16, padding: "8px", borderRadius: 8, background: `${pc}10`, border: `1px solid ${pc}25`, fontSize: 11, color: pc, fontWeight: 700, letterSpacing: "0.06em" }}>⚙ ADMIN PORTAL</div>
            </div>
          )}

          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ background: `linear-gradient(135deg,${pc}18,${ac}08)`, borderRadius: 12, padding: 14, border: `1px solid ${pc}20` }}>
              <div style={{ fontSize: 11, color: "#5A6A88", marginBottom: 4 }}>WELCOME BACK</div>
              <div style={{ color: "#0D1428", fontSize: 17, fontWeight: 700 }}>James Doe</div>
              <div style={{ color: pc, fontSize: 10, marginTop: 2 }}>{config.companyName} · {config.planNames.premium}</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {["14 Documents", "3 Contacts", "16.9 GB Used", "$284 Earned"].map((stat, i) => (
                <div key={i} style={{ background: "#FFFFFF", borderRadius: 10, padding: 10, border: `1px solid ${pc}12` }}>
                  <div style={{ color: [pc, "#D99A6B", "#F6AD55", "#6FAE8B"][i], fontSize: 15.5, fontWeight: 700 }}>{stat.split(" ")[0]}</div>
                  <div style={{ color: "#5A6A88", fontSize: 11 }}>{stat.split(" ").slice(1).join(" ")}</div>
                </div>
              ))}
            </div>
            <div style={{ background: "#FFFFFF", borderRadius: 10, padding: 12, border: `1px solid ${pc}12` }}>
              <div style={{ color: "#5A6A88", fontSize: 10, marginBottom: 8 }}>STORAGE USAGE</div>
            ...
            <div style={{ textAlign: "center", padding: "10px", borderRadius: 10, background: `linear-gradient(135deg,${pc},${ac})`, cursor: "pointer", fontSize: 13.5, fontWeight: 700, color: "#F0F4FA" }}>
              Upload Document →
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#EAF0FC", borderTop: `1px solid ${pc}12`, padding: "8px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ color: "#8A9AB8", fontSize: 10 }}>{config.footerText}</div>
          <div style={{ color: pc, fontSize: 10, fontFamily: "var(--font-mono)" }}>Powered by {config.companyName}</div>
        </div>
      </div>
    </div>
  );
...
.fpd-wl .eyebrow{font-size:11px;font-weight:600;color:${MUTED};display:flex;align-items:center;gap:7px;}
.fpd-wl .pg-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;}
.fpd-wl .pg-h1{font-size:27px;color:${TEXT};font-weight:600;margin-bottom:4px;letter-spacing:-0.02em;font-family:var(--font-display);}
.fpd-wl .pg-sub{color:${MUTED};font-size:14.5px;}
...
.fpd-wl .enable-pill span{color:${MUTED};font-size:13.5px;}
.fpd-wl .btn-ghost{display:inline-flex;align-items:center;gap:6px;padding:9px 15px;border-radius:16px;font-size:14.5px;cursor:pointer;font-family:var(--font-body);border:1px solid;background:none;}
.fpd-wl .btn-primary{display:inline-flex;align-items:center;gap:6px;padding:9px 20px;border-radius:16px;font-size:14.5px;font-weight:700;color:#fff;border:none;cursor:pointer;background:linear-gradient(180deg,#7E6BD8,#5B6EE1);box-shadow:0 6px 18px -8px rgba(91,110,225,0.7);font-family:var(--font-body);}
...
.fpd-wl .status-banner span{color:${SOFT};font-size:14.5px;}
...
.fpd-wl-seg button{display:inline-flex;align-items:center;gap:6px;padding:9px 15px;border-radius:99px;font-size:14.5px;color:${MUTED};background:none;border:none;cursor:pointer;font-family:var(--font-body);}
...
/* fields */
.fpd-wl .field label{display:block;margin-bottom:6px;font-size:10.5px;font-weight:600;color:${MUTED};}
.fpd-wl .field input{width:100%;padding:11px 14px;border-radius:16px;background:#0F1624;border:1px solid rgba(255,255,255,0.08);color:${TEXT};font-size:15.5px;outline:none;font-family:var(--font-body);}
...
.fpd-wl .sec-title{font-family:var(--font-display);font-size:17px;color:${TEXT};margin-bottom:4px;}

/* presets */
.fpd-wl .preset-lbl{color:${MUTED};font-size:12.5px;font-family:var(--font-mono);margin-bottom:12px;}
.fpd-wl .brand-preset{padding:16px;border-radius:18px;text-align:left;cursor:pointer;font-family:var(--font-body);}
.fpd-wl .brand-preset-logo{width:28px;height:28px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;}
.fpd-wl .brand-preset-name{color:${TEXT};font-size:13.5px;font-weight:600;}
.fpd-wl .brand-preset-tag{color:${MUTED};font-size:12.5px;}
...
.fpd-wl .color-preset span{color:#F0F4FA;font-size:10px;font-weight:700;}
...
.fpd-wl .upload-box span{color:#6FAE8B;font-size:14.5px;}
...
.fpd-wl .info-strip p{color:${SOFT};font-size:14.5px;}
.fpd-wl .feature-row{display:flex;align-items:center;justify-content:space-between;padding:16px;}
.fpd-wl .feature-row-label{color:${TEXT};font-size:15.5px;font-weight:500;}
.fpd-wl .feature-row-desc{color:${MUTED};font-size:13.5px;margin-top:2px;}
.fpd-wl .feature-count{color:${MUTED};font-size:13.5px;}

/* plan cards */
.fpd-wl .plan-card{padding:22px;border-radius:22px;border:1px solid;background:linear-gradient(180deg,#0D1421 0%,#0A0F1A 100%);}
.fpd-wl .plan-badge{font-size:11px;font-family:var(--font-mono);margin-bottom:8px;}
.fpd-wl .plan-input{width:100%;padding:11px 14px;border-radius:16px;margin-bottom:10px;outline:none;font-family:var(--font-display);font-size:19px;font-weight:700;color:${TEXT};}
.fpd-wl .plan-displayed{font-size:12.5px;font-family:var(--font-mono);}
.fpd-wl .plan-example{color:${MUTED};font-size:12.5px;margin-top:4px;}
.fpd-wl .plan-tag{padding:8px 14px;border-radius:16px;font-size:14.5px;}
...
.fpd-wl .dns-box .dl{color:${MUTED};font-size:12.5px;font-family:var(--font-mono);margin-bottom:4px;}
.fpd-wl .dns-box .dv{color:#D99A6B;font-size:13.5px;font-family:var(--font-mono);}
...
.fpd-wl .warn-box{padding:12px 14px;border-radius:16px;background:rgba(217,165,94,0.08);border:1px solid rgba(217,165,94,0.24);color:${WARN};font-size:13.5px;}

/* preview tab summary */
.fpd-wl .summary-tile{padding:16px;}
.fpd-wl .summary-lbl{color:${MUTED};font-size:12.5px;font-family:var(--font-mono);margin-bottom:4px;}
.fpd-wl .summary-val{font-size:14.5px;font-weight:600;word-break:break-all;}
`;
      ...
      <div className="wrap fpd-fade-in-up">
        ...
        {config.enabled && (
          <div className="status-banner">
            ...
            <button className="btn-ghost" style={{ marginLeft: "auto", color: "#6FAE8B", background: "rgba(91,110,225,0.14)", borderColor: "rgba(91,110,225,0.3)", padding: "6px 12px", fontSize: 12.5 }}>
              ...
              <div className="card pad space-y-4">
                ...
                <div className="swatch-preview">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <div style={{ width: 48, height: 32, borderRadius: 8, background: config.primaryColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: "#F0F4FA", fontWeight: 700 }}>Primary</div>
                    <div style={{ width: 48, height: 32, borderRadius: 8, background: config.accentColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: "#F0F4FA", fontWeight: 700 }}>Accent</div>
                    <button style={{ padding: "6px 14px", borderRadius: 8, background: `linear-gradient(135deg,${config.primaryColor},${config.accentColor})`, color: "#F0F4FA", fontSize: 12.5, fontWeight: 700, border: "none" }}>CTA Button</button>
                    <div style={{ padding: "6px 14px", borderRadius: 8, background: `${config.primaryColor}22`, color: config.primaryColor, fontSize: 12.5, border: `1px solid ${config.primaryColor}55` }}>Badge</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div className="card pad space-y-4">
              ...
              <div className="email-preview-box">
                ...
                <div style={{ color: TEXT, fontSize: 14.5 }}>From: <strong>{config.senderName}</strong> &lt;{config.supportEmail}&gt;</div>
                <div style={{ color: MUTED, fontSize: 13.5, marginTop: 4 }}>Subject: Welcome to {config.companyName} — Your Legacy Begins Today</div>
              </div>
