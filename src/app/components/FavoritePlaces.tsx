...
const CAT_META: Record<string, { emoji:string; color:string; bg:string }> = {
  ...
  "Museum / Art":      { emoji:"🏛️", color:"#2040C0", bg:"rgba(32,64,192,0.1)" },
  ...
  "Religious / Church":{ emoji:"⛪",  color:"#2040C0", bg:"rgba(32,64,192,0.1)" },
  ...
  "Family Home":       { emoji:"🏡",  color:"#2040C0", bg:"rgba(32,64,192,0.1)" },
};

function getMeta(cat:string) { return CAT_META[cat] || { emoji:"📍", color:"#8A9AB8", bg:"rgba(138,154,184,0.1)" }; }
          ...
          {allCats.map(cat => {
            ...
            return (
              <button key={cat} onClick={() => setFilterCat(cat)}
                ...
                style={{ background:filterCat===cat?meta.bg:"rgba(32,64,192,0.04)", color:filterCat===cat?meta.color:"var(--muted-foreground)", border:`1px solid ${filterCat===cat?meta.color+"40":"rgba(32,64,192,0.1)"}` }}>
            ...
            <div key={place.id} className="rounded-2xl overflow-hidden" style={CARD}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap gap-1">
                  {place.tags.map(tag => (
                    <span key={tag} className="px-1.5 py-0.5 rounded text-xs" style={{ background:"rgba(32,64,192,0.06)", color:"var(--primary)" }}>#{tag}</span>
                  ))}
                </div>
              ...
              <div className="flex gap-2 mt-3">
                {place.phone && <button onClick={() => toast.success(`Calling ${place.name}: ${place.phone}`)} className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg" style={{ background:"rgba(32,64,192,0.06)", color:"var(--primary)" }}><Phone size={10}/>{place.phone}</button>}
                {place.website && <button onClick={() => toast.success(`Opening: ${place.website}`)} className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg" style={{ background:"rgba(32,64,192,0.06)", color:"var(--primary)" }}><Globe size={10}/>{place.website}</button>}
              </div>
              </div>
            </div>
          );
