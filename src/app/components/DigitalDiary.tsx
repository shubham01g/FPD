...
const CARD: React.CSSProperties = {
  background: "#FFFFFF",
  border: "1px solid rgba(32,64,192,0.1)",
...
const moodConfig: Record<Mood, { icon: React.ReactNode; label: string; color: string; bg: string }> = {
  ...
  sad:       { icon: <Cloud size={16}/>,    label: "Sad",       color: "#9F7AEA", bg: "#FAF5FF" },
...
const TAGS_PALETTE = ["#2040C0","#48BB78","#9F7AEA","#F6AD55","#FC8181","#38B2AC","#ED8936","#E53E3E"];

function TypeBadge({ type, color }: { type: EntryType; color?: string }) {
  const c = color ?? "#2040C0";
    ...
    <div className="flex flex-col items-center gap-5 py-8">
      <div className="relative">
        <div className="w-24 h-24 rounded-full flex items-center justify-center"
          style={{ background: recording ? "rgba(229,62,62,0.1)" : "rgba(32,64,192,0.1)", border: `3px solid ${recording?"#E53E3E":"#2040C0"}`, transition:"all 0.3s" }}>
          {recording
            ? <Square size={28} color="#E53E3E" fill="#E53E3E"/>
            : <Mic size={28} color="#2040C0"/>}
        </div>
        {recording && (
          <div className="absolute inset-0 rounded-full animate-ping" style={{ background:"rgba(229,62,62,0.15)" }}/>
        )}
      </div>
      <div style={{ fontFamily:"var(--font-mono)", fontSize:24, color:"#0D1428", fontWeight:700 }}>
      ...
      <button onClick={recording ? stop : start}
        ...
        style={{ background: recording ? "linear-gradient(135deg,#E53E3E,#FC8181)" : "linear-gradient(135deg,#2040C0,#3355E0)", color:"#fff", boxShadow:`0 4px 16px ${recording?"rgba(229,62,62,0.35)":"rgba(32,64,192,0.35)"}` }}>
...
function VideoRecorderPanel({ onSave }: { onSave: (duration: string) => void }) {
  ...
  if (!preview) {
    return (
      <div className="flex flex-col items-center gap-5 py-8">
        <div className="w-24 h-24 rounded-full flex items-center justify-center" style={{ background:"rgba(32,64,192,0.1)", border:"3px solid #2040C0" }}>
          <Video size={28} color="#2040C0"/>
        </div>
        <div style={{ color:"#5A6A88", fontSize:13 }}>Start your camera to record a video diary entry</div>
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex gap-3">
        ...
        <button onClick={stopAll} className="px-6 py-2.5 rounded-xl text-sm" style={{ background:"#F0F4FA", color:"#5A6A88" }}>Cancel</button>
      </div>
    </div>
  );
}
    ...
    <div style={{ background:"#F0F4FA", minHeight:"100%", padding:24 }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }} className="space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 style={{ fontFamily:"var(--font-display)", fontSize:26, color:"#0D1428", marginBottom:4 }}>Digital Diary</h1>
            <p style={{ color:"#5A6A88", fontSize:13 }}>{today} · {entries.length} entries · Written, audio & video recordings</p>
          </div>
          ...
          <div className="flex gap-1 p-1 rounded-xl" style={CARD}>
            {[{id:"all",label:"All"},{id:"written",label:"📝"},{id:"audio",label:"🎙️"},{id:"video",label:"🎥"}].map(f => (
              <button key={f.id} onClick={() => setFilterType(f.id as any)}
                ...
                style={{ background:filterType===f.id?"#2040C0":"transparent", color:filterType===f.id?"#fff":"#5A6A88", fontWeight:filterType===f.id?600:400 }}>
                {f.label}
              </button>
            ))}
          </div>
          ...
          <div className="lg:col-span-2 space-y-3">
            {filtered.length === 0 && (
              <div className="py-16 text-center rounded-2xl" style={CARD}>
                <BookOpen size={36} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/>
                <div style={{ color:"#8A9AB8" }}>No entries found. Start writing your story.</div>
              </div>
            )}
            {filtered.map(entry => {
              ...
              return (
                <div key={entry.id}
                  ...
                  style={{ ...CARD, borderColor:selected?.id===entry.id?"#2040C0":"rgba(32,64,192,0.1)", borderWidth:selected?.id===entry.id?2:1 }}
                    ...
                    <div style={{ borderRadius:10, overflow:"hidden", marginBottom:12, height:120, position:"relative" }}>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.3)" }}>
                        <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background:"rgba(255,255,255,0.9)" }}>
                          <Play size={18} color="#0D1428" fill="#0D1428"/>
                        </div>
                      </div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span style={{ color:"#8A9AB8", fontSize:11, ...MONO }}>{entry.date}</span>
                      ...
                      <div style={{ color:"#0D1428", fontSize:15, fontWeight:600 }}>{entry.title}</div>
                    </div>
                    {mood && (
                  ...
                  {entry.body && (
                    <p style={{ color:"#5A6A88", fontSize:13, lineHeight:1.7 }} className="line-clamp-2">{entry.body}</p>
                  )}
                  {entry.type === "audio" && entry.duration && (
                    <div className="flex items-center gap-3 mt-3 px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.06)" }}>
                      <button onClick={e => { e.stopPropagation(); setPlayingAudio(playingAudio===entry.id ? null : entry.id); if (playingAudio !== entry.id) toast.success("▶ Playing audio entry — " + entry.duration); }}
                        className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background:"#2040C0", color:"#fff" }}>
                        {playingAudio===entry.id ? <Pause size={14}/> : <Play size={14}/>}
                      </button>
                      <div style={{ flex:1 }}>
                        <div className="h-1 rounded-full" style={{ background:"rgba(32,64,192,0.2)" }}>
                          <div className="h-1 rounded-full" style={{ width:playingAudio===entry.id?"35%":"0%", background:"#2040C0", transition:"width 0.3s" }}/>
                        </div>
                      </div>
                      <span style={{ color:"#8A9AB8", fontSize:11, ...MONO }}>{entry.duration}</span>
                    </div>
                  )}
                  {entry.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-3">
                      {entry.tags.map(tag => (
                        <span key={tag} className="px-2 py-0.5 rounded-full text-xs" style={{ background:"#EAF0FC", color:"#2040C0" }}>#{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
            ...
            {selected ? (
              <div className="rounded-2xl p-5 sticky top-4" style={CARD}>
                <div className="flex items-center justify-between mb-4">
                  <div style={{ color:"#5A6A88", fontSize:11, ...MONO }}>{selected.date}</div>
                  <div className="flex gap-1">
                    ...
                    }} style={{ color:"#8A9AB8", padding:4 }}><Edit2 size={13}/></button>
                    <button onClick={() => { handleDelete(selected.id); }} style={{ color:"#FC8181", padding:4 }}><Trash2 size={13}/></button>
                    <button onClick={() => setSelected(null)} style={{ color:"#8A9AB8", padding:4 }}><X size={13}/></button>
                  </div>
                </div>
                ...
                <div style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#0D1428", marginBottom:12, fontWeight:600 }}>{selected.title}</div>
                {selected.body && (
                  <div style={{ color:"#374669", fontSize:14, lineHeight:1.9, whiteSpace:"pre-wrap" }}>{selected.body}</div>
                )}
                {selected.type === "audio" && (
                  <div className="mt-4 px-4 py-4 rounded-xl text-center" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.15)" }}>
                    <Mic size={24} color="#2040C0" style={{ margin:"0 auto 8px" }}/>
                    <div style={{ color:"#0D1428", fontSize:13 }}>Voice Recording · {selected.duration}</div>
                    <button onClick={() => { setPlayingAudio(playingAudio===selected.id ? null : selected.id); toast.success(playingAudio===selected.id ? "⏸ Paused" : "▶ Playing audio — " + selected.duration); }} className="mt-3 flex items-center gap-2 mx-auto px-4 py-2 rounded-xl text-sm" style={{ background:"#2040C0", color:"#fff" }}>
                ...
                {selected.type === "video" && (
                  <div className="mt-4 rounded-xl overflow-hidden relative" style={{ cursor:"pointer" }} onClick={() => toast.success("▶ Video playback started — demo mode")}>
                    {selected.thumbnail
                      ...
                      : <div className="h-32 flex items-center justify-center" style={{ background:"rgba(32,64,192,0.08)" }}><Video size={28} color="#2040C0"/></div>
                    }
                    <div className="absolute inset-0 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.25)" }}>
                      <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background:"rgba(255,255,255,0.9)" }}>
                        <Play size={20} color="#0D1428" fill="#0D1428"/>
                      </div>
                    </div>
                  </div>
                )}
                {selected.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-4">
                    {selected.tags.map(t => <span key={t} className="px-2 py-0.5 rounded-full text-xs" style={{ background:"#EAF0FC", color:"#2040C0" }}>#{t}</span>)}
                  </div>
                )}
              </div>
            ) : (
              <div className="rounded-2xl p-6 text-center" style={CARD}>
                <BookOpen size={28} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/>
                <div style={{ color:"#8A9AB8", fontSize:13, marginBottom:12 }}>Select an entry to read</div>
                <div className="space-y-2">
                  <div style={{ color:"#0D1428", fontSize:13, fontWeight:600 }}>Your Diary Stats</div>
                  {[
                    ...
                    <div key={s.label} className="flex justify-between px-3 py-2 rounded-lg" style={{ background:"#F5F8FE" }}>
                      <span style={{ color:"#5A6A88", fontSize:12 }}>{s.label}</span>
                      <span style={{ color:"#2040C0", fontSize:12, fontWeight:600 }}>{s.value}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.4)", backdropFilter:"blur(8px)" }}>
          <div className="w-full max-w-2xl rounded-2xl overflow-hidden" style={CARD}>
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor:"rgba(32,64,192,0.08)" }}>
              <h3 style={{ fontFamily:"var(--font-display)", fontSize:18, color:"#0D1428" }}>New Diary Entry</h3>
              <button onClick={() => setCreating(false)} style={{ color:"#8A9AB8" }}><X size={16}/></button>
            </div>
            <div className="p-6 space-y-4 overflow-y-auto" style={{ maxHeight:"80vh" }}>
              ...
              <div className="grid grid-cols-4 gap-2">
                {typeButtons.map(tb => (
                  <button key={tb.id} onClick={() => setNewType(tb.id)}
                    ...
                    style={{ background:newType===tb.id?"rgba(32,64,192,0.12)":"#F5F8FE", border:`2px solid ${newType===tb.id?"#2040C0":"transparent"}`, color:newType===tb.id?"#2040C0":"#5A6A88" }}>
                    {tb.icon}
              ...
              {/* Title */}
              <div>
                <label style={{ color:"#8A9AB8", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>TITLE</label>
                <input value={form.title} onChange={e => setForm(p=>({...p,title:e.target.value}))} placeholder="What's on your mind today?"
                  className="w-full px-4 py-3 rounded-xl" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.12)", color:"#0D1428", fontSize:15, outline:"none", fontFamily:"var(--font-display)" }}/>
              </div>

              {/* Written body */}
              {newType === "written" && (
                <div>
                  <label style={{ color:"#8A9AB8", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>YOUR ENTRY</label>
                  <textarea value={form.body} onChange={e => setForm(p=>({...p,body:e.target.value}))} placeholder="Write freely. This is your space…" rows={8}
                    className="w-full px-4 py-3 rounded-xl resize-none" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.12)", color:"#0D1428", fontSize:14, outline:"none", lineHeight:1.8 }}/>
                </div>
              )}

              {/* Audio recorder */}
              {newType === "audio" && (
                <div className="rounded-2xl" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.12)" }}>
                  {audioSaved
              ...
              {/* Video recorder */}
              {newType === "video" && (
                <div className="rounded-2xl p-4" style={{ background:"#F5F8FE", border:"1px solid rgba(32,64,192,0.12)" }}>
                  {videoDuration
              ...
              {newType === "photo" && (
                <div>
                  ...
                  <div onClick={() => fileRef.current?.click()}
                    ...
                    style={{ borderColor:"rgba(32,64,192,0.25)", background:"#F5F8FE" }}>
                    <Upload size={28} color="#2040C0" style={{ opacity:0.6 }}/>
                    <div style={{ color:"#5A6A88", fontSize:13 }}>Upload video, photo, or audio file</div>
                    <div style={{ color:"#8A9AB8", fontSize:11 }}>MP4, MOV, JPG, PNG, MP3, WAV</div>
                  </div>
              ...
              {/* Mood */}
              <div>
                <label style={{ color:"#8A9AB8", fontSize:11, ...MONO, display:"block", marginBottom:6 }}>HOW ARE YOU FEELING?</label>
                <div className="flex gap-2">
                  {(Object.entries(moodConfig) as [Mood, typeof moodConfig[Mood]][]).map(([k, cfg]) => (
                    <button key={k} onClick={() => setForm(p=>({...p,mood:k}))}
                      ...
                      style={{ background:form.mood===k?cfg.bg:"#F5F8FE", border:`2px solid ${form.mood===k?cfg.color:"transparent"}`, color:cfg.color }}>
                      {cfg.icon}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label style={{ color:"#8A9AB8", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>TAGS (comma separated)</label>
                ...
                <div>
                  <label style={{ color:"#8A9AB8", fontSize:11, ...MONO, display:"block", marginBottom:4 }}>PRIVACY</label>
                  <button onClick={() => setForm(p=>({...p,private:!p.private}))}
                    className="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm"
                    style={{ background:form.private?"rgba(229,62,62,0.08)":"#F5F8FE", border:`1px solid ${form.private?"rgba(229,62,62,0.25)":"rgba(32,64,192,0.12)"}`, color:form.private?"#E53E3E":"#5A6A88" }}>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={() => setCreating(false)} className="px-6 py-3 rounded-xl text-sm" style={{ background:"#F0F4FA", color:"#5A6A88" }}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
