...
const FOLDER_CATALOG = [
  ...
  { id:"insurance", label:"Insurance Policies",      emoji:"🛡️",  color:"#9F7AEA" },
...
function ContactSection({
  ...
  getVerifStatus, advanceStatus, simulateVerify, verifying,
}: {
  type: ContactType;
  contacts: Contact[];
  onAdd: () => void;
        ...
        <div className="px-5 py-3 border-b" style={{ background:"rgba(108,92,231,0.04)", borderColor:"rgba(108,92,231,0.1)" }}>
          <div className="flex items-start gap-2">
            ...
            <div style={{ color:"rgba(255,255,255,0.7)", fontSize:11, lineHeight:1.6 }}>
              ...
              {contacts.length > 1 && " This order is legally binding."}
            </div>
          </div>
        </div>
      )}
                      ...
                      <div className="mt-3 space-y-2">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {(vs === "not_sent" || vs === "pending") && (
                            <ScanButton folder="legal" onUpload={doc => { setVerifStatus(p=>({...p,[c.id]:"id_submitted"})); toast.success(`ID scanned for ${c.name}`); }} size="sm" label="Scan ID"/>
                          )}
                        </div>
                      </div>
                    );
