...
function brandColor(brand: string): string {
  return { Visa:"#1A1F71", Mastercard:"#EB001B", Amex:"#007BC1", Discover:"#FF6600" }[brand] ?? "#2040C0";
}
    ...
    <div className="rounded-2xl overflow-hidden" style={{ border:"1px solid rgba(32,64,192,0.12)" }}>
      <div className="flex items-center justify-between px-4 py-3 border-b"
        ...
        <div className="flex items-center gap-2">
          <CreditCard size={14} color="#2040C0"/>
          <span style={{ color:"#2040C0", fontSize:12, fontWeight:700, ...MONO }}>PAYMENT METHOD ON FILE</span>
        </div>
        {hasCard && !editing && (
      ...
      {hasCard && !editing && (
        <div className="p-4 space-y-3">
          <div className="flex items-center gap-4 px-4 py-4 rounded-xl"
            style={{ background:"linear-gradient(135deg,#0A1428,#152040)", border:"1px solid rgba(32,64,192,0.3)" }}>
          ...
          <div className="flex gap-3">
            <button onClick={saveCard} disabled={saving}
              ...
              style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA",
                boxShadow:"0 0 20px rgba(32,64,192,0.3)", opacity:saving?0.7:1 }}>
