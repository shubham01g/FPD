  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center"
      ...
      <div className="w-full max-w-2xl mx-4 rounded-3xl overflow-hidden flex flex-col"
        style={{ background:"#0A0F1E", border:"1px solid rgba(108,92,231,0.2)", maxHeight:"96vh" }}>

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b"
          style={{ borderColor:"rgba(108,92,231,0.2)" }}>
          <div className="flex items-center gap-3">
            <div className="rounded-xl p-2" style={{ background:"rgba(108,92,231,0.15)" }}>
              <Scan size={18} color="#A29BFE"/>
            </div>
            <div>
              ...
              {cameraError ? (
                ...
                <>
                  ...
                  <div className="absolute inset-6 pointer-events-none"
                    style={{ border:"2px solid rgba(108,92,231,0.7)", borderRadius:8, boxShadow:"0 0 0 4000px rgba(0,0,0,0.35)" }}>
                    {/* Corner marks */}
                    ...
                    {/* Scan line animation */}
                    <div style={{ position:"absolute", left:0, right:0, height:2, background:"linear-gradient(90deg,transparent,rgba(108,92,231,0.8),transparent)", animation:"scanLine 2s linear infinite", top:"50%" }}/>
                  </div>
                  {/* Flash overlay */}
            ...
            {!cameraError && (
              <div className="flex items-center justify-between px-6 py-4" style={{ background:"#060D1A" }}>
                ...
                <button onClick={capture}
                  ...
                  <div style={{ width:56, height:56, borderRadius:"50%", background:"#6C5CE7" }}/>
                </button>
          ...
          <div className="flex flex-col" style={{ flex:1, minHeight:0, overflowY:"auto" }}>
              ...
              <div className="grid grid-cols-4 gap-2">
                {(Object.entries(SCAN_FILTERS) as [ScanMode, typeof SCAN_FILTERS[ScanMode]][]).map(([id, cfg]) => (
                  <button key={id} onClick={() => applyFilter(id)}
                    ...
                    style={{ background:scanMode===id?"rgba(108,92,231,0.25)":"rgba(255,255,255,0.04)",
                      border:`1px solid ${scanMode===id?"#6C5CE7":"rgba(255,255,255,0.08)"}` }}>
                    <span style={{ fontSize:10, fontWeight:700, color:scanMode===id?"#A29BFE":"#4A5A7A" }}>{cfg.label}</span>
            ...
            <div className="px-5 py-4 space-y-3 border-t" style={{ borderColor:"rgba(255,255,255,0.05)" }}>
              <div>
                ...
                <input value={docName} onChange={e => setDocName(e.target.value)}
                  ...
                  style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(108,92,231,0.3)", color:"#E8EDF5", fontSize:14, outline:"none" }}/>
              </div>
              <div>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {FOLDERS.map(f => (
                    <button key={f.id} onClick={() => setDocFolder(f.id)}
                      ...
                      style={{ background:docFolder===f.id?"rgba(108,92,231,0.2)":"rgba(255,255,255,0.04)",
                        border:`1px solid ${docFolder===f.id?"rgba(108,92,231,0.5)":"rgba(255,255,255,0.06)"}` }}>
                      <span style={{ fontSize:14 }}>{f.emoji}</span>
                      <span style={{ fontSize:8, color:docFolder===f.id?"#A29BFE":"#4A5A7A", lineHeight:1.2 }}>{f.label.split(" ")[0]}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex gap-3 px-5 py-4 border-t" style={{ borderColor:"rgba(255,255,255,0.05)" }}>
              ...
              <button onClick={confirm} disabled={uploading || !docName.trim()}
                ...
                style={{ background:"linear-gradient(135deg,#6C5CE7,#8B7CF6)", color:"#F0F4FA",
                  boxShadow:"0 0 24px rgba(108,92,231,0.4)", opacity:uploading||!docName.trim()?0.6:1 }}>
                {uploading ? (
  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        ...
        style={{ ...styles[size], background:"rgba(108,92,231,0.08)", color:"#6C5CE7", border:"1px solid rgba(108,92,231,0.2)" }}>
