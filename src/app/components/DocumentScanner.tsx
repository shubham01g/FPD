      ...
      <div className="w-full max-w-2xl mx-0 sm:mx-4 rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div style={{ color:"#E8EDF5", fontSize:19, fontWeight:600 }}>Document Scanner</div>
              <div style={{ color:"#4A5A7A", fontSize:14, fontFamily:"var(--font-mono)" }}>
            ...
            <div className="relative flex-1 bg-black" style={{ minHeight:280 }}>
              {cameraError ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 p-6 text-center">
                  ...
                  <div style={{ color:"#FC8181", fontSize:17.5, lineHeight:1.7 }}>{cameraError}</div>
            ...
            {!cameraError && (
              <div className="flex items-center justify-between px-6 py-4" style={{ background:"#060D1A" }}>
                <button onClick={() => fileRef.current?.click()}
                  ...
                  style={{ color:"#4A5A7A" }}>
                  <Upload size={20}/>
                  <span style={{ fontSize:11, fontFamily:"var(--font-mono)" }}>UPLOAD</span>
                </button>
                <input ref={fileRef} type="file" accept="image/*,application/pdf" className="hidden" onChange={handleFileSelect}/>
                ...
                <button onClick={() => setFacing(f => f === "user" ? "environment" : "user")}
                  ...
                  <span style={{ fontSize:11, fontFamily:"var(--font-mono)" }}>FLIP</span>
                </button>
              </div>
            )}
          </div>
        ...
        {phase === "preview" && capturedUrl && (
          <div className="flex flex-col" style={{ flex:1, minHeight:0, overflowY:"auto" }}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {(Object.entries(SCAN_FILTERS) as [ScanMode, typeof SCAN_FILTERS[ScanMode]][]).map(([id, cfg]) => (
                  <button key={id} onClick={() => applyFilter(id)}
                    ...
                    <span style={{ fontSize:12.5, fontWeight:700, color:scanMode===id?"#6FAE8B":"#4A5A7A" }}>{cfg.label}</span>
                    <span style={{ fontSize:10, color:"#2A3A5A", textAlign:"center", lineHeight:1.3 }}>{cfg.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="px-5 py-4 space-y-3 border-t" style={{ borderColor:"rgba(255,255,255,0.05)" }}>
              <div>
                <label style={{ color:"#4A5A7A", fontSize:12.5, fontFamily:"var(--font-mono)", display:"block", marginBottom:6 }}>DOCUMENT NAME *</label>
                <input value={docName} onChange={e => setDocName(e.target.value)}
                  placeholder="e.g. Last Will and Testament"
                  className="w-full px-4 py-3 rounded-xl"
                  style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(91,110,225,0.3)", color:"#E8EDF5", fontSize:17.5, outline:"none" }}/>
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                  {FOLDERS.map(f => (
                    <button key={f.id} onClick={() => setDocFolder(f.id)}
                      ...
                      <span style={{ fontSize:17.5 }}>{f.emoji}</span>
                      <span style={{ fontSize:10, color:docFolder===f.id?"#6FAE8B":"#4A5A7A", lineHeight:1.2 }}>{f.label.split(" ")[0]}</span>
                    </button>
                  ))}
                </div>
              </div>
...
export function ScanButton({
  ...
  const styles: Record<string, React.CSSProperties> = {
    sm: { padding:"6px 12px", fontSize:14, borderRadius:10, gap:5 },
    md: { padding:"8px 16px", fontSize:16, borderRadius:12, gap:7 },
    lg: { padding:"12px 24px", fontSize:19, borderRadius:14, gap:9 },
  };
