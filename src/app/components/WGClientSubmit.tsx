...
export function WGClientSubmit({ token = "TOKEN_JAMES_003" }: { token?: string }) {
  ...
  return (
    <div className="min-h-screen" style={{ background:"#F8F4FF", fontFamily:"var(--font-body)" }}>

      {/* Header */}
      <div className="px-5 py-4 border-b"
        style={{ background:"#fff", borderColor:"rgba(159,122,234,0.15)", boxShadow:"0 1px 12px rgba(159,122,234,0.08)" }}>
        <div className="max-w-lg mx-auto flex items-center gap-3">
          <img src={fpdSquareLogo} alt="FPD" style={{ width:36, height:36, borderRadius:8, objectFit:"cover" }}/>
          <div>
            <div style={{ fontFamily:"var(--font-display)", color:"#9F7AEA", fontSize:11, fontWeight:700, letterSpacing:"0.06em" }}>
              FINAL PASS DOWN
            </div>
      ...
      <div className="max-w-lg mx-auto px-5 py-8 space-y-5">
        ...
        <div className="p-5 rounded-2xl glow-surface"
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span style={{ color:"#C4B5FD", fontSize:12, fontWeight:700, fontFamily:"var(--font-mono)" }}>
          ...
          <p style={{ color:"#8AA3C8", fontSize:13, lineHeight:1.7 }}>
            Your specialist <strong style={{ color:"#C4B5FD" }}>{specialistName}</strong> is ready to build your vault. Use this page to send your documents — take a photo with your phone or upload a file. It's just like sending a photo in a text message.
          </p>
        ...
        {/* Tab switcher */}
        <div className="flex gap-1 p-1 rounded-xl" style={{ background:"rgba(255,255,255,0.9)", border:"1px solid rgba(159,122,234,0.15)" }}>
          <button onClick={() => setView("submit")}
            className="flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all"
            style={{ background:view==="submit"?"#9F7AEA":"transparent", color:view==="submit"?"#fff":"#5A6A88" }}>
            📤 Send a Document
          </button>
          <button onClick={() => setView("status")}
            className="flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all"
            style={{ background:view==="status"?"#9F7AEA":"transparent", color:view==="status"?"#fff":"#5A6A88" }}>
        ...
        {view === "submit" && (
          <div className="space-y-4">
              ...
              {!selectedFile ? (
                <div className="space-y-3">
                  ...
                  <button onClick={() => fileRef.current?.click()}
                    ...
                    style={{ background:"rgba(159,122,234,0.06)", color:"#9F7AEA", border:"2px dashed rgba(159,122,234,0.3)" }}>
            ...
            <div className="p-5 rounded-2xl glow-surface" style={{ background:"#fff", border:"1px solid rgba(159,122,234,0.12)" }}>
              ...
              {showCategoryPicker && (
                <div className="mt-2 rounded-xl overflow-hidden" style={{ border:"1px solid rgba(159,122,234,0.2)" }}>
                  {DOCUMENT_CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => { setCategory(cat.id); setShowCategoryPicker(false); }}
                      ...
                      style={{ background:category===cat.id?"rgba(159,122,234,0.08)":"#fff",
                        borderColor:"rgba(159,122,234,0.08)", color:"#0D1428" }}>
                      <span style={{ fontSize:20 }}>{cat.emoji}</span>
                      <span style={{ fontSize:14 }}>{cat.label}</span>
                      {category===cat.id && <CheckCircle size={14} color="#9F7AEA" style={{ marginLeft:"auto" }}/>}
                    </button>
                  ))}
                </div>
              )}
            ...
            <button onClick={submitDocument} disabled={uploading || !selectedFile || !category}
              ...
              style={{ background:uploading || !selectedFile || !category
                  ...
                  : "linear-gradient(135deg,#9F7AEA,#C4B5FD)",
                color: uploading || !selectedFile || !category ? "#8A9AB8" : "#04080F",
                boxShadow: uploading || !selectedFile || !category ? "none" : "0 0 32px rgba(159,122,234,0.4)",
                fontSize:16 }}>
              {uploading
          ...
          <div className="space-y-4">
            ...
            {submissions.length === 0 ? (
              <div className="py-12 text-center rounded-2xl"
                style={{ background:"#fff", border:"1px solid rgba(159,122,234,0.12)" }}>
                <Star size={36} color="rgba(159,122,234,0.2)" style={{ margin:"0 auto 12px" }}/>
                  ...
                  <div key={sub.id} className="p-4 rounded-2xl glow-surface"
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex items-center justify-center rounded-xl flex-shrink-0"
                        style={{ width:40, height:40, background:"rgba(159,122,234,0.08)", fontSize:20 }}>
