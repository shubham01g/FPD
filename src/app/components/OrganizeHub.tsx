...
const folders = [
  { id:1, name:"Legal Documents",  files:8,   size:"12.4 MB", color:"#2040C0", desc:"Wills, trusts, power of attorney" },
...
const remStatus = {
  ...
  completed:{ color:"#5A6A88", bg:"rgba(107,114,128,0.12)", label:"DONE" },
};
const occType = {
  birthday:    { color:"#9F7AEA", bg:"rgba(159,122,234,0.12)", icon:"🎂" },
  anniversary: { color:"#2040C0", bg:"rgba(32,64,192,0.12)",   icon:"💍" },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.8)", backdropFilter:"blur(8px)" }}>
      <div className="w-full max-w-sm rounded-2xl p-6" style={GLASS}>
        <div className="flex items-center justify-between mb-5"><h3 style={{ fontFamily:"var(--font-display)", fontSize:17, color:"#0D1428" }}>Add Reminder</h3><button onClick={onClose} style={{ color:"#5A6A88" }}><X size={15}/></button></div>
        <div className="space-y-3">
          {[{key:"title",label:"REMINDER TITLE",ph:"e.g. Update Will"},{key:"dueDate",label:"DUE DATE",ph:"e.g. Sep 15, 2026"},{key:"notes",label:"NOTES",ph:"Optional notes"}].map(f=>(
            <div key={f.key}><label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>{f.label}</label>
          ...
          <div><label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>FREQUENCY</label>
            <select value={form.frequency} onChange={e=>setForm(f=>({...f,frequency:e.target.value}))} className="w-full px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.2)", color:"#0D1428", fontSize:13, outline:"none" }}>
              <option>Annual</option><option>Quarterly</option><option>Monthly</option><option>6 months</option><option>One-time</option>
            </select>
          </div>
          <div><label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>CATEGORY</label>
            <select value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))} className="w-full px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.2)", color:"#0D1428", fontSize:13, outline:"none" }}>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={submit} disabled={loading} className="flex-1 py-2.5 rounded-xl font-semibold text-sm" style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", opacity:loading?0.7:1 }}>{loading?"Saving...":"Create Reminder"}</button>
            <button onClick={onClose} className="px-4 py-2.5 rounded-xl text-sm" style={{ background:"rgba(32,64,192,0.06)", color:"#5A6A88" }}>Cancel</button>
          </div>
        </div>
      </div>
    </div>
...
function AddOccasionModal({ onClose, onAdd }: { onClose:()=>void; onAdd:(o:Omit<Occasion,"id">)=>Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.8)", backdropFilter:"blur(8px)" }}>
      <div className="w-full max-w-sm rounded-2xl p-6" style={GLASS}>
        <div className="flex items-center justify-between mb-5"><h3 style={{ fontFamily:"var(--font-display)", fontSize:17, color:"#0D1428" }}>Add Occasion</h3><button onClick={onClose} style={{ color:"#5A6A88" }}><X size={15}/></button></div>
        <div className="space-y-3">
          ...
          <div><label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>TYPE</label>
            <select value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value as any}))} className="w-full px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.2)", color:"#0D1428", fontSize:13, outline:"none" }}>
              <option value="birthday">Birthday</option><option value="anniversary">Anniversary</option><option value="holiday">Holiday</option>
            </select>
          </div>
          <div className="flex gap-3 pt-1">
            <button onClick={submit} disabled={loading} className="flex-1 py-2.5 rounded-xl font-semibold text-sm" style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", opacity:loading?0.7:1 }}>{loading?"Saving...":"Save Occasion"}</button>
            <button onClick={onClose} className="px-4 py-2.5 rounded-xl text-sm" style={{ background:"rgba(32,64,192,0.06)", color:"#5A6A88" }}>Cancel</button>
          </div>
        </div>
      </div>
...
export function OrganizeHub() {
  ...
  const FOLDER_COLORS = ["#2040C0","#48BB78","#9F7AEA","#ED8936","#FC8181","#F6AD55","#4A90D9","#38B2AC"];
  ...
  return (
    <div className="p-6 space-y-5 relative" style={{ maxWidth:1100, ...GRID }}>
      <div>
        <h1 style={{ fontFamily:"var(--font-display)", fontSize:26, color:"#0D1428", marginBottom:4 }}>Organize</h1>
        <p style={{ color:"#5A6A88", fontSize:13 }}>Personal folders, reminders, and important family occasions — fully interactive in demo mode.</p>
      </div>
      <div className="flex gap-1 p-1 rounded-2xl" style={{ background:"rgba(255,255,255,0.95)", width:"fit-content" }}>
        {tabs.map(t=>(
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={()=>setShowAddFolder(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-2xl"
              style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", fontWeight:700, fontSize:13, boxShadow:"0 0 20px rgba(32,64,192,0.3)" }}>
              ...
              <button key={f.id} onClick={()=>toast.success(`Opened: ${f.name}`)}
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div style={{ color:"#0D1428", fontSize:14, fontWeight:500 }}>{f.name}</div>
                    <div style={{ color:"#8A9AB8", fontSize:10, ...MONO }}>{f.files} files · {f.size}</div>
                  </div>
                  {(f as any).locked && <span style={{ color:"#E53E3E", fontSize:9, ...MONO, fontWeight:700 }}>LOCKED</span>}
                </div>
                <div style={{ color:"#5A6A88", fontSize:12 }}>{f.desc}</div>
              </button>
            ))}
          </div>
          {/* Add folder modal */}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.7)", backdropFilter:"blur(6px)" }}>
              <div className="w-full max-w-sm rounded-2xl p-6 space-y-4" style={GLASS}>
                <div className="flex items-center justify-between">
                  <h3 style={{ fontFamily:"var(--font-display)", fontSize:17, color:"#0D1428" }}>Create New Folder</h3>
                  <button onClick={()=>setShowAddFolder(false)} style={{ color:"#5A6A88" }}><X size={15}/></button>
                </div>
                <div>
                  <label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>FOLDER NAME *</label>
                  <input value={newFolder.name} onChange={e=>setNewFolder(p=>({...p,name:e.target.value}))}
                    placeholder="e.g. Medical Directives"
                    className="w-full px-4 py-3 rounded-xl" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.2)", color:"#0D1428", fontSize:13, outline:"none" }}/>
                </div>
                <div>
                  <label style={{ color:"#5A6A88", fontSize:10, ...MONO, display:"block", marginBottom:5 }}>DESCRIPTION (optional)</label>
      ...
      {tab==="reminders" && (
        <div className="space-y-3">
          ...
          {reminders.length===0 && <div className="py-12 text-center rounded-2xl" style={GLASS}><Bell size={28} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/><div style={{ color:"#8A9AB8" }}>No reminders yet.</div></div>}
          {reminders.map(r=>{
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <Bell size={13} color="#2040C0"/>
                      <span style={{ color:"#0D1428", fontSize:14, fontWeight:500 }}>{r.title}</span>
                    </div>
                    <div className="flex flex-wrap gap-4 mb-1">
                      <span style={{ color:"#5A6A88", fontSize:12 }}>Due: <strong style={{ color:"#0D1428" }}>{r.dueDate}</strong></span>
                      <span style={{ color:"#5A6A88", fontSize:12 }}>Repeats: {r.frequency}</span>
                      <span className="px-2 py-0.5 rounded text-xs" style={{ background:"rgba(32,64,192,0.06)", color:"#5A6A88" }}>{r.category}</span>
                    </div>
                    {r.notes && <div style={{ color:"#8A9AB8", fontSize:11 }}>{r.notes}</div>}
                  </div>
      ...
      {tab==="occasions" && (
        <div className="space-y-4">
          ...
          {occasions.length===0 && <div className="py-12 text-center rounded-2xl" style={GLASS}><Calendar size={28} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 12px" }}/><div style={{ color:"#8A9AB8" }}>No occasions yet.</div></div>}
          <div className="grid md:grid-cols-2 gap-4">
            {occasions.map(o=>{
              ...
              return (
                <div key={o.id} className="p-5 rounded-2xl" style={GLASS}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1">
                      <div style={{ color:"#0D1428", fontSize:14, fontWeight:500 }}>{o.name}</div>
                  ...
                  <div style={{ color:"#5A6A88", fontSize:12, marginBottom:4 }}>{o.recipient}</div>
                  {o.notes && <div style={{ color:"#5A6A88", fontSize:11, fontStyle:"italic" }}>{o.notes}</div>}
                </div>
              );
            })}
          </div>
