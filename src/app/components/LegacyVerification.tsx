...
const INPUT: React.CSSProperties = { background:"rgba(91,110,225,0.05)", border:"1px solid rgba(91,110,225,0.2)", borderRadius:10, padding:"10px 14px", color:"var(--foreground)", fontSize:17.5, outline:"none", width:"100%" };
  ...
  return (
    <div className="p-6 space-y-6" style={{ maxWidth:900 }}>
      <div className="flex items-start justify-between">
        <div>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:32.5, color:"var(--foreground)", marginBottom:4 }}>Contact Verification</h1>
        ...
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-2xl"
          style={{ background:"linear-gradient(135deg,#5B6EE1,#5B6EE1)", color:"#070D1A", fontWeight:600, fontSize:17.5 }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="p-4 rounded-2xl text-center" style={CARD}>
            ...
            <div style={{ color:"var(--muted-foreground)", fontSize:14, ...MONO }}>{s.label.toUpperCase()}</div>
          </div>
        ))}
      </div>
          ...
          return (
            <div key={contact.id} className="p-6 rounded-2xl" style={CARD}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="flex items-center justify-center rounded-full font-semibold flex-shrink-0"
                    style={{ width:48, height:48, background:"rgba(91,110,225,0.1)", color:"var(--primary)", fontSize:20, fontFamily:"var(--font-display)" }}>
                    {contact.avatar}
                  </div>
                  <div>
                    <div style={{ color:"var(--foreground)", fontSize:20, fontWeight:500 }}>{contact.name}</div>
                    <div style={{ color:"var(--muted-foreground)", fontSize:16 }}>{contact.relationship}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl" style={{ background:sc.bg, color:sc.color }}>
                    {sc.icon}<span style={{ fontSize:14, ...MONO, fontWeight:600 }}>{sc.label}</span>
                  </div>
              ...
              <div className="grid md:grid-cols-3 gap-4 mt-4">
                <div className="flex items-center gap-2"><Mail size={13} color="var(--muted-foreground)"/><span style={{ color:"var(--muted-foreground)", fontSize:16 }}>{contact.email}</span></div>
                <div className="flex items-center gap-2"><Phone size={13} color="var(--muted-foreground)"/><span style={{ color:"var(--muted-foreground)", fontSize:16 }}>{contact.phone}</span></div>
                <div className="flex items-center gap-2"><UserCheck size={13} color="var(--muted-foreground)"/><span style={{ color:"var(--muted-foreground)", fontSize:16 }}>{contact.idType}</span></div>
              </div>

              <div className="mt-3 px-4 py-3 rounded-2xl" style={{ background:"#EAF0FC" }}>
                <div style={{ color:"var(--muted-foreground)", fontSize:14, ...MONO, marginBottom:2 }}>ACCESS TRIGGER</div>
                <div style={{ color:"var(--foreground)", fontSize:16 }}>{contact.accessTrigger}</div>
              </div>

              {contact.notes && (
                <div style={{ color:"var(--muted-foreground)", fontSize:15, marginTop:8, fontStyle:"italic" }}>{contact.notes}</div>
              )}
                ...
                {contact.status === "pending" && (
                  <>
                    ...
                    <button onClick={() => toast.success(`Invite resent to ${contact.email}`)}
                      className="text-xs px-3 py-1.5 rounded-2xl ml-auto"
                ...
                {contact.status === "id_submitted" && (
                  <>
                    ...
                    <button onClick={() => simulateVerify(contact.id)} disabled={verifying===contact.id}
                      className="text-xs px-3 py-1.5 rounded-2xl ml-auto flex items-center gap-1.5"
      ...
      <div className="p-6 rounded-2xl" style={CARD}>
        <h3 style={{ fontFamily:"var(--font-display)", fontSize:20, color:"var(--foreground)", marginBottom:16 }}>Verification Process</h3>
        <div className="grid md:grid-cols-4 gap-4">
          {[
            ...
            <div key={s.step} className="text-center">
              ...
              <div style={{ color:"var(--foreground)", fontSize:17.5, fontWeight:500, marginBottom:4 }}>{s.label}</div>
              <div style={{ color:"var(--muted-foreground)", fontSize:15 }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.7)", backdropFilter:"blur(4px)" }}>
          <div className="w-full max-w-lg rounded-2xl p-7 overflow-y-auto" style={{ ...CARD, maxHeight:"90vh" }}>
            <div className="flex items-center justify-between mb-6">
              <h3 style={{ fontFamily:"var(--font-display)", fontSize:25, color:"var(--foreground)" }}>Add Legacy Contact</h3>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={f.key}>
                  <label style={{ color:"var(--muted-foreground)", fontSize:15, display:"block", marginBottom:6 }}>{f.label}</label>
                  <input value={(form as any)[f.key]} onChange={F(f.key)} placeholder={f.placeholder} style={INPUT}/>
                </div>
              ))}
              <div>
                <label style={{ color:"var(--muted-foreground)", fontSize:15, display:"block", marginBottom:6 }}>ID TYPE THEY WILL SUBMIT</label>
              ...
              <div className="border-2 border-dashed rounded-2xl p-5 text-center" style={{ borderColor:"rgba(91,110,225,0.3)" }}>
                ...
                <div style={{ color:"var(--foreground)", fontSize:16, marginBottom:4 }}>Upload or Scan Their ID Now (optional)</div>
                <div style={{ color:"var(--muted-foreground)", fontSize:15, marginBottom:12 }}>Driver's License, Passport, or State ID</div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={addContact}
                  ...
                  style={{ background:"linear-gradient(135deg,#5B6EE1,#5B6EE1)", color:"#070D1A", fontSize:17.5 }}>
