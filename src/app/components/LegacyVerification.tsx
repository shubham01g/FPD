...
const INPUT: React.CSSProperties = { background:"rgba(32,64,192,0.05)", border:"1px solid rgba(32,64,192,0.2)", borderRadius:10, padding:"10px 14px", color:"var(--foreground)", fontSize:14, outline:"none", width:"100%" };
...
const statusConfig: Record<VerifStatus, { label:string; color:string; bg:string; icon:React.ReactNode }> = {
  ...
  id_submitted: { label:"ID SUBMITTED",  color:"#4A90D9", bg:"rgba(74,144,217,0.12)", icon:<Upload size={13}/> },
    ...
    <div className="p-6 space-y-6" style={{ maxWidth:900 }}>
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-xl"
          style={{ background:"linear-gradient(135deg,#2040C0,#4466D8)", color:"#070D1A", fontWeight:600, fontSize:14 }}>
      ...
      {/* Info banner */}
      <div className="flex gap-3 px-5 py-4 rounded-xl border" style={{ background:"rgba(74,144,217,0.06)", borderColor:"rgba(74,144,217,0.25)" }}>
        <Shield size={16} color="#4A90D9" style={{ flexShrink:0, marginTop:2 }}/>
            ...
            <div key={contact.id} className="p-6 rounded-2xl" style={CARD}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="flex items-center justify-center rounded-full font-semibold flex-shrink-0"
                    style={{ width:48, height:48, background:"rgba(32,64,192,0.1)", color:"var(--primary)", fontSize:16, fontFamily:"var(--font-display)" }}>
                    {contact.avatar}
                  </div>
                  <div>
              ...
              <div className="mt-3 flex items-center gap-3 flex-wrap">
                {contact.status === "pending" && (
                  <>
                    ...
                    <button onClick={() => toast.success(`Invite resent to ${contact.email}`)}
                      ...
                      style={{ color:"var(--primary)", background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.15)" }}>
                      Resend Invite
                    </button>
                  </>
                )}
                {contact.status === "id_submitted" && (
                  <>
                    <div className="flex items-center gap-2 text-xs" style={{ color:"#4A90D9" }}>
                ...
                {contact.status === "pending" && (
                  <button onClick={() => { setContacts(p=>p.map(c=>c.id===contact.id?{...c,status:"id_submitted"}:c)); toast.success("ID submission simulated"); }}
                    ...
                    style={{ color:"#4A90D9", background:"rgba(74,144,217,0.08)", border:"1px solid rgba(74,144,217,0.2)" }}>
                    Simulate ID Submit
                  </button>
                )}
          ...
          {[
            ...
            <div key={s.step} className="text-center">
              <div className="mx-auto mb-3 flex items-center justify-center rounded-full"
                style={{ width:36, height:36, background:"rgba(32,64,192,0.12)", color:"var(--primary)", fontFamily:"var(--font-display)", fontWeight:700 }}>
                {s.step}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.7)", backdropFilter:"blur(4px)" }}>
          <div className="w-full max-w-lg rounded-2xl p-7 overflow-y-auto" style={{ ...CARD, maxHeight:"90vh" }}>
            ...
            <div className="space-y-4">
              ...
              <div className="border-2 border-dashed rounded-xl p-5 text-center" style={{ borderColor:"rgba(32,64,192,0.3)" }}>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={addContact}
                  ...
                  style={{ background:"linear-gradient(135deg,#2040C0,#4466D8)", color:"#070D1A", fontSize:14 }}>
