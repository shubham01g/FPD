...
export function WGSessionTimer({ clientId, clientName, specialistName, onSessionEnd }: WGSessionTimerProps) {
  ...
  if (state === "idle") {
    return (
      <div className="rounded-2xl overflow-hidden" style={{ background:"rgba(32,64,192,0.03)", border:"1px solid rgba(32,64,192,0.12)" }}>
        <div className="px-4 py-3 border-b flex items-center gap-2"
          style={{ borderColor:"rgba(32,64,192,0.08)", background:"rgba(32,64,192,0.04)" }}>
          <Clock size={14} color="#2040C0"/>
          <span style={{ color:"#2040C0", fontSize:12, fontWeight:700, ...MONO }}>SESSION TIMER & BILLING</span>
          {hasCard
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex gap-2">
            {[["phone","📞 Phone Call"],["video","📹 Video Call"]].map(([t,label]) => (
              <button key={t} onClick={() => setSessionType(t as "phone"|"video")}
                ...
                style={{ background:sessionType===t?"rgba(32,64,192,0.1)":"rgba(32,64,192,0.04)",
                  border:`1.5px solid ${sessionType===t?"#2040C0":"rgba(32,64,192,0.1)"}`,
                  color:sessionType===t?"#2040C0":"#5A6A88" }}>
                {label}
              </button>
            ))}
          </div>
          <button onClick={startSession} disabled={!hasCard}
            ...
            style={{ background:hasCard?"linear-gradient(135deg,#2040C0,#3355E0)":"rgba(32,64,192,0.15)",
              color:hasCard?"#F0F4FA":"#8A9AB8",
              boxShadow:hasCard?"0 0 20px rgba(32,64,192,0.3)":"none" }}>
  ...
  if (state === "running" || state === "paused") {
    ...
    return (
      <div className="rounded-2xl overflow-hidden" style={{ background:state==="running"?"rgba(32,64,192,0.04)":"rgba(246,173,85,0.04)", border:`2px solid ${state==="running"?"rgba(32,64,192,0.25)":"rgba(246,173,85,0.35)"}` }}>
        {/* Timer display */}
        ...
        {/* Billing meter */}
        <div className="mx-4 mb-4 p-3 rounded-xl" style={{ background:"rgba(32,64,192,0.06)", border:"1px solid rgba(32,64,192,0.12)" }}>
          <div className="flex items-center justify-between mb-2">
            <span style={{ color:"#5A6A88", fontSize:11, ...MONO }}>CURRENT CHARGE</span>
            <span style={{ color:"#2040C0", fontSize:18, fontFamily:"var(--font-display)", fontWeight:700 }}>
            ...
            {Array.from({length: Math.max(currentBlocks, 1)}).map((_, i) => (
              <div key={i} className="flex items-center gap-1">
                <div className="flex items-center justify-center rounded-lg text-xs font-bold"
                  style={{ width:44, height:28, background:i < currentBlocks?"#2040C0":"rgba(32,64,192,0.1)", color:i < currentBlocks?"#fff":"#8A9AB8", fontFamily:"var(--font-mono)", fontSize:10 }}>
          ...
          <div className="h-1.5 rounded-full" style={{ background:"rgba(32,64,192,0.12)" }}>
            <div className="h-1.5 rounded-full transition-all" style={{ width:`${progressToNext}%`, background:"#2040C0" }}/>
          </div>
  ...
  if (state === "billing") {
    return (
      <div className="rounded-2xl overflow-hidden" style={{ background:"rgba(32,64,192,0.03)", border:"2px solid rgba(32,64,192,0.25)" }}>
        <div className="px-5 py-4 border-b text-center"
          style={{ borderColor:"rgba(32,64,192,0.1)", background:"rgba(32,64,192,0.05)" }}>
          <div style={{ color:"#2040C0", fontSize:13, fontWeight:700 }}>Confirm Session Billing</div>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={k} className="flex justify-between py-2 border-b" style={{ borderColor:"rgba(32,64,192,0.07)" }}>
            ...
            <div className="flex justify-between py-2">
              ...
              <span style={{ color:"#2040C0", fontSize:20, fontFamily:"var(--font-display)", fontWeight:700 }}>${amount.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
              ...
              style={{ background:"rgba(32,64,192,0.05)", border:"1px solid rgba(32,64,192,0.15)", color:"#0D1428", fontSize:12, outline:"none" }}/>
          </div>

          <div className="flex gap-3">
            <button onClick={confirmBilling}
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm"
              style={{ background:"linear-gradient(135deg,#2040C0,#3355E0)", color:"#F0F4FA", boxShadow:"0 0 20px rgba(32,64,192,0.3)" }}>
...
export function WGBillingHistory({ clientId }: { clientId?: string }) {
  ...
  if (records.length === 0) {
    return (
      <div className="py-6 text-center rounded-xl" style={{ background:"rgba(32,64,192,0.03)", border:"1px solid rgba(32,64,192,0.1)" }}>
        <DollarSign size={24} color="rgba(32,64,192,0.2)" style={{ margin:"0 auto 8px" }}/>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div style={{ color:"#2040C0", fontSize:13, fontWeight:700 }}>
      ...
      <div className="rounded-xl overflow-hidden" style={{ border:"1px solid rgba(32,64,192,0.1)" }}>
        <div className="grid px-4 py-2.5 border-b"
          style={{ gridTemplateColumns:"auto 1fr auto auto auto auto", gap:12, background:"#EAF0FC", borderColor:"rgba(32,64,192,0.1)", alignItems:"center" }}>
