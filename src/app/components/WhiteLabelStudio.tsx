...
const INPUT: React.CSSProperties = { background: "rgba(91,167,214,0.05)", border: "1px solid rgba(91,167,214,0.2)", color: "#FFFFFF", fontSize: 17.5, outline: "none", borderRadius: 12, padding: "11px 14px", width: "100%" };
      ...
      <div className="flex items-start gap-3.5 mb-6">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span style={{ ...MONO, color: "#6FAE8B", fontSize: 14 }}>STEP {step}</span>
          </div>
          <h3 style={{ ...DISPLAY, fontSize: 22.5, color: TEXT, fontWeight: 600, marginTop: 2 }}>{title}</h3>
          <p style={{ color: MUTED, fontSize: 16, marginTop: 3, lineHeight: 1.6 }}>{sub}</p>
        </div>
      </div>
      {children}
    </div>
...
function Field({ label, value, onChange, placeholder, icon, disabled }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; icon?: React.ReactNode; disabled?: boolean }) {
  return (
    <div>
      <label style={{ color: MUTED, fontSize: 14, ...MONO, display: "block", marginBottom: 7 }}>{label.toUpperCase()}</label>
  ...
  return (
    <div className="p-5 sm:p-6 rounded-2xl flex flex-col sm:flex-row sm:items-center gap-4 justify-between"
      ...
      <div className="flex items-start gap-3.5 min-w-0">
        ...
        <div className="min-w-0">
          <div style={{ ...DISPLAY, color: TEXT, fontSize: 20, fontWeight: 700 }}>White Label is locked</div>
          <p style={{ color: MUTED, fontSize: 16, lineHeight: 1.6, marginTop: 3 }}>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <span style={{ color: MUTED, fontSize: 14, ...MONO }}>LIVE PLATFORM PREVIEW</span>
      ...
      <div className="rounded-2xl overflow-hidden mx-auto transition-all" style={{ border: `1px solid ${primary}40`, boxShadow: `0 0 40px ${primary}18`, width: device === "mobile" ? 300 : "100%", background: "#070A12" }}>
        ...
        <div className="flex items-center justify-between px-4 py-3" style={{ background: "linear-gradient(90deg,rgba(91,110,225,0.10),transparent)", borderBottom: `1px solid ${primary}22` }}>
          <div className="flex items-center gap-2.5">
            <div style={{ width: 30, height: 30, borderRadius: 8, background: `linear-gradient(135deg,${primary},${accent})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800, color: "#fff", ...DISPLAY }}>{(logo || "WL").slice(0, 3)}</div>
            <div className="min-w-0">
              <div style={{ color: TEXT, fontSize: 16, fontWeight: 700, ...DISPLAY, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{name || "Your Brand"}</div>
              {device === "desktop" && <div style={{ color: MUTED, fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 220 }}>{tagline || "Your tagline here"}</div>}
            </div>
          </div>
        ...
        <div className="flex" style={{ minHeight: 168 }}>
          {device === "desktop" && (
            <div className="py-3 px-2 flex flex-col gap-1" style={{ width: 132, borderRight: `1px solid ${primary}18` }}>
              {navItems.map((n, i) => (
                <div key={n} className="px-2.5 py-2 rounded-xl" style={{ background: i === 0 ? `${primary}22` : "transparent", color: i === 0 ? "#fff" : MUTED, fontSize: 14, fontWeight: i === 0 ? 600 : 400 }}>{n}</div>
              ))}
            </div>
          )}
          <div className="flex-1 p-4">
            <div style={{ color: TEXT, fontSize: 17.5, fontWeight: 700, ...DISPLAY, marginBottom: 3 }}>Welcome back</div>
            <div style={{ color: MUTED, fontSize: 13.5, marginBottom: 12 }}>Here's your legacy at a glance.</div>
            <div className="grid grid-cols-3 gap-2 mb-3">
              {["Documents", "Contacts", "Storage"].map((k, i) => (
                <div key={k} className="rounded-2xl p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${primary}18` }}>
                  <div style={{ color: i === 0 ? primary : accent, fontSize: 19, fontWeight: 800, ...DISPLAY }}>{["128", "6", "62%"][i]}</div>
                  <div style={{ color: MUTED, fontSize: 10.5 }}>{k}</div>
                </div>
              ))}
            </div>
            <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl" style={{ background: `linear-gradient(135deg,${primary},${accent})`, color: "#fff", fontSize: 14, fontWeight: 700 }}>
      ...
      <div className="relative overflow-hidden p-7 sm:p-9" style={{ ...GLASS, background: "linear-gradient(135deg,#0E1830,#101728 60%)" }}>
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-4" style={{ background: "rgba(91,110,225,0.12)", border: "1px solid rgba(91,110,225,0.3)" }}>
            ...
            <span style={{ color: "#6FAE8B", fontSize: 14, letterSpacing: "0.12em", ...MONO }}>WHITE LABEL STUDIO</span>
          </div>
          <h1 style={{ ...DISPLAY, fontSize: "clamp(2.1rem,4.3vw,3.1rem)", fontWeight: 800, color: TEXT, lineHeight: 1.1, letterSpacing: "-0.02em", maxWidth: 640 }}>
          ...
          <p style={{ color: SOFT, fontSize: 19.5, lineHeight: 1.7, maxWidth: 560, marginTop: 12 }}>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-6">
            {["Your domain & branding", "Lifetime recurring commission", "Dedicated onboarding"].map(t => (
              <span key={t} className="flex items-center gap-2" style={{ color: MUTED, fontSize: 16 }}><CheckCircle size={14} color="#FFFFFF" /> {t}</span>
            ))}
          </div>
          <div className="mt-6 inline-flex items-center gap-2 px-3.5 py-2 rounded-2xl" style={{ background: config.enabled ? "rgba(72,187,120,0.12)" : "rgba(91,167,214,0.08)", border: `1px solid ${config.enabled ? "rgba(72,187,120,0.35)" : "rgba(91,167,214,0.2)"}` }}>
            ...
            <span style={{ color: config.enabled ? "#D99A6B" : MUTED, fontSize: 15.5, fontWeight: 600 }}>{config.enabled ? "White-label portal is LIVE" : "Draft — not published yet"}</span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <SectionCard step={2} icon={<Palette size={18} />} title="Brand colors" sub="Pick a preset or set exact hex values — they theme buttons, highlights, and accents everywhere.">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {([["Primary", "primaryColor"], ["Accent", "accentColor"]] as const).map(([lbl, key]) => (
                <div key={key}>
                  <label style={{ color: MUTED, fontSize: 14, ...MONO, display: "block", marginBottom: 7 }}>{lbl.toUpperCase()} COLOR</label>
          ...
          <SectionCard step={3} icon={<Globe size={18} />} title="Custom domain" sub="Run the platform on your own web address. We'll issue the SSL certificate automatically.">
            ...
            <div className="flex items-start gap-2.5 mt-4 p-3.5 rounded-2xl" style={{ background: "rgba(91,110,225,0.06)", border: "1px solid rgba(91,110,225,0.15)" }}>
              ...
              <p style={{ color: MUTED, fontSize: 15.5, lineHeight: 1.65 }}>
            ...
            <div className="space-y-3">
                      ...
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span style={{ ...DISPLAY, color: TEXT, fontSize: 19.5, fontWeight: 700 }}>{pkg.name}</span>
                            {pkg.badge && <span className="px-2 py-0.5 rounded-full" style={{ background: `${pkg.color}22`, color: pkg.color, fontSize: 12.5, fontWeight: 700, ...MONO }}>{pkg.badge}</span>}
                          </div>
                          <div style={{ color: MUTED, fontSize: 15, marginTop: 2 }}>{pkg.userLimitLabel} · <span style={{ color: "#D99A6B" }}>{pkg.commission}% lifetime commission</span></div>
                          <div style={{ color: MUTED, fontSize: 14, marginTop: 3, ...MONO }}>{bl.setup}</div>
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <div style={{ ...DISPLAY, color: on ? pkg.color : TEXT, fontSize: 25, fontWeight: 800, lineHeight: 1 }}>{bl.price}</div>
                        <div style={{ color: MUTED, fontSize: 13.5, marginTop: 2 }}>{bl.unit}</div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-x-4 gap-y-1.5 mt-4 pt-4" style={{ borderTop: `1px solid ${pkg.color}22` }}>
                        {pkg.features.map(f => (
                          <div key={f} className="flex items-start gap-2">
                            ...
                            <span style={{ color: SOFT, fontSize: 15.5, lineHeight: 1.5 }}>{f}</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {!activePackages.length && (
                <div className="p-5 rounded-2xl text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(91,167,214,0.25)" }}>
                  <p style={{ color: MUTED, fontSize: 16 }}>No partner packages are available right now. Contact our team to build a custom plan.</p>
                </div>
              )}
            </div>
          </SectionCard>
        ...
        <div className="lg:col-span-2 space-y-6 lg:sticky lg:top-6">
          <div className="p-5 sm:p-6" style={GLASS}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span style={{ ...DISPLAY, color: TEXT, fontSize: 20, fontWeight: 600 }}>Preview</span>
            </div>
          ...
          <div className="p-5 sm:p-6" style={GLASS}>
            <div className="space-y-2.5 mb-5">
              {[
                ...
                <div key={k as string} className="flex items-center justify-between gap-3 py-1.5" style={{ borderBottom: "1px solid rgba(91,167,214,0.1)" }}>
                  <span style={{ color: MUTED, fontSize: 15.5 }}>{k}</span>
                  <span style={{ color: SOFT, fontSize: 15.5, fontWeight: 600, textAlign: "right", maxWidth: 170, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{v}</span>
                </div>
              ))}
            </div>
            ...
            <p style={{ color: MUTED, fontSize: 14, lineHeight: 1.6, marginTop: 14, textAlign: "center" }}>
              {locked
